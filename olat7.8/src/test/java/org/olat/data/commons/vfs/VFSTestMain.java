...
public class VFSTestMain {
    ...
    private void doit() {
        ...
        /*
         * VFSContainer c2 = new LocalFolderImpl(new File("c:/temp/f2")); stats(c2); VFSContainer named = new NamedContainerImpl("namedcontainer", c2, true);
         * stats(named); MergeSource m = new MergeSource(cont, new VFSContainer[] { named}); stats(m); VFSItem q = m.resolveFile("/bookmarks.html"); VFSItem a =
         * m.resolveFile("/ifa/sammlung.zip"); VFSItem d = m.resolveFile("/ifa"); VFSItem b = m.resolveFile("/namedcontainer/zzz/zzztest.txt"); VFSItem f =
         * m.resolveFile("/namedcontainer"); VFSItem g = m.resolveFile("/namedcontainer/zzz"); VFSItem e = m.resolveFile("/"); //c2.copyFrom(cont); dump(m, 0);
         */
        // VFSContainer del = new LocalFolderImpl(new File("c:/temp/f2/felix/ifa"));
        // del.delete();

        /*
         * try { StreamedImpl st = new StreamedImpl("testit.html",new FileInputStream("c:/temp/felix/bookmarks.html")); c2.copyFrom(st); } catch (FileNotFoundException e)
