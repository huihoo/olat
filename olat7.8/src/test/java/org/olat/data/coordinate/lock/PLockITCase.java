 ...
 * 
 */
public class PLockITCase extends OlatTestCase {
    ...
    public void testReentrantLock() {
        // Ignore Test if DB is HSQLDB. HSQLDB does not support
        // LockMode.UPGRADE, which is a prerequisite for testing
        // PessimisticLockManager
        assumeTrue(!getHsqlDbConfigured());

        final long start = System.currentTimeMillis();
        final String asset = "p1";
        // make sure the lock is created first
        final PLock pc = pessimisticLockManager.findOrPersistPLock(asset);
        assertNotNull(pc);
        DBFactory.getInstance().closeSession();

        // test double acquisition within same transaction
        final PLock pc1 = pessimisticLockManager.findOrPersistPLock(asset);
        assertNotNull(pc1);
        final PLock pc2 = pessimisticLockManager.findOrPersistPLock(asset);
        assertNotNull(pc2);
        DBFactory.getInstance().closeSession();

        // and without explicit transaction boundary.
        final PLock p1 = pessimisticLockManager.findOrPersistPLock(asset);
        assertNotNull(p1);
        final PLock p2 = pessimisticLockManager.findOrPersistPLock(asset);
        assertNotNull(p2);
    ...
    public void testLockWaitTimout() {
        ...
        // make sure all three row entries for the locks are created, otherwise
        // the system-wide locking
        // applied on lock-row-creation cannot support row-level-locking by
        // definition.

        final PLock pc3 = pessimisticLockManager.findOrPersistPLock("blibli");
        assertNotNull(pc3);
        DBFactory.getInstance().closeSession();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    sleep(2500);
                    final PLock p3 = pessimisticLockManager.findOrPersistPLock("blibli");
                    assertNotNull(p3);
                } catch (final Exception e) {
                    exceptionHolder.add(e);
                } finally {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    final PLock p2 = pessimisticLockManager.findOrPersistPLock("blibli");
                    assertNotNull(p2);
                    sleep(60000);
                    // holding the lock for more than the transaction timeout
                    // (normally 30secs, configured where? hib) should cause a
                    // lock timeout
    ...
    public void testSingleRowLockingSupported() {
        ...
        // make sure both row entries for the locks are created, otherwise the
        // system-wide locking
        // applied on lock-row-creation cannot support row-level-locking by
        // definition.
        final PLock pc1 = pessimisticLockManager.findOrPersistPLock("blabla");
        assertNotNull(pc1);
        final PLock pc2 = pessimisticLockManager.findOrPersistPLock("blublu");
        assertNotNull(pc2);
        DBFactory.getInstance().closeSession();

        final List<Long> holder = new ArrayList<Long>(1);
        // first thread acquires the lock and waits and continues holding the
        // lock for some time.
        final PLock p1 = pessimisticLockManager.findOrPersistPLock("blabla");
        assertNotNull(p1);

        new Thread(new Runnable() {
            @Override
            public void run() {
                final PLock p2 = pessimisticLockManager.findOrPersistPLock("blublu");
                assertNotNull(p2);
                final long p2Acquired = System.currentTimeMillis();
        ...
        sleep(5000);
        final long p1AboutToRelease = System.currentTimeMillis();
        DBFactory.getInstance().closeSession();

        // if row locking is not supported, then the timestamp when p2 has been
        // acquired will be shortly -after- p1 has been released
        final boolean singleRowLockingOk = holder.size() > 0 && holder.get(0).longValue() < p1AboutToRelease;
    ...
    public void testNestedLockingSupported() {
        ...
        final List<Long> holder = new ArrayList<Long>(1);
        // first thread acquires the two locks and waits and continues holding
        // the lock for some time.
        final PLock p1 = pessimisticLockManager.findOrPersistPLock("blabla");
        assertNotNull(p1);
        final PLock p3 = pessimisticLockManager.findOrPersistPLock("blibli");
        assertNotNull(p3);
        ...
        sleep(5000);
        final boolean acOk = holder.size() == 0;
        DBFactory.getInstance().closeSession();
        sleep(5000);
        final boolean acNowOk = holder.size() == 1;
    ...
    public void testDeadLockTimeout() {
        ...
        /**
         * t1 t2 bla bli .. .. .. .. .. .. bli .. .. .. bla -> deadlock! t2 waits on bla (already acquired by t1, but t1 waits on bli, already acquired by t2)
         */
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    assertNotNull(p1);
                    sleep(2500);
                    // now try to acquire blibli but that fails, since blibli is
                    // already locked by thread 2.
                    // but thread 2 cannot continue either, since it is waiting
                    // for lock blabla, which is already hold by thread 1
                    // -> deadlock
                    final PLock p3 = pessimisticLockManager.findOrPersistPLock("blibli");
                    assertNotNull(p3);
                } catch (final Exception e) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    assertNotNull(p2);
                    sleep(5000);
                    final PLock p3 = pessimisticLockManager.findOrPersistPLock("blabla");
                    assertNotNull(p3);
                } catch (final Exception e) {
                    exceptionHolder.add(e);
                } finally {
    ...
    public void testPerf() {
        ...
        // take 500 threads (created and started with no delay (as fast as the
        // vm can) trying to acquire a plock on 20 different olatresourceables.
        // measure how long that takes. and warn if it exceeds an upper
        // boundary.
        // the server is assumed to have around 2GHz cpu and 2GBytes RAM.
        // the first thread to acquire a new olatres will first lock on the
        // global lock and then create the new entry to lock upon.
        // we therefore also measure how long it takes again when all locks have
        // already been inserted into the plock table.
        // results: on a std. laptop with postgres 8, 500 threads with 20
        // resourceables take about 3000ms (thread creation inclusive)
        // -> about

        // 1. prepare collection
        final int numthreads = 500;
        final int numores = 1;
        final int maxwait = 12; // seconds to wait for completion (upper
        ...
        for (i = 0; i < maxwait; i++) {
            final int donecnt = c.getThreadsDoneCnt();
        ...
        // 4. wait till all are finished or it takes too long
        for (i = 0; i < maxwait; i++) {
            final int donecnt = c2.getThreadsDoneCnt();
            if (donecnt < numthreads) {
                try {
                    Thread.sleep(1000);
                } catch (final InterruptedException e) {
    ...
    public void testSync() {
        ...
        for (int i = 0; i < MAX_COUNT + MAX_USERS_MORE; i++) {
            final Identity i1 = JunitTestHelper.createAndPersistIdentityAsUser("u" + i);
            identities.add(i1);
            System.out.println("testSync: Identity=" + "u" + i + "created");
            DBFactory.getInstance().closeSession();
        }

        final SecurityGroup group2 = securityManager.createAndPersistSecurityGroup();
        // make sure the lock has been written to the disk (tests for
        // createOrFind see other methods)
        DBFactory.getInstance().closeSession();
        // PLock p1 = pessimisticLockManager.findOrPersistPLock("befinsert");
        // assertNotNull(p1);
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        final Identity id = identities.get(j);
                        //
                        final PLock p2 = pessimisticLockManager.findOrPersistPLock("befinsert");
                        assertNotNull(p2);
        ...
        // now count
        DBFactory.getInstance().closeSession();
        final int cnt2 = securityManager.countIdentitiesOfSecurityGroup(group2);
    ...
    void doNoLockingEnrol(final Identity i, final SecurityGroup group) {
        // check that below max
        try {
            final StringBuilder sb = new StringBuilder();
            final int cnt = securityManager.countIdentitiesOfSecurityGroup(group);
            sb.append("enrol:cnt:" + cnt);
            if (cnt < MAX_COUNT) {
                // now sleep a while to allow others to think also that there is
                // still space left in the group
                sleep(100);
                // now add the user to the security group
                sb.append(" adding " + i.getName() + ": current.. " + cnt + ", max = " + MAX_COUNT);
                securityManager.addIdentityToSecurityGroup(i, group);
            }
