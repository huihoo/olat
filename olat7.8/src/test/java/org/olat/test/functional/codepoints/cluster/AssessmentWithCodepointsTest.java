...
/**
 * Tests the Assessment (AssessmentTool, TestRun, EfficiencyStatement) - in cluster mode. <br/>
 * 
 * Test setup: <br/>
 * Expects a special "AssessmentTool" course. It is also supposed that there is a learning group associated with this course which contains at least one student and one
 * tutor. <br/>
 * Test case: <br/>
 * - Student runs a test (Test 1) in a special "AssessmentTool" course. <br/>
 * - Tutor opens the assessment tool of the same course for the specified student and checks whether he see the correct score. Tutor changes the score. <br/>
 * - Student should see the correct score in course run as in "Evidence of achievement". <br/>
 * Details: <br/>
 * Test the doInSync update of the EfficiencyStatement for the STUDENT for the "AssessmentTool" course. <br/>
 * Setup: STUDENT runs a test "Test 1", score (2) is stored. <br/>
 * If useCodepoints is false: STUDENT stores score (1) and tutor score (7), in this order; else if useCodepoints is true: TUTOR stores score (7) and STUDENT stores score
 * (1), in this order.
 * 
 * 
...
public class AssessmentWithCodepointsTest extends BaseSeleneseTestCase {
    ...
    /**
     * Tests assessment tool. Student runs a test (Test 1) in a special "AssessmentTool" course. Tutor opens the assessment tool of the same course for the specified
     * student and checks whether he see the correct score. Tutor changes the score. Student should see the correct score in course run as in "Evidence of achievement"
     * 
     * @throws Exception
     */
    public void testEfficiencyStatementUpdate() throws Exception {
        ...
        boolean useCodepoints = false; // LD: 6.12.2012 do not run with codepoints anymore, because it fails too often (error cause: OLAT-3570)
    ...
    /**
     * Test the doInSync update of the EfficiencyStatement for the STUDENT for the "AssessmentTool" course. Uses Codepoints.
     * <p>
     * Setup: STUDENT runs a test "Test 1", score (2) is stored.
     * <p>
     * If useCodepoints is false: STUDENT stores score (1) and tutor score (7), in this order. else if useCodepoints is true: TUTOR stores score (7) and STUDENT stores
     * score (1), in this order
     * 
     * @param useCodepoints
     * @throws Exception
     */
    private void doTestEfficiencyStatementUpdateWithCodepoints(boolean useCodepoints) throws Exception {
        ...
        CodepointRef beforeSyncCp_A = codepointClient_A
                .getCodepoint("org.olat.lms.course.assessment.NewCachePersistingAssessmentManager.beforeSyncUpdateUserEfficiencyStatement");
        beforeSyncCp_A.setHitCount(0);
        if (useCodepoints)
            beforeSyncCp_A.enableBreakpoint();

        CodepointRef doInSyncCp_A = codepointClient_A
                .getCodepoint("org.olat.lms.course.assessment.NewCachePersistingAssessmentManager.doInSyncUpdateUserEfficiencyStatement");
        doInSyncCp_A.setHitCount(0);
        if (useCodepoints)
            doInSyncCp_A.enableBreakpoint();

        codepointClient_B = Context.getContext().createCodepointClient(2);
        CodepointRef doInSyncCp_B = codepointClient_B
                .getCodepoint("org.olat.lms.course.assessment.NewCachePersistingAssessmentManager.doInSyncUpdateUserEfficiencyStatement");
        doInSyncCp_B.setHitCount(0);
        if (useCodepoints)
            doInSyncCp_B.enableBreakpoint();

        CodepointRef afterSyncCp_B = codepointClient_B
                .getCodepoint("org.olat.lms.course.assessment.NewCachePersistingAssessmentManager.afterSyncUpdateUserEfficiencyStatement");
        ...
        // what tutor sees
        /*
         * if(useCodepoints) { //assert score on "Struktur 1" to be the sum of the "Test 1" and "Bewertung 1" scores assertEquals("1.000",
         * selenium_B.getText("ui=course::assessment_scoreInTable(title=Struktur 1)")); //assert score on "Struktur 2" to be the half of sum of the "Test 1" and
         * "Bewertung 1" scores assertEquals("0.500", selenium_B.getText("ui=course::assessment_scoreInTable(title=Struktur 2)")); } else {
         */
        ...
        String achievedScore = selenium_A.getText("ui=qti::achievedScore()");
        for (int i = 0; i < 10 && !("1".equals(achievedScore)); i++) {
            achievedScore = selenium_A.getText("ui=qti::achievedScore()");
            SeleniumHelper.safeWait(100);
            System.out.println("achieved score read " + i + " times");
        }
        assertEquals("1", achievedScore);
        ...
        if (useCodepoints) {
            ...
            assertEquals("7.000", selenium_A.getText("ui=course::assessment_scoreInTable(title=Test 1)"));
        }
