...
/**
 * Tests course editing in clustered mode, and that the GUI locking works, that is a course could be edited by only a single user at a time. <br/>
 * Test setup: <br/>
 * Needs a clean "Demo Course".</br>
 * 
 * Test case: <br/>
 * The cluster_B node should be able the see the latest inserted node by cluster_A node after the course lock release. </br> - cluster_A: inserts a Structure course
 * element, assertBreakpointReached </br> - cluster_B: try to open the course editor of the same course but gets a "course locked" warning </br> - cluster_A: continues
 * the thread </br> - cluster_A: closes course editor </br> - cluster_B: opens editor, selects the first Structure node, changes the title, closes the editor </br>
 * 
...
public class CourseEditingTest extends BaseSeleneseTestCase {
    ...
    /**
     * Needs a clean "Demo Course".</br> Tests course editing in clustered mode. The cluster_B node should be able the see the latest inserted node by cluster_A node
     * after the course lock release. </br> - cluster_A: inserts a Structure course element, assertBreakpointReached </br> - cluster_B: try to open the course editor of
     * the same course but gets a "course locked" warning </br> - cluster_A: continues the thread </br> - cluster_A: closes course editor </br> - cluster_B: opens editor,
     * selects the first Structure node, changes the title, closes the editor </br>
     * 
     * @throws Exception
     */
    public void testCourseEditing() throws Exception {
        ...
        CodepointRef startInsertCp_A = codepointClient_A.getCodepoint("org.olat.presentation.course.editor.EditorMainController.startInsertNode");
        ...
        // selenium_B.waitForPageToLoad("30000");
        /*
         * for (int second = 0;; second++) { //TODO: ld: for some reason this check doesn't work if (second >= 120) fail("timeout"); try { if
         * (selenium_B.isTextPresent("This course is currently edited by")) break; } catch (Exception e) {} Thread.sleep(1000); }
         */
