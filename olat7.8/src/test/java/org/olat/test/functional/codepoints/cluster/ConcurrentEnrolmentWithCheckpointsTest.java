 ...
 * 
 * Test case: <br/>
 * 5 users try to subscribe to group with just one place, only one should get into group. <br/>
...
public class ConcurrentEnrolmentWithCheckpointsTest extends BaseSeleneseTestCase {
    ...
    public void testConcurrentEnrolmentWithCodepointsTest() throws Exception {
        ...
        CodepointRef doInSync1Cp_1 = codepointClient_1.getCodepoint("org.olat.lms.course.EnrollmentManager.doInSync1");
        doInSync1Cp_1.setHitCount(0);
        doInSync1Cp_1.enableBreakpoint();

        CodepointRef doInSync2Cp_1 = codepointClient_1.getCodepoint("org.olat.lms.course.EnrollmentManager.doInSync2");
        doInSync2Cp_1.setHitCount(0);
        doInSync2Cp_1.enableBreakpoint();

        CodepointRef afterDoInSyncCp_1 = codepointClient_1.getCodepoint("org.olat.lms.course.EnrollmentManager.afterDoInSync");
        ...
        CodepointRef doInSyncCp_2 = codepointClient_2.getCodepoint("org.olat.lms.course.EnrollmentManager.doInSync1");
...

        {
            ...
            EnrolmentRun enrolmentRun = courseRun.selectEnrolment("Enrollment");
        ...
        // All students enrol, only student01 should get into group
        {
            ...
            assertEquals("expected  1 winner", 1, numWinners);
            assertEquals("expected  1 looser", 1, numLoosers);

            // OLAT-6702: screen update implemented for enrollment table
            // all other users should have no more "Enrol" link (GUI update via cluster message)
            // or if the link is still around (message latency) should not be able to enrol
            for (int i = 2; i < 5; i++) {
                try {
                    seleniums[i].waitForPageToLoad("30000");
                    seleniums[i].click("ui=course::content_enrollment_enrolOnGroup(nameOfGroup=learning group selenium 5)");
                } catch (Exception ex) {
                    // expected
                    continue;
                }
                if (!seleniums[i].isTextPresent("In the meantime this group is complete. Please select another one.")) {
                    fail("student0" + (i + 1) + " must not be able to enrol.");
                }
            }
        }
        {
            ...
            /*
             * OLATWorkflowHelper workflow = context.getOLATWorkflowHelper(context.getStandardAuthorOlatLoginInfos(1)); LRDetailedView lRDetailedView =
             * workflow.getLearningResources().searchMyResource(COURSE_NAME); lRDetailedView.deleteLR();
             */
        }
    }
}
