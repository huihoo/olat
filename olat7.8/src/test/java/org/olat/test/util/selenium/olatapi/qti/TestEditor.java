...
public class TestEditor extends OLATSeleniumWrapper {
    ...
    /**
     * Checks the question type. QUESTION_TYPES: SINGLE_CHOICE, MULTIPLE_CHOICE, KPRIM, GAP_TEXT
     * 
    ...
    /**
     * Try to delete the current selected section/question node but not possible. A Test must contain at least a section node.
     * 
     * @throws Exception
     */
    public void deleteUndeleteable(boolean isSection) throws Exception {
        ...
        for (int second = 0;; second++) {
            if (second >= 60)
                fail("timeout");
            try {
                if (selenium.isTextPresent(cannotBeDeteledMsg))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
