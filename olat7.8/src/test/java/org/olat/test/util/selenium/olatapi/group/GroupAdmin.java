...
public class GroupAdmin extends OLATSeleniumWrapper {
    ...
    /**
     * Import participants to this group. Accepts a formatted input string, one username per row. Assumes that there is at least a new user in the participants list
     * 
     * @param participants
     */
     ...
     */
    /*
     * public CourseRun close() { selenium.click("ui=groupManagement::toolbox_groupManagement_close()"); selenium.waitForPageToLoad("30000"); return new
     * CourseRun(selenium); }
     */
    ...
    /**
     * Toggle tool selection, if in Administration/Tools page. (e.g. if you want to change the selection ON/OFF for wiki, just pass true for this parameter)
     * 
     * @param toggleInfo
     ...
     * @param toggleWiki
     */
    public void setTools(boolean toggleInfo, boolean toggleContactForm, boolean toggleCalendar, boolean toggleFolder, boolean toggleForum, boolean toggleWiki,
            boolean toggleChat) {
        selectToolsTab();
        if (toggleInfo) {
    ...
    public void selectCalendarWriteAccess(String text) {
        ...
        if (selenium.isElementPresent("ui=group::content_tools_calendarAccess_save()")
                && !selenium.isChecked("ui=group::content_tools_calendarAccess_calendarAccess(label=" + text + ")")) {
    ...
    /**
     * Change displayXXX if the corresponding input is not null. If one input param is null, do nothing for that one.
     * 
     * @param dsiplayOwners
    ...
    public void setMemberDisplayOptions(Boolean displayOwners, Boolean displayParticipants, Boolean displayWaitingList) {
        ...
        if (displayParticipants != null && displayParticipants
                && !selenium.isChecked("ui=commons::flexiForm_labeledCheckbox(formElementLabel=Members can see participants)")) {
            selenium.click("ui=commons::flexiForm_labeledCheckbox(formElementLabel=Members can see participants)");
        } else if (displayParticipants != null && !displayParticipants
                && selenium.isChecked("ui=commons::flexiForm_labeledCheckbox(formElementLabel=Members can see participants)")) {
