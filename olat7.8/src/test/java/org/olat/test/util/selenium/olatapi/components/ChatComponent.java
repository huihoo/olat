...
public class ChatComponent extends OLATSeleniumWrapper {
    ...
    /**
     * Works for any chat window: course/group/buddy, provided there is only one chat window open, else use the method with chatroomLabel parameter. <br/>
     * This should be called only after calling <code>openChat</code> or <code>openProjectGroupChat</code>.
     * 
     * @param msg
     */
     ...
     * @param username
     * 
     *            public void openBuddyChat(String username) { selenium.click("ui=buddiesChat::openOnlineUserList()"); //selenium.waitForPageToLoad("90000");
     *            sleepThread(3000); //selenium.selectWindow(selenium.getAllWindowTitles()[1]);
     * 
     *            selenium.selectWindow(selenium.getAllWindowTitles()[2]); selenium.windowFocus(); assertTrue(selenium.isTextPresent("List")); }
     */
