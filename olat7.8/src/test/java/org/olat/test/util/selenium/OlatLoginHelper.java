    ...
    public static void olatLogin(Selenium selenium, OlatLoginInfos loginInfos) {
        ...
        try {
            for (int second = 0;; second++) {
                ...
                try {
                    if ("OLAT - Online Learning And Training".equals(selenium.getTitle()))
                        break;
                } catch (Exception e) {
                }
                sleep(500);
            }
            if (selenium.isTextPresent("OLAT-Benutzername") && !language.equals("Deutsch")) {
            ...
            login: while (true) {
                ...
                if (selenium.isElementPresent("ui=home::menu_settings()") && language.equals("English")) {
                    // language is correct
                    break;
                }
                if (selenium.isElementPresent("ui=home::menu_einstellungen()") && language.equals("Deutsch")) {
                    // language is correct
                    break;
                }

                if (!selenium.isElementPresent("ui=home::menu_einstellungen()") && !selenium.isElementPresent("ui=home::menu_settings()")) {
                ...
                if (selenium.isElementPresent("ui=home::menu_einstellungen()")) {
                    ...
                    System.out.println("We're in german, lets set the correct language");
                    selenium.click("ui=home::menu_einstellungen()");
                    selenium.waitForPageToLoad("30000");
                    selenium.click("ui=home::content_einstellungen_tabs_system()");
                    selenium.waitForPageToLoad("60000");
                    selenium.select("ui=home::content_einstellungen_system_sprache()", "label=" + language);
                    selenium.click("ui=home::content_einstellungen_system_speichern()");
                    selenium.waitForPageToLoad("30000");
                    System.out.println("HTMLSOURCE: " + selenium.getHtmlSource());
                    sleep(600);
                    ...
                    // then we're in german, lets switch to english
                    System.out.println("We're in english, lets set the correct language");
                    selenium.click("ui=home::menu_settings()");
                    selenium.waitForPageToLoad("30000");
                    selenium.click("ui=home::content_settings_tabs_system()");
                    selenium.waitForPageToLoad("30000");
                    selenium.select("ui=home::content_settings_system_general_language()", "label=" + language);
            ...
            System.out.println("login of user " + username + " to url " + selenium.getLocation() + " successful");
            success = true;
        } finally {
