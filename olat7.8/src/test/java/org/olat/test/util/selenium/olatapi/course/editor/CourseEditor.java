...
/**
 * OLAT abstraction for the CourseEditor. One can get a CourseEditor instance either via the CourseRun, via the LRDetailedView, or via LearningResources
...
public class CourseEditor extends OLATSeleniumWrapper {
    ...
    public enum CourseElemTypes {
        STRUCTURE, SINGLE_PAGE, EXTERNAL_PAGE, CP_LEARNING_CONTENT, SCORM_LEARNING_CONTENT, FORUM, WIKI, FILE_DIALOG, FOLDER, ASSESSMENT, TASK, TEST, SELF_TEST, QUESTIONNAIRE, ENROLMENT, CONTACT_FORM, BLOG, PODCAST, TOPIC_ASSIGNMENT, CALENDAR, LTI_PAGE, INFO_MESSAGE
    }

    // ALL SUPPORTED INSERT TYPES
    public enum InsertPosition {
        FIRST_CHILD_OF_ROOT, LAST_CHILD_OF_ROOT, FIRST_CHILD_OF_ELEMENT, FOLLOWING_SIBLING_OF_ELEMENT
    }
    ...
    /**
     * Insert a course element of the given type, with the newTitle if any provided (newTitle!=null). If newTitle==null the element gets the default title.
     * 
     * @param elementType
     * @param asFirstChildOfRoot
     *            , if true as first child, else as last child
     * @param newTitle
     * @return Returns an instance of the CourseElementEditor by default, or an EnrolmentEditor if an enrolment element was selected, or an TestElementEditor,
     *         SelfTestElementEditor, FolderEditor, SinglePageEditor, AssessmentEditor.
     */
    ...
    /**
     * Inserts a course element of the given type, in the given position (one of the possible InsertType) relative to the element with elementTitle. If newTitle==null the
     * element gets the default title.
     * <p>
    ...
    /**
     * Moves the current selected course element and inserts it accordingly with the insertPosition.
     * <p>
    ...
    /**
     * Straightforward course publish. Click "Publish", "Select all", "Next", select "All registered users", "Finish".
     * 
     */
    ...
    public void publishCourseAfterCourseTitleChanged() throws Exception {
        ...
        for (int second = 0;; second++) {
            if (second >= 60)
                fail("timeout");
            try {
                if (selenium.isTextPresent("Publishing"))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
        ...
        for (int second = 0;; second++) {
            if (second >= 60)
                fail("timeout");
            try {
                if (selenium.isTextPresent("Selected modifications published successfully"))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
     ...
     * 
     * @param title
     * @return Returns an instance of the CourseElementEditor by default, or an EnrolmentEditor if an enrolment element was inserted, or an TestElementEditor,
     *         SelfTestElementEditor, FolderEditor, SinglePageEditor, AssessmentEditor.
     */
    ...
    /**
     * Selects the root if title provided, else assumes the root is selected. Per default the root is selected at open course editor.
     * 
     * @return
     */
    ...
    public void chooseWikiForElement(String wikiTitle, String authorName) {
        ...
        selenium.click("ui=courseEditor::content_bbWiki_chooseWiki()");
        selenium.waitForPageToLoad("30000");
        // the search button was temporary removed due to OLAT-6697
