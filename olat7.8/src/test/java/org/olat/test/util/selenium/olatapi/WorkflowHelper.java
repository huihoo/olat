...
/**
 * This class contains helper methods used by tests in order to setup olat server(s) as single vm, cluster mode-single node, cluster-mode-multiple nodes.
 * <p>
 * 
 * @author Stefan
 * 
...
public class WorkflowHelper {
    ...
    /**
     * Find a file inside the seleniumtests dir. Returns file if any found, null otherwise.
     * 
    ...
    public static void deleteAllLearningResourcesFromAuthor(String author) {
        ...
        while (System.currentTimeMillis() < maxEnd) {
            ...
            if (bodyText.contains("cannot be deleted")) {
                ...
                if (!selenium.isElementPresent("ui=learningResources::content_firstLearningResourceInTable()")) {
                    // throw new
                    // AssertionError("Got an ERROR box saying that the course is referenced. Tried to delete the referred course. But can't find it. Other author?");
    ...
    public static OlatLoginInfos createUserIfNotExists(OlatLoginInfos loginInfos, String username, String password, boolean isSystemUser, boolean userManagementRole,
            boolean groupManagementRole, boolean authorRole, boolean systemAdminRole) throws InterruptedException {

        if (Context.getContext().getStandardAdminOlatLoginInfos(1).getUsername().equals(username)) {
            throw new IllegalArgumentException("Thou shall not use an existing username! (" + username + ")");
        }
        if (Context.getContext().getStandardAuthorOlatLoginInfos(1).getUsername().equals(username)) {
            throw new IllegalArgumentException("Thou shall not use an existing username! (" + username + ")");
        }
        if (Context.getContext().getStandardStudentOlatLoginInfos(1).getUsername().equals(username)) {
            throw new IllegalArgumentException("Thou shall not use an existing username! (" + username + ")");
        }
        if (Context.getContext().getStandardGuestOlatLoginInfos(1).getUsername().equals(username)) {
            throw new IllegalArgumentException("Thou shall not use an existing username! (" + username + ")");
        }
        ...
        if (!password.matches(".*[0-9].*")) {
            throw new IllegalArgumentException("password must contain an numerical value as well mate");
        }
        if (username.matches(".*[A-Z].*")) {
            throw new IllegalArgumentException("username must not contain CAPITAL letters!");
        }
        ...
        if (selenium.isTextPresent("No user was found with these attributes.")) {
            ...
            System.out.println("Create the user '" + username + "', couldn't find it.");
            ...
            if (selenium.isTextPresent("No user was found with these attributes.")) {
                throw new IllegalStateException("Couldnt create user " + username);
            }
            // perfect!
        } else {
        ...
        if (userManagementRole != selenium.isChecked("ui=userManagement::content_userdetail_roles_isUsermanager()")) {
        ...
        if (groupManagementRole != selenium.isChecked("ui=userManagement::content_userdetail_roles_isGroupmanager()")) {
        ...
        if (authorRole != selenium.isChecked("ui=userManagement::content_userdetail_roles_isAuthor()")) {
            if (authorRole) {
        ...
        if (systemAdminRole != selenium.isChecked("ui=userManagement::content_userdetail_roles_isAdmin()")) {
            if (systemAdminRole) {
        ...
        try {
            return new OlatLoginInfos(loginInfos.getSeleniumHostname(), loginInfos.getSeleniumBrowserId(), loginInfos.getFullOlatServerUrl(), username, password);
    ...
    /**
     * 1. Imports course if not already imported 2. Modifies access - change course access to "All registered OLAT users" 3. Publishes course
     * 
     * @param zippedCourse
     ...
     */
    public static void importCourse(File zippedCourse, String newTitleOfCourse, String newDescriptionOfCourse) throws InterruptedException {
        ...
        selenium_1.waitForPageToLoad("30000");
        selenium_1.click("ui=learningResources::toolbox_learningResource_modifyProperties()");
        selenium_1.waitForPageToLoad("30000");
     ...
     * 
     */
    public static void addTutorToGroup(String tutorUsername, String courseName, String groupName) {
        Selenium selenium = Context.getContext().createSeleniumAndLogin(Context.getContext().getStandardAdminOlatLoginInfos(1));
        ...
        selenium.click("ui=commons::flexiForm_genericLink(buttonLabel=Search)");
        selenium.waitForPageToLoad("30000");
        selenium.click("ui=groupManagement::content_learningGroupsEditor_addMembers_checkUser(username=" + tutorUsername + ")");
        selenium.click("ui=commons::usertable_adduser_choose()");
        selenium.waitForPageToLoad("30000");
    ...
    /**
     * Helper method. Makes a copy of the course courseName, with the given name, and publishes as public visible.
     * 
     * @param context
     * @param courseName
    ...
    /*
     * public static void deleteAllFromMyEntriesStartingWith(OlatLoginInfos loginInfos, String str) { System.out.println("=====================================");
     * System.out.println("DELETE ALL FROM MYENTRIES STARTING WITH: " + str + " for user "+loginInfos.getUsername());
     * System.out.println("                                START"); System.out.println("====================================="); Selenium selenium =
     * Context.getContext().createSeleniumAndLogin(loginInfos); int cnt = 0; boolean all = false; while (true) { System.out.println("COUNTER: "+cnt);
     * selenium.click("ui=tabs::learningResources()"); selenium.waitForPageToLoad("30000"); selenium.click("ui=learningResources::menu_myEntries()");
     * selenium.waitForPageToLoad("30000"); selenium.type("ui=learningResources::content_searchInTableField()", str); try{ System.out.println("Waiting 1 sec...");
     * Thread.sleep(1000); } catch(InterruptedException ie) { ie.printStackTrace(System.out); } selenium.submit("ui=learningResources::content_searchInTableForm()");
     * selenium.waitForPageToLoad("30000");
     * 
     * 
     * // now loop through the list until there is no learning resource left to be deleted if
     * (!selenium.isElementPresent("ui=learningResources::content_firstLearningResourceInTable()")) { all = true; break; }
     * selenium.click("ui=learningResources::content_firstLearningResourceInTable()"); selenium.waitForPageToLoad("30000"); String resourceTitle =
     * selenium.getText("//div[span/a/span/text()='Back']//h4"); System.out.println("Resource title: "+resourceTitle); if (!resourceTitle.startsWith(str)) { throw new
     * IllegalStateException("resource title not what we were looking for: "+resourceTitle+". search str: "+str); }
     * selenium.click("ui=learningResources::toolbox_learningResource_delete()"); selenium.waitForPageToLoad("30000"); selenium.click("ui=groups::content_deleteYes()");
     * selenium.waitForPageToLoad("30000"); final String bodyText = selenium.getBodyText(); if (bodyText.contains("cannot be deleted")) {
     * System.out.println("DELETION FAILED FOR SOME REASON... GIVING UP"); System.out.println(bodyText); break; } else if (bodyText.contains("Entry deleted.")){
     * System.out.println("YESSS, WE DELETED A RESOURCE!!! "); cnt++; continue; } else {
     * System.out.println("COULDNT FIGURE OUT WHETHER DELETION WAS SUCCESSFUL OR NOT, GIVING UP..."); System.out.println(bodyText); break; } } selenium.close();
     * selenium.stop(); System.out.println("====================================="); System.out.println("DELETE ALL FROM MYENTRIES STARTING WITH: " + str);
     * System.out.println(" (deleted " + cnt + " entries, all="+all+")"); System.out.println("                                 DONE");
     * System.out.println("=====================================");
     * 
     * }
     * 
     * public static void deleteAllCoursesStartingWith(String str) { System.out.println("=====================================");
     * System.out.println("DELETE ALL COURSES STARTING WITH: " + str); System.out.println("                                START");
     * System.out.println("====================================="); Selenium selenium = Context.getContext().createSeleniumAndLogin(); int cnt = 0; boolean all = false;
     * while (true) { System.out.println("COUNTER: "+cnt); selenium.click("ui=tabs::learningResources()"); selenium.waitForPageToLoad("30000");
     * selenium.click("ui=learningResources::menu_searchForm()"); selenium.waitForPageToLoad("30000");
     * selenium.type("ui=learningResources::content_searchForm_titleField()",str); selenium.click("ui=learningResources::content_searchForm_search()");
     * selenium.waitForPageToLoad("30000");
     * 
     * 
     * // now loop through the list until there is no learning resource left to be deleted if
     * (!selenium.isElementPresent("ui=learningResources::content_firstLearningResourceInTable()")) { all = true; break; }
     * selenium.click("ui=learningResources::content_firstLearningResourceInTable()"); selenium.waitForPageToLoad("30000");
     * selenium.click("ui=learningResources::toolbox_learningResource_delete()"); selenium.waitForPageToLoad("30000"); selenium.click("ui=groups::content_deleteYes()");
     * selenium.waitForPageToLoad("30000"); final String bodyText = selenium.getBodyText(); if (bodyText.contains("cannot be deleted")) {
     * System.out.println("DELETION FAILED FOR SOME REASON... GIVING UP"); break; } else if (bodyText.contains("Entry deleted.")){
     * System.out.println("YESSS, WE DELETED A RESOURCE!!! "); cnt++; continue; } else {
     * System.out.println("COULDNT FIGURE OUT WHETHER DELETION WAS SUCCESSFUL OR NOT, GIVING UP..."); System.out.println(bodyText); break; } } selenium.close();
     * selenium.stop(); System.out.println("====================================="); System.out.println("DELETE ALL COURSES STARTING WITH: " + str);
     * System.out.println(" (deleted " + cnt + " courses, all="+all+")"); System.out.println("                                 DONE");
     * System.out.println("====================================="); }
     */
