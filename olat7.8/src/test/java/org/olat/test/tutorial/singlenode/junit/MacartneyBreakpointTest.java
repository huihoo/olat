...
public class MacartneyBreakpointTest extends BaseSeleneseTestCase {
    ...
    public void testMacartneyBreakpoint() throws Exception {
        ...
        for (int second = 0;; second++) {
            ...
            try {
                if ("OLAT - Online Learning And Training".equals(selenium1.getTitle()))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
        ...
        for (int second = 0;; second++) {
            ...
            try {
                if ("OLAT - Online Learning And Training".equals(selenium2.getTitle()))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
        ...
        CodepointRef beforeSyncCp = codepointClient_
                .getCodepoint("org.olat.commons.coordinate.cluster.ClusterSyncer.doInSync-before-sync.org.olat.portal.macartney.MacartneyPortletRunController.<init>");
        beforeSyncCp.setHitCount(0);
        beforeSyncCp.enableBreakpoint();

        CodepointRef inSyncCp = codepointClient_
                .getCodepoint("org.olat.commons.coordinate.cluster.ClusterSyncer.doInSync-in-sync.org.olat.portal.macartney.MacartneyPortletRunController.<init>");
