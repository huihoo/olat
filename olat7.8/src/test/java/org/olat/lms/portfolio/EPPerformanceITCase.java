...
/**
 * Description:<br>
 * do queries to check for performance bottlenecks: - Create a lot of artefacts, do load and searches - create a lot of huge maps, publish them for all users, do searches
 * <P>
 * Initial Date: 26.01.2011 <br>
 * 
 * @author Roman Haag, roman.haag@frentix.com, http://www.frentix.com
 */
public class EPPerformanceITCase extends OlatTestCase {
    ...
    private void internalTestManyArtefactCreation(final int artefactAmount) {
        long start = System.currentTimeMillis();
        final Runtime r = Runtime.getRuntime();
        for (int j = 0; j < artefactAmount; j++) {
            final AbstractArtefact artefact = createAndFillArtefact(j);
            // tag the artefacts
            if (j % 2 == 0) {
            ...
            if (j % 100 == 0) {
                log.info("created another 100 artefacts! -> " + j);
        ...
        log.info("created " + artefactAmount + " artefacts in: " + (now - start) + " ms.");
        start = System.currentTimeMillis();
        List<AbstractArtefact> artList = epFrontendManager.getArtefactPoolForUser(ident1);
        now = System.currentTimeMillis();
        log.info("querying all of them took: " + (now - start) + " ms.");
        ...
        now = System.currentTimeMillis();
        log.info("filter artefacts by one tag took: " + (now - start) + " ms.");
        ...
        artList = epFrontendManager.filterArtefactsByFilterSettings(filterSettings, ident1, new Roles(false, false, false, false, false, false, false));
        now = System.currentTimeMillis();
        log.info("filter artefacts by tagList1 took: " + (now - start) + " ms.");
        assertEquals(artList.size(), artefactAmount / 2);
     ...
     * @return
     */
    private AbstractArtefact createAndFillArtefact(final int j) {
        final AbstractArtefact artefact = epFrontendManager.createAndPersistArtefact(ident1, ForumArtefact.FORUM_ARTEFACT_TYPE);
    ...
    private void deleteMaps() {
        final List<PortfolioStructure> publicMaps = epFrontendManager.getStructureElementsFromOthers(ident2, null, ElementType.STRUCTURED_MAP, ElementType.DEFAULT_MAP);
        int i = 1;
        for (final PortfolioStructure portfolioStructure : publicMaps) {
            i++;
    ...
    private void internalTestCreateManyMaps(final int mapAmount) {
        long start = System.currentTimeMillis();
        // prepare some artefacts to link to maps later
        final ArrayList<AbstractArtefact> artefacts = new ArrayList<AbstractArtefact>(10);
        for (int i = 1; i < 11; i++) {
            artefacts.add(createAndFillArtefact(i));
        }

        for (int k = 1; k < mapAmount; k++) {
            final PortfolioStructureMap map = epFrontendManager.createAndPersistPortfolioDefaultMap(ident1, "a test map number " + k, LOREM_STRING_512);
            // attach sites and structures to it
            final ArrayList<PortfolioStructure> structs = new ArrayList<PortfolioStructure>();
            final PortfolioStructure page1 = epFrontendManager.createAndPersistPortfolioPage(map, "test page1 for map " + k, LOREM_STRING_512);
            structs.add(page1);
            final PortfolioStructure struct11 = epFrontendManager.createAndPersistPortfolioStructureElement(page1, "struct1 in page1 for map" + k, LOREM_STRING_512);
            structs.add(struct11);
            final PortfolioStructure struct12 = epFrontendManager.createAndPersistPortfolioStructureElement(page1, "struct2 in page1 for map" + k, LOREM_STRING_512);
            structs.add(struct12);
            final PortfolioStructure page2 = epFrontendManager.createAndPersistPortfolioPage(map, "test page2 for map " + k, LOREM_STRING_512);
            structs.add(page2);
            final PortfolioStructure struct21 = epFrontendManager.createAndPersistPortfolioStructureElement(page2, "struct1 in page2 for map" + k, LOREM_STRING_512);
            structs.add(struct21);
            final PortfolioStructure struct22 = epFrontendManager.createAndPersistPortfolioStructureElement(page2, "struct2 in page2 for map" + k, LOREM_STRING_512);
            structs.add(struct22);

            // attach different artefacts to several places in map
            int l = 1;
            for (final Iterator<PortfolioStructure> iterator = structs.iterator(); iterator.hasNext();) {
                final PortfolioStructure portfolioStructure = iterator.next();
        ...
        long now = System.currentTimeMillis();
        log.info("created " + mapAmount + " maps, attached artefacts and shared maps to public in: " + (now - start) + " ms.");

        // load all maps
        start = System.currentTimeMillis();
        final List<PortfolioStructure> publicMaps = epFrontendManager.getStructureElementsFromOthers(ident2, null, ElementType.STRUCTURED_MAP, ElementType.DEFAULT_MAP);
        now = System.currentTimeMillis();
        log.info("got all public maps in: " + (now - start) + " ms.");
        ...
        int j = 0;
        final Runtime r = Runtime.getRuntime();
        for (final PortfolioStructure map : publicMaps) {
            j++;
     ...
     * @param countChildQ
     */
    private void showStatsForStep(final int step, final long start, final long sharedQ, final long countArtefactQ, final long countChildQ, final Runtime r) {
        long now;
        now = System.currentTimeMillis();
        log.info("---------------------------------------------");
        log.info("show actual query stats for step with " + step + " maps processed.");
        log.info(" free memory: " + r.freeMemory());
        log.info(" simulated queries to show all public maps, took: " + (now - start) + " ms so far.");
        log.info("  Q: share state average:			" + (sharedQ / step) + "		total: " + sharedQ);
        log.info("  Q: artefact count average:		" + (countArtefactQ / step) + "		total: " + countArtefactQ);
        log.info("  Q: child count average: 		" + (countChildQ / step) + "		total: " + countChildQ);
    }

    private String getLoremStringFactor512(final int factor) {
