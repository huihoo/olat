 ...
 * @author guido
 */
public class WikiUnitITCase extends OlatTestCase {
    ...
    public void testWikiStuff() {
        final WikiManager wikiMgr = WikiManager.getInstance();
        final OLATResourceable ores = wikiMgr.createWiki();
        ...
        final WikiPage pageTest = wiki.getPage(WikiManager.generatePageId("test-ä"));
        ...
        page1.setContent(WIKI_CONTENT + "\nThis is a new line");
        wikiMgr.saveWikiPage(ores, page1, true, wiki);
        final List diffs = wiki.getDiff(page1, page1.getVersion() - 1, page1.getVersion());
        final ChangeInfo change = (ChangeInfo) diffs.get(0);
