...
public class LinksPortletRunController extends BasicController {
	...
	protected LinksPortletRunController(final UserRequest ureq, final WindowControl wControl) {
		super(ureq, wControl);
		...
		final boolean isGuest = !ureq.getUserSession().isAuthenticated();
		String inst = new String();
		if (!isGuest) {
			inst = ureq.getIdentity().getUser().getProperty(UserConstants.INSTITUTIONALNAME, getLocale());
		}

		final StringBuffer sb = new StringBuffer();

		// Inhalt verarbeiten
		final Map<String, PortletInstitution> content = LinksPortlet.getContent();
		...
		if (sb.length() > 0) {
			final String portletContent = "<ul>" + sb.toString() + "</ul>";
	 ...
	 */
	private void appendContentFor(final Map<String, PortletInstitution> content, final String inst, final String lang, final StringBuffer sb) {
		for (final PortletLink link : content.get(inst).getLinks()) {
	 ...
	 */
	private void appendContent(final PortletLink link, final StringBuffer sb) {
	 ...
	 * @return
	 */
	private String buildContentLine(final String title, final String URL, final String descr, final String target, final String lang) {
		final StringBuffer sb = new StringBuffer();
		sb.append("<a href=\"");
		sb.append(URL);
		sb.append("\" title=\"");
	...
	/**
	 * @see org.olat.presentation.framework.layout.DefaultController#event(org.olat.gui.UserRequest, org.olat.gui.components.Component, org.olat.presentation.framework.layout.Event)
	 */
	@Override
	...
	/**
	 * @see org.olat.presentation.framework.layout.DefaultController#doDispose(boolean)
	 */
	@Override
