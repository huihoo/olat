...
public class LLEditForm extends FormBasicController {
	...
	private final List<TextElement> lTargetInputList;
	private final List<MultipleSelectionElement> lHtmlTargetInputList;
	private final List<TextElement> lDescriptionInputList;
	private final List<TextElement> lCommentInputList;
	private final List<FormLink> lDelButtonList;
	private final List<LLModel> linkList;
	private final List<FormLink> lAddButtonList;
	private FormLayoutContainer titleContainer;
	private long counter = 0;

	public LLEditForm(final UserRequest ureq, final WindowControl wControl, final ModuleConfiguration moduleConfig) {
		super(ureq, wControl);
	...
	protected void formOK(final UserRequest ureq) {
		...
		for (int i = 0; i < lTargetInputList.size(); i++) {
			final LLModel link = (LLModel) lTargetInputList.get(i).getUserObject();
	...
	protected void formInnerEvent(final UserRequest ureq, final FormItem source, final FormEvent event) {
		if (source instanceof TextElement) {
			final int i = lTargetInputList.indexOf(source);
			if (i >= 0) {
				String linkValue = ((TextElement) source).getValue();
				if (!linkValue.contains("://")) {
					linkValue = "http://".concat(linkValue.trim());
				}
				final boolean selected = lHtmlTargetInputList.get(i).isSelected(0);
	...
	protected void initForm(final FormItemContainer formLayout, final Controller listener, final UserRequest ureq) {
		...
		for (int i = 0; i < linkList.size(); i++) {
			final LLModel link = linkList.get(i);
	 ...
	 */
	private void addNewFormLink(final int index, final LLModel link) {
		// add link target
		final TextElement lTarget = uifactory.addTextElement("target" + counter, null, -1, link.getTarget(), titleContainer);
		lTarget.clearError();
		...
		// add html target
		final MultipleSelectionElement htmlTargetSelection = uifactory.addCheckboxesHorizontal("html_target" + counter, titleContainer, new String[] { BLANK_KEY },
				new String[] { "" }, null);
		...
		final TextElement lDescription = uifactory.addTextElement("description" + counter, null, -1, link.getDescription(), titleContainer);
		...
		// add link comment
		final TextElement lComment = uifactory.addTextElement("comment" + counter, null, -1, link.getComment(), titleContainer);
		lComment.setDisplaySize(20);
		lComment.setUserObject(link);
		lCommentInputList.add(index, lComment);

		// add link add action button
		final FormLink addButton = new FormLinkImpl("add" + counter, "add" + counter, "ll.table.add", Link.BUTTON_SMALL);
		...
		final FormLink delButton = new FormLinkImpl("delete" + counter, "delete" + counter, "ll.table.delete", Link.BUTTON_SMALL);
