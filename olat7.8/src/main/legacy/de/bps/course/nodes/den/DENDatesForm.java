...
public class DENDatesForm extends FormBasicController {
	 ...
	 */
	public DENDatesForm(final UserRequest ureq, final WindowControl wControl, final Translator translator, final int layout) {
		super(ureq, wControl, "dateform");
		this.setTranslator(translator);
		this.layout = layout;

		/*
		 * REVIEW:pb:2009-11-23:Hi Stefan, soweit ich gesehen habe, wird der int Layout nur verwendet um die context help im velocity zu switchen mein Vorschlag wäre,
		 * hier zur Konstruktorenzeit anstelle des int's die relevanten HELP Strings ins velocity zu schreiben und im velocity nur noch eine zeile mit
		 * contextHelp(${XYZ},${UVW}) haben.
		 */
	...
	protected void initForm(final FormItemContainer formLayout, final Controller listener, final UserRequest ureq) {
		...
		final boolean createsDateLayout = layout == CREATE_DATES_LAYOUT;
		...
		final String durationTELabel = layout == EDIT_SINGLE_DATE_LAYOUT ? "config.dates.duration.single" : "config.dates.duration";
	...
	/*
	 * helper method set mandatory, validity checks depending on creates date layout.
	 */
	private void setupTextElementDependingOn(final boolean createDatesLayout, final TextElement textElement) {
	...
	protected boolean validateFormLogic(final UserRequest ureq) {
		...
		if (layout == CREATE_DATES_LAYOUT) {
			if (retakeTE.isEmpty() || subjectTE.isEmpty() || locationTE.isEmpty() || participantsTE.isEmpty()) {
				...
				try {
					final int numRetakes = Integer.parseInt(retakeTE.getValue());
	...
	public int getNumParts() {
		if (!participantsTE.isEmpty()) { return Integer.parseInt(participantsTE.getValue()); }
		return 0;
	}
	 ...
	 * @param value
	 */
	public void setNumParts(final int value) {
	 ...
	 * @param date
	 */
	public void setFormDate(final Date date) {
