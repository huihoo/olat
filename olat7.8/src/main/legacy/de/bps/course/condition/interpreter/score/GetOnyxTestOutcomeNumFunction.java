...
public class GetOnyxTestOutcomeNumFunction extends AbstractFunction {
	 ...
	 * @param userCourseEnv
	 */
	public GetOnyxTestOutcomeNumFunction(final UserCourseEnvironment userCourseEnv) {
		super(userCourseEnv);
	}
	...
	public Object call(final Object[] inStack) {
		...
		try {
			...
			try {
				...
				log.warn("Unable to get results from OnyxReporter for user " + identity.getName());
				// e.printStackTrace();
			} catch (final Exception e) {
				e.printStackTrace();
			}
