...
public class IQRunController extends BasicController implements GenericEventListener {
    ...
    IQRunController(final UserCourseEnvironment userCourseEnv, final ModuleConfiguration moduleConfiguration, final IQSecurityCallback secCallback,
            final UserRequest ureq, final WindowControl wControl, final IQTESTCourseNode testCourseNode) {
        super(ureq, wControl);
        ...
        if (qtiChangeLog.length > 0) {
            ...
            for (int i = qtiChangeLog.length - 1; i >= 0; i--) {
                ...
                if (!showAll && qtiChangeLog[i].isPublic()) {
                    ...
                    qtiChangelog.append(StringHelper.escapeHtml(qtiChangeLog[i].getLogMessage()));
                    qtiChangelog.append("\n********************************\n");
                } else if (showAll) {
        ...
        if (source == startButton && startButton.isEnabled()) {
            ...
            if (returnController instanceof IQDisplayController) {
                ...
                if (displayController.isReady()) {
                    ...
                    final Controller disposedRestartController = new DisposedRepoEntryRestartController(ureq, getWindowControl(), courseRepositoryEntry,
                    ...
                    final Controller layoutController = new LayoutMain3ColsController(ureq, getWindowControl(), empty, empty,
                            disposedRestartController.getInitialComponent(), "disposed course" + courseRepositoryEntry.getResourceableId());
                    layoutCtr.setDisposedMessageController(layoutController);
        ...
        if (source == displayController) {
            if (event instanceof IQSubmittedEvent) {
                ...
                if (type.equals(AssessmentInstance.QMD_ENTRY_TYPE_ASSESS)) {
                    ...
                    sendConfirmations(urequest.getIdentity(), userCourseEnv.getCourseEnvironment().getCourseTitle(), userCourseEnv.getCourseEnvironment()
                            .getRepositoryEntryId(), new Long(courseNode.getIdent()), courseNode.getShortTitle());
    ...
    private void sendConfirmations(Identity student, String courseTitle, Long courseRepositoryEntryId, Long courseNodeId, String testTitle) {
        boolean isMailSucessfullySent = getConfirmationLearnService().sendTestSubmitStudentConfirmation(student, courseTitle, courseRepositoryEntryId, courseNodeId,
                testTitle);
        if (isMailSucessfullySent) {
            log.info("Sent 'confirmation at submit test' email to " + student + "with email=" + student.getAttributes().getEmail());
        } else {
            log.warn("Could not send 'confirmation at submit test' email to " + student + "with email=" + student.getAttributes().getEmail());
        }
        if (isConfirmationEnabled()) {
            List<Identity> tutors = userCourseEnv.getCourseEnvironment().getCourseGroupManager()
                    .getCourseOwnersAndTutors(userCourseEnv.getCourseEnvironment().getCourseOLATResourceable());
            getConfirmationLearnService().sendTestSubmitTutorConfirmation(tutors, student, courseTitle, courseRepositoryEntryId, courseNodeId, testTitle);
        }
    }

    private boolean isConfirmationEnabled() {
        Object isConfirmationEnabled = modConfig.get(IQTESTCourseNode.CONFIRMATION_REQUESTED);
