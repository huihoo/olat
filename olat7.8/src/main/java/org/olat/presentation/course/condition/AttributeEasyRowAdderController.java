...
import org.olat.lms.course.condition.operators.OperatorManager;
import org.olat.lms.security.authentication.shibboleth.AttributeTranslator;
...
import org.olat.presentation.security.authentication.shibboleth.ShibbolethAuthenticationController;
...
public class AttributeEasyRowAdderController extends FormBasicController {
    ...
    public AttributeEasyRowAdderController(final UserRequest ureq, final WindowControl wControl, final Form parentForm) {
        ...
        setTranslator(PackageUtil.createPackageTranslator(ShibbolethAuthenticationController.class, ureq.getLocale(), getTranslator()));
        ...
        preselectedAttributeValue = getUserService().getUserProperty(ureq.getIdentity().getUser(), UserConstants.INSTITUTIONALNAME, getLocale());
    ...
    /**
     * org.olat.presentation.framework.components.form.flexible.FormEvent)
     */
    @Override
    ...
    private void addRowAt(final int rowPos) {
        ...
        for (int j = 0; j < attrKeys.length; j++) {
            ...
            if (translated.indexOf(Translator.NO_TRANSLATION_ERROR_PREFIX) == 0) {
                final Translator translator = getUserService().getUserPropertiesConfig().getTranslator(getTranslator());
    ...
    /**
     * Internal method to update a row's value element. This can be a text input box or a selection drop down depending on the shibboleth module configuration and the
     * selected attribute. The method will set the given value as the users selected / inputed value
     * 
     * @param attribute
     ...
     *            the row ID
     * @param value
     *            The value that should be selected / used in the text input field. Can be NULL.
     */
    private void updateValueElementForAttribute(final String attribute, final int row, final String value) {
        final String[] selectableKeys = getSelectableKeys(attribute);
    ...
    private void attributeAsSelectBox(final String attribute, final String value, final String[] selectableKeys, final SingleSelection iselect, final TextElement tei) {
        ...
        if (value != null) {
            ...
            if (Arrays.asList(selectableKeys).contains(value)) {
                ...
                log.warn("could not select value::" + value + " for shibboleth attribute::" + attribute + " in course easy mode, not found in selectable list", null);
            }
        }
