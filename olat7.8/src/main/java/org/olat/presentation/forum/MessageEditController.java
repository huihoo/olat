...
public class MessageEditController extends FormBasicController {
    ...
    protected static final Integer MAX_BODY_LENGTH = 10000;
     ...
     * @param forumCallback
     * @param message
     *            may be a new message created by ForumService.createMessage() which is not yet saved in db
     * @param quoteMessage
     *            may be null if Editor isn't used to reply to a message
     */
    public MessageEditController(final UserRequest ureq, final WindowControl control, final ForumCallback forumCallback, final Message message, final Message quoteMessage) {
        ...
        this.fm = getForumService();

        tempUploadFolder = createOlatRootFolder();
     ...
     * @return
     */
    private OlatRootFolderImpl createOlatRootFolder() {
        return (OlatRootFolderImpl) getForumService().getTempUploadFolder();
    }
    ...
    protected void initForm(final FormItemContainer formLayout, final Controller listener, final UserRequest ureq) {
        ...
        if (editMode.equals(EDITMODE_REPLYMSG)) {
            final FormLayoutContainer replyMsgLayout = FormLayoutContainer.createCustomFormLayout("replyMsg", getTranslator(),
                    PackageUtil.getPackageVelocityRoot(this.getClass()) + "/msg-preview.html");
            ...
            replyMsgLayout.contextPut("userFirstAndLastName", getUserService().getFirstAndLastname(identity.getUser()));
    ...
    private void createOrUpdateAttachmentListLayout(final FormItemContainer formLayout) {
        ...
        if (message.getKey() != null) {
            final VFSContainer msgContainer = fm.getMessageContainer(message.getForum().getKey(), message.getKey());
        ...
        if (attachLayout == null) {
            tmpLayout = FormLayoutContainer.createCustomFormLayout("attachLayout", getTranslator(), PackageUtil.getPackageVelocityRoot(this.getClass())
    ...
    protected void formInnerEvent(final UserRequest ureq, final FormItem source, @SuppressWarnings("unused") final FormEvent event) {
        if (source == fileUpload) {
            if (fileUpload.isUploadSuccess()) {
                ...
                if (isFileSizeOK()) {
                    boolean fileExists = isFilenameAlreadyInUse(fileName);
    ...
    private boolean isFileSizeOK() {
        return fileUpload.getUploadSize() / 1024 < fileUpload.getMaxUploadSizeKB();
    }

    /**
     * checking tmp-folder and msg-container for filename
     */
    private boolean isFilenameAlreadyInUse(final String fileName) {
        ...
        if (msgContainer != null) {
            ...
            for (final VFSItem file : tmpFList) {
                ...
                try {
                    ...
                    removeTempUploadedFiles();
                    throw new RuntimeException("I/O error saving uploaded file:" + msgContainer.getPath());
                }
            }
        }
        removeTempUploadedFiles();
