...
/**
 * <h3>Description:</h3> This controller can be used to edit one or more I18nItems <h3>Events thrown by this controller:</h3>
 * <ul>
...
class TranslationToolI18nItemEditCrumbController extends CrumbFormBasicController {
    ...
    /**
     * Constructor for the item edit controller. Use the initialzeI18nitemAsCurrentItem() to set the current item from the given i18nItems list to a specific value after
     ...
     * @param customizingMode
     *            true: edit overlay customization files and not language files; false: edit language files
     */
    ...
    /**
     * Initializes the view of the controller for the given i18n item. The item must exist in the i18nItems list that has been used when constructing this object
     * 
     * @param ureq
    ...
    public void initialzeI18nitemAsCurrentItem(UserRequest ureq, I18nItem toBeActivatedItem) {
        ...
        log.error("Could not init view for item with key::" + toBeActivatedItem.getKey() + " in bundle::" + toBeActivatedItem.getBundleName() + " in language::"
                + toBeActivatedItem.getLocale().toString(), null);
    }
    ...
    protected void initForm(FormItemContainer formLayout, Controller listener, UserRequest ureq) {
        ...
        compareSwitch = formFactory.addCheckboxesHorizontal("compareSwitch", null, this.flc, new String[] { KEYS_ENABLED }, new String[] { KEYS_EMPTY }, null);// i18nLabel
                                                                                                                                                               // set to
                                                                                                                                                               // null ->
                                                                                                                                                               // disabled
                                                                                                                                                               // label
        ...
        compareLangSelection = formFactory.addDropdownSingleselect("compareLangSelection", this.flc, comparelangKeys, compareLangValues, compareLangCssClasses);
    ...
    private void updateCompareArea(UserRequest ureq) {
        ...
        if (compareSwitch.isSelected(0)) {
            I18nManager i18nMgr = I18nManager.getInstance();
            // Add target value
            String compareValue = i18nMgr.getLocalizedString(currentItem.getBundleName(), currentItem.getKey(), null, compareLocale, customizingMode, false, false,
                    false, 0);
    ...
    private void doSaveCurrentItem(UserRequest ureq) {
        ...
        if (!StringHelper.containsNonWhitespace(newAnnotation))
            newAnnotation = null;
        // check if the same
