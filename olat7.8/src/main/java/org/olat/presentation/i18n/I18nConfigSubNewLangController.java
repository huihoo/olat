...
import org.olat.presentation.framework.core.components.form.flexible.impl.rules.FormItemDependencyRuleImpl;
...
/**
 * Description:<br>
 * This form allows the user to enter a new language key and create the language immediately. The form implements the user interface and calls the necessary manager code
 * to create the language. <h3>Events fired by this controller</h3>
 * <ul>
 * <li>Event.CANCELLED_EVENT</li>
 * </ul>
 * When the user presses the save button, the system language preference of this user will be set to the new user and he will be logged out immediately.
 * <P>
...
class I18nConfigSubNewLangController extends FormBasicController {
    ...
    protected void initForm(FormItemContainer formLayout, @SuppressWarnings("unused") Controller listener, @SuppressWarnings("unused") UserRequest ureq) {
        ...
        // Add warn message
        String warnPage = PackageUtil.getPackageVelocityRoot(this.getClass()) + "/i18nConfigurationNewWarnMessage.html";
    ...
    protected void formOK(UserRequest ureq) {
        ...
        if (success) {
            ...
            log.info("Audit:Created new language::" + localeKey, null);
    ...
    /**
     * Helper method to update the language in the user profile of the current user to the given language and send the user to the page.
     * 
     * @param ureq
     * @param localeKey
     */
    /* TODO: ORID-1007 this has not been yet tested because not enabled */
    private void updateUserLocaleAndLogout(UserRequest ureq, String localeKey) {
        User currUser = ureq.getIdentity().getUser();
        currUser = getUserService().loadUserByKey(currUser.getKey());
        currUser.getPreferences().setLanguage(I18nManager.getInstance().getLocaleOrDefault(localeKey));
        getUserService().saveUser(currUser);
