 ...
 * <p>
 */
package org.olat.presentation.admin.sysinfo.logging;
...
/**
 * Description:<br>
 * The log real time viewer controller offers the possibility to live-view the log file. This can be handy for debugging or to monitor certain actions like a user import.
 * <P>
...
public class LogRealTimeViewerController extends BasicController implements JobListener {
     ...
     *            The logging package or a specific class. All messages from the given package will be displayed
     * @param level
     *            The log level to include in the list. Note that this will not change the log level on the packages itself, use the admin console to do this. This
     *            filters only the messages that are below this level
     * @param removeLogNoise
     *            true: remove a lot of brasato specific stuff that is normally not interesting; false: show log entries as they are logged. Example if set to true: <br>
     *            <code>2008-08-22 11:20:40 [QuartzScheduler_Worker-1] INFO  LDAPUserSynchronizerJob  - OLAT::INFO ^%^ I441 ^%^ org.olat.connectors.ldap ^%^ n/a ^%^ n/a ^%^ n/a ^%^ n/a ^%^ n/a ^%^ Starting LDAP user synchronize job</code>
     * <br>
     *            will become<br>
    ...
    public LogRealTimeViewerController(UserRequest ureq, WindowControl control, String loggingPackage, Level level, boolean removeLogNoise) {
        ...
        try {
            ...
            log.error("Can not parse log viewer cron expression", e);
        } catch (SchedulerException e) {
            log.error("Problem when creating log viewer scheduler", e);
        }
    ...
    protected void doDispose() {
        if (logViewerVC != null) { // don't clean up twice
            ...
            try {
                ...
                log.error("Can not delete log viewer job", e);
            }
            ...
            try {
                ...
                log.error("Error while closing log viewer string writer", e);
            }
            writer = null;
            updateLink = null;
            logViewerVC = null;
    ...
    protected void event(UserRequest ureq, Component source, Event event) {
        ...
        if (source == stopLink) {
            ...
            try {
                ...
                log.error("Can not pause log viewer job", e);
            }
        }
        if (source == startLink) {
            // update viewable links
            ...
            try {
                ...
                log.error("Can not resume log viewer job", e);
            }
        }
    }
	 ...
	 */
    @Override
    public void jobExecutionVetoed(@SuppressWarnings("unused") JobExecutionContext arg0) {
	 ...
	 */
    @Override
    public void jobToBeExecuted(@SuppressWarnings("unused") JobExecutionContext arg0) {
        // nothing to do, see jobWasExecuted()
    }
	 ...
	 */
    @Override
    public void jobWasExecuted(@SuppressWarnings("unused") JobExecutionContext arg0, JobExecutionException arg1) {
        updateLogViewFromWriter();
    }
}
