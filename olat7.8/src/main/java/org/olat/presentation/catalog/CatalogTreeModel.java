...
/**
 * Description:<br>
 * The catalog tree model implements an old-school selection tree model to move or insert a catalog entry into a catalog. Dependin on your catalog size and your browser
 * it might be slow. In the ajax mode the new ajax version shold be used.
 * <p>
...
public class CatalogTreeModel extends GenericTreeModel {
     ...
     * 
     * @param catEntryList
     *            The list of all entries, preloaded with one fetch
     * @param entryToMove
     *            The entry to be moved or NULL to just select a catalog category
     * @param ownedEntries
     *            A list of catalog categories that are manageable by the current user or NULL if no such check should be performed
     */
    public CatalogTreeModel(final List<CatalogEntry> catEntryList, final CatalogEntry entryToMove, final List<CatalogEntry> ownedEntries) {
     ...
     * @return
     */
    private GenericTreeNode buildNode(final CatalogEntry catEntry) {
        final GenericTreeNode node = new GenericTreeNode();
     ...
     */
    private GenericTreeNode buildTree(final List<CatalogEntry> entryList) {
        ...
        for (int cnt = 0; cnt < entryList.size(); cnt++) {
            final CatalogEntry elem = entryList.get(cnt);
            // create a tree node and add it to the entry map
            final GenericTreeNode n = addNode(elem);
            if (n != null) {
     ...
     * @return
     */
    private GenericTreeNode addNode(final CatalogEntry entry) {
        ...
        if (node == null && entry.getType() == CatalogEntry.TYPE_NODE) {
            if (entry.getParent() != null) {
                ...
                if (node != null) {
                    final GenericTreeNode build = buildNode(entry);
                    node.addChild(build);
    ...
    private void addMissingNodes(CatalogEntry fault) {
        ...
        while (fault != null) {
            if (entryMap.get(fault.getKey()) == null) {
                final GenericTreeNode build = buildNode(fault);
                if (fault.getParent() != null) {
     ...
     */
    private void calculateAccessibility(final GenericTreeNode rootNode) {
        GenericTreeNode node = null;
        // first : allow access to all children of the owned entries
        if (ownedEntries != null) {
            for (final CatalogEntry entry : ownedEntries) {
     ...
     * @param accessible
     */
    private void changeAccessibility(final GenericTreeNode node, final boolean accessible) {

        if (accessible && !node.getTitle().equals(chooseableTitle)) {
            final GenericTreeNode chooseable = new GenericTreeNode();
