...
/**
 * Description:<br>
 * This class represents the catalog data model that can be used with the ajax tree controller. It supports dynamic loading of catalog elements and can be configured to
 * display only the categories or the categories and the children.
 * <p>
...
public class CatalogAjaxTreeModel extends AjaxTreeModel {
    ...
    private final boolean allowDragNdrop;
    private final CatalogEntry toBeMovedEntry;
     ...
     * 
     * @param rootEntry
     *            The catalog root entry
     * @param toBeMovedEntry
     *            The entry what should be moved or NULL if no such entry. If not null this entry will not be displayed in the tree
     * @param ownedEntries
     *            List of entries (categories) that are owned by the current user. In category move operations the target category will be checked against this list. Can
     *            be NULL
     * @param showLeafs
     *            true: show also leafs in the tree; false: show only categories
     * @param allowDragNdrop
     *            true: enable drag&dop of categories and leafs; false: don't use drag&drop
     */
    public CatalogAjaxTreeModel(final CatalogEntry rootEntry, final CatalogEntry toBeMovedEntry, final List<CatalogEntry> ownedEntries, final boolean showLeafs,
            final boolean allowDragNdrop) {
    ...
    public List<AjaxTreeNode> getChildrenFor(final String nodeId) {
        final List<AjaxTreeNode> childNodes = new ArrayList<AjaxTreeNode>();
        // load node with given id
        final Long entryKey = Long.parseLong(nodeId);
        final CatalogEntry entry = catalogService.loadCatalogEntry(entryKey);
        // load children of node and add them to the list
        final List<CatalogEntry> childEntries = catalogService.getChildrenOf(entry);
        for (final CatalogEntry childEntry : childEntries) {
            ...
            if (childEntry.getType() == CatalogEntry.TYPE_NODE || showLeafs) {
                final AjaxTreeNode childNode = buildNode(childEntry);
     ...
     * @return
     */
    private AjaxTreeNode buildNode(final CatalogEntry catalogEntry) {
        AjaxTreeNode node;
        try {
            ...
            if (catalogEntry.getType() == CatalogEntry.TYPE_NODE) {
                ...
                if (ownedEntries != null && !catalogService.isEntryWithinCategory(catalogEntry, ownedEntries)) {
                ...
                if (!allowDragNdrop && !catalogService.hasChildEntries(catalogEntry, CatalogEntry.TYPE_NODE)) {
