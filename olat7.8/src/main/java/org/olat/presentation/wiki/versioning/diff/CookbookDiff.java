/*
 * This file is part of "SnipSnap Wiki/Weblog". Adapted from <a href="http://javacook.darwinsys.com/">Ian Darwin's Java Cookbook</a> See Diff.java for more information
 * Result was printed, now the result is a list of ChangeInfo objects. Input was from two files, now diff takes two Strings. Changed to Java coding style. diff Text file
 * difference utility. ---- Copyright 1987, 1989 by Donald C. Lindsay, School of Computer Science, Carnegie Mellon University. Copyright 1982 by Symbionics. Use without
 * fee is permitted when not for direct commercial advantage, and when credit to the source is given. Other uses require specific permission. Adaption Copyright (c) 2002
 * Stephan J. Schmidt, Matthias L. Jugel All Rights Reserved. Please visit http://snipsnap.org/ for updates and contact. --LICENSE NOTICE-- This program is free software;
 * you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the
 * License, or (at your option) any later version. This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied
 ...
 * General Public License along with this program; if not, write to the Free Software Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA. --LICENSE NOTICE--
 */
...
public class CookbookDiff {
    ...
    /**
     * blocklen is the info about found blocks. It will be set to 0, except at the line#s where blocks start in the old file. At these places it will be set to the # of
     * lines in the block. During printout , this # will be reset to -1 if the block is printed as a MOVE block (because the printout phase will encounter the block
     * twice, but must only print it once.) The array declarations are to MAXLINECOUNT+2 so that we can have two extra lines (pseudolines) at line# 0 and line#
     * MAXLINECOUNT+1 (or less).
     */
    private int blocklen[];

    /** Do one string comparison. Called with both strings. */
    public List diff(final String oldText, final String newText) {
        Node.panchor = null;
     ...
     */
    void inputScan(final String input, final SourceInfo pinfo) {
        pinfo.maxLine = 0;
    ...
    /**
     * storeline Places line into symbol table. --------- Expects pinfo.maxLine initted: increments. Places symbol table handle in pinfo.ymbol. Expects pinfo is either
     * oldinfo or newinfo.
     */
    void storeLine(final String linebuffer, final SourceInfo pinfo) {
        final int linenum = ++pinfo.maxLine; /* note, no line zero */
    ...
    /*
     * transform Analyzes the file differences and leaves its findings in the global arrays oldinfo.other, newinfo.other, and blocklen. Expects both files in symtab.
     * Expects valid "maxLine" and "symbol" in oldinfo and newinfo.
     */
    void transform() {

        int oldline, newline;
        final int oldmax = oldInfo.maxLine + 2; /* Count pseudolines at */
        final int newmax = newInfo.maxLine + 2; /* ..front and rear of file */
    ...
    /*
     * scanunique Scans for lines which are used exactly once in each file. Expects both files in symtab, and oldinfo and newinfo valid. The appropriate "other" array
     * entries are set to the line# in the other file. Claims pseudo-lines at 0 and XXXinfo.maxLine+1 are unique.
     */
    void scanUnique() {
        int oldline, newline;
        Node psymbol;
    ...
    /*
     * scanafter Expects both files in symtab, and oldinfo and newinfo valid. Expects the "other" arrays contain positive #s to indicate lines that are unique in both
     * files. For each such pair of places, scans past in each file. Contiguous groups of lines that match non-uniquely are taken to be good-enough matches, and so marked
     * in "other". Assumes each other[0] is 0.
     */
    void scanAfter() {
        int oldline, newline;
    ...
    // last one
    private void setLast(final ChangeInfo info, final List result) {
        setLast(result);
        last = info;
    }

    private void setLast(final List result) {
        if (null != last) {
    ...
    /*
     * newconsume Part of printout. Have run out of old file. Print the rest of the new file, as inserts and/or moves.
     */
    private void newConsume(final List result) {
        for (;;) {
            if (printnewline > newInfo.maxLine) {
                break; /* end of file */
            }
     ...
     */
    private void oldConsume(final List result) {
        for (;;) {
            if (printoldline > oldInfo.maxLine) {
                break; /* end of file */
            }
     ...
     */
    private void showDelete(final List result) {
        if (printstatus != delete) {
            final ChangeInfo info = new ChangeInfo(ChangeInfo.DELETE, printoldline, printoldline);
            setLast(info, result);
        }
        printstatus = delete;
     ...
     */
    private void showInsert(final List result) {
        if (printstatus == change) {
            // result.add(">>>>     CHANGED TO");
        } else if (printstatus != insert) {
            final ChangeInfo info = new ChangeInfo(ChangeInfo.INSERT, printoldline, printoldline);
            setLast(info, result);
        }
        printstatus = insert;
     ...
     */
    private void showChange(final List result) {
        if (printstatus != change) {
            final ChangeInfo info = new ChangeInfo(ChangeInfo.CHANGE, printoldline, printoldline);
            setLast(info, result);
        }
        printstatus = change;
        lines.add(oldInfo.symbol[printoldline].getSymbol());
     ...
     */
    private void showSame(final List result) {
        int count;
        printstatus = idle;
     ...
     */
    private void showMove(final List result) {
        final int oldblock = blocklen[printoldline];
        final int newother = newInfo.other[printnewline];
        ...
        if (newblock < 0) {
            ...
            final ChangeInfo info = new ChangeInfo(ChangeInfo.MOVE, newother, printoldline);
            setLast(info, result);
...
/**
 * Class "node". The symbol table routines in this class all understand the symbol table format, which is a binary tree. The methods are: addSymbol, symbolIsUnique,
 * showSymbol.
 */
class Node { /* the tree is made up of these nodes */
    ...
    /**
     * matchsymbol Searches tree for a match to the line. If node's linestate == freshnode, then created the node.
     */
    static Node matchsymbol(final String pline) {
        int comparison;
        Node pnode = panchor;
        if (panchor == null) {
    ...
    /**
     * addSymbol(String pline) - Saves line into the symbol table. Returns a handle to the symtab entry for that unique line. If inoldfile nonzero, then linenum is
     * remembered.
     */
    static Node addSymbol(final String pline, final boolean inoldfile, final int linenum) {
        Node pnode;
    ...
    /**
     * symbolIsUnique Arg is a ptr previously returned by addSymbol. -------------- Returns true if the line was added to the symbol table exactly once with inoldfile
     * true, and exactly once with inoldfile false.
     */
