...
import org.olat.presentation.framework.core.components.form.flexible.impl.elements.richText.plugins.TinyMCECustomPluginFactory;
import org.olat.presentation.framework.core.components.form.flexible.impl.elements.richText.plugins.olatmovieviewer.OlatMovieViewerPlugin;
...
/**
 * Description:<br>
 * This configuration object is used to configure the features of the TinyMCE HTML editor. Use the addXYZConfiguration() methods to add some default settings. You can
 * also manually tweak the configuration, make sure you understand how Tiny works. See http://wiki.moxiecode.com for more information
 * <P>
...
public class RichTextConfiguration implements Disposable {
    ...
    public static final String[] MEDIA_SUFFIXES_VALUES = { "swf", "dcr", "mov", "qt", "mpg", "mp3", "mp4", "mpeg", "avi", "wmv", "wm", "asf", "asx", "wmx", "wvx", "rm",
            "ra", "ram" };
    ...
    /**
     * Constructor, only used by RichText element itself. Use richtTextElement.getEditorConfiguration() to acess this object
     * 
     * @param domID
     *            The ID of the flexi element in the browser DOM
     * @param rootFormDispatchId
     *            The dispatch ID of the root form that deals with the submit button
     */
    ...
    public void setConfigProfileFormEditorMinimalistic(UserSession usess, boolean externalToolbar, Theme guiTheme) {
        ...
        if (enabledCustomPlugins.size() > 0)
            theme_advanced_buttons1.add(SEPARATOR_BUTTON);
     ...
     */
    public void setConfigProfileFormEditor(boolean fullProfile, UserSession usess, boolean externalToolbar, Theme guiTheme, VFSContainer baseContainer,
        ...
        if (fullProfile) {
            ...
            int profile;
            if (baseContainer == null)
                profile = CONFIG_PROFILE_FORM_EDITOR_FULL;
            else
                profile = CONFIG_PROFILE_FORM_EDITOR_FULL_WITH_MEDIABROWSER;
            ...
            int profile;
            if (baseContainer == null)
                profile = CONFIG_PROFILE_FORM_EDITOR_SIMPLE;
            else
                profile = CONFIG_PROFILE_FORM_EDITOR_SIMPLE_WITH_MEDIABROWSER;
            List<TinyMCECustomPlugin> enabledCustomPlugins = customPluginFactory.getCustomPlugionsForProfile(profile);
            for (TinyMCECustomPlugin tinyMCECustomPlugin : enabledCustomPlugins) {
                setCustomPluginEnabled(true, tinyMCECustomPlugin, profile);
            }
     ...
     * @param customLinkTreeModel
     */
    public void setConfigProfileFileEditor(UserSession usess, boolean externalToolbar, Theme guiTheme, VFSContainer baseContainer, String relFilePath,
            CustomLinkTreeModel customLinkTreeModel) {
        setConfigBasics(usess, externalToolbar, guiTheme);
        // Line 1
        ...
        if (usess.getRoles().isAdministrativeUser()) {
            // add html direct editor
        ...
        int profile;
        if (baseContainer == null)
            profile = CONFIG_PROFILE_FILE_EDITOR_FULL;
        else
            profile = CONFIG_PROFILE_FILE_EDITOR_FULL_WITH_MEDIABROWSER;
        List<TinyMCECustomPlugin> enabledCustomPlugins = customPluginFactory.getCustomPlugionsForProfile(profile);
        for (TinyMCECustomPlugin tinyMCECustomPlugin : enabledCustomPlugins) {
            setCustomPluginEnabled(true, tinyMCECustomPlugin, profile);
        }
    ...
    /**
     * Internal helper to generate the common configurations which are used by each profile
     * 
     * @param usess
     * @param externalToolbar
     * @param guiTheme
    ...
    private void setConfigBasics(UserSession usess, boolean externalToolbar, Theme guiTheme) {
        ...
        // Set link targets
        Translator trans = PackageUtil.createPackageTranslator(this.getClass(), loc);
     ...
     * Don't add something like this: function() {alert('loading successfull')}, use the following notation instead: (alert('loading successfull'))
     * 
     * @param functionName
     */
     ...
     * 
     * if enabled its possible to enter/leave the tinyMCE-editor with TAB-key. drawback is, that you cannot enter tabs in the editor itself or navigate over buttons! see
     * http://bugs.olat.org/jira/browse/OLAT-6242
     * 
     ...
     *            true: use inline popups; false: use browser window popup windows
     */
     ...
     * 
     * @param position
     *            The location of the toolbar. Use THEME_ADVANCED_TOOLBAR_LOCATION_* values
     * @param autoHide
     *            true: try to hide toolbar when mouse leafs the editor area; false, never hide toolbar (only applicable when position is external)
     * @param alignment
     *            The alignment of the toolbar, use THEME_ADVANCED_TOOLBAR_ALIGN_VALUE_* values
     */
    ...
    /**
     * Set a specific doctype. Note that this can trigger buggy IE effects. Don't use this unless you really need this! <br>
    ...
    /**
     * Enable or disable areas in the editor content that can't be modified at all. The areas are identified with the nonEditableCSSClass.
     * 
     * @param noneditableContentEnabled
     *            true: use non-editable areas; false: all areas are editable
     * @param nonEditableCSSClass
     *            the class that identifies the non-editable fields or NULL to use the default value 'mceNonEditable'
     */
     ...
     *            true: show past buttons; false: hide past buttons (paste via keyboard shortcuts still works
     * @param cleanupOnPastEnabled
     *            true: cleanup pasted code before inserting into the DOM; false: don't cleanup
    ...
    public void setAdvancedPasteEnabled(boolean pasteEnabled, boolean showPasteButtons, boolean cleanupOnPastEnabled, boolean convertWordLists, int row) {
        ...
        if (pasteEnabled) {
            ...
            if (showPasteButtons)
                buttonRow.add(PASTE_PLUGIN_BUTTONGROUP);
            // enabled by default, disable on request
            if (!cleanupOnPastEnabled)
                setNonQuotedConfigValue(PASTE_AUTO_CLEANUP_ON_PASTE, VALUE_FALSE);
            if (!convertWordLists)
                setNonQuotedConfigValue(PASTE_AUTO_CLEANUP_ON_PASTE, VALUE_FALSE);
        } else {
     ...
     *            true: fullscreen opens in new window; false: fullscreen opens in same window.
     * @param row
     *            The row where to place the plugin buttons
     */
     ...
     *            true: also show the element attributes view; false: don't use the elements attributes view
     * @param row
     *            The row where to place the plugin buttons
     */
    public void setXhtmlXtrasEnabled(boolean xhtmlXtrasEnabled, boolean elementAttributesEnabled, int row) {
        ...
        if (xhtmlXtrasEnabled) {
            ...
            if (elementAttributesEnabled)
                buttonRow.add(XHTMLXTRAS_PLUGIN_BUTTON_ATTRIBS);
        } else {
    ...
    /**
     * Set the target list that can be used by author in the target dropdown of links. E.g. 'displayname=targetname,displayname2=targetname2'. The standard targets like
     * _blank must not be added, use only the application specific targets here.
     * 
     * @param targetList
     *            coma separated list of GUIname=target or NULL to reset the list
     */
    public void setLinkTargets(String targetList) {
        ...
        if (targetList != null) {
            ...
            if (quotedConfigValues.containsKey(THEME_ADVANCED_LINK_TARGETS))
                quotedConfigValues.remove(THEME_ADVANCED_LINK_TARGETS);
        }
    }
     ...
     * 
     * @param cssPath
     *            path to CSS separated by comma or NULL to not use any specific CSS files
     */
    ...
    /**
     * Set the content CSS form the given theme. This will add the content.css from the default themen and override it with the current theme content.css
     * 
     * @param theme
     */
    ...
    /**
     * Set the forced root element that is entered into the edit area when the area is empty. By default this is a &lt;p&gt; element
     * 
     * @param rootElement
     */
     ...
     */
    public void setFileBrowserCallback(VFSContainer vfsContainer, CustomLinkTreeModel customLinkTreeModel, String[] supportedImageSuffixes,
    ...
    /**
     * Set an optional path relative to the vfs container of the file browser callback that is used as the upload destination when a user uploads a file and you don't
     * whant the file to be uploaded into the vfs container itself but rather in another directory, e.g. a special media directory.
     * 
     ...
     * @param relFilePath
     *            The file path of the HTML file relative to the documentBaseContainer
     * @param usess
     *            The user session
     */
    ...
    /**
     * Set a tiny configuration value that must not be quoted with quotes, e.g. JS function references or boolean values
     * 
     * @param key
     *            The configuration key
     * @param value
    ...
    /**
     * Get the relative file path in relation to the browser base container or an empty string when on same level as base container (e.g. in form and not file mode) or
     * NULL when the link browser and base container are not set at all
     * 
     * @return
     */
    ...
    /**
     * append all configurations to the given string buffer as js array key-value pairs. The creation of the array is not part of this method.
     * 
     * @param sb
    ...
    void appendConfigToTinyJSArray(StringOutput sb) {
        ...
        if (i > 0 && theme_advanced_disable.size() > 0)
            sb.append(",");
     ...
     * 
     * @param The
     *            string buffer where to append the js load commands
     */
    ...
    // ISSUE OLAT-7055: implemented this method as solution to disable media buttons for blog and podcast
    public void disableMediaAndOlatMovieViewer(VFSContainer baseContainer, boolean fullProfile, int row) {
        disableMedia(row);
        disableOlatMovieViewer(baseContainer);
    }

    private void disableMedia(int row) {
        setMediaEnabled(false, row);
    }

    private void disableOlatMovieViewer(VFSContainer baseContainer) {
        int profile;
        if (baseContainer == null)
            profile = CONFIG_PROFILE_FORM_EDITOR_FULL;
        else
        ...
        List<TinyMCECustomPlugin> enabledCustomPlugins = customPluginFactory.getCustomPlugionsForProfile(profile);
        for (TinyMCECustomPlugin tinyMCECustomPlugin : enabledCustomPlugins) {
            if (tinyMCECustomPlugin instanceof OlatMovieViewerPlugin) {
                setCustomPluginEnabled(false, tinyMCECustomPlugin, profile);
            }
        }
    }
