...
public class DispatcherAction implements Dispatcher {
    ...
    /**
     * incrementing an int is not atomar which needs to be synchronized AtomicInt provides the same without the need for syncing.
     */
    ...
    /**
     * As decoding a url is an expensive operation we try to minimize it here as most mapped pathes contain only ascii chars
     * 
     * @param request
     * @param pathInfo
     * @return
    ...
    private String subtractContextPath(HttpServletRequest request, String pathInfo) throws UnsupportedEncodingException {
        ...
        if (requestUri.indexOf(DOUBLE_SLASH) != -1)
            requestUri = requestUri.replaceAll(DOUBLE_SLASH, "/");
    ...
    public static void handleError() {
        if (log.isDebugEnabled())
            log.debug("handleError : do rollback");
     ...
     */
    /*
     * public static String getPathDefault() { return PATH_DEFAULT; }
     */
     ...
     * used by spring
     * 
     * @param defaultDispatcherName
     *            The defaultDispatcherName to set.
     */
