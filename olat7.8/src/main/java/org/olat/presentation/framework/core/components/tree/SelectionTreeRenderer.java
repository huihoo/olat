...
import org.olat.presentation.framework.core.components.form.FormRenderer;
...
public class SelectionTreeRenderer implements ComponentRenderer {
     ...
     * <code>PACKAGE</code>
     */
    public static final String PACKAGE = org.olat.presentation.framework.core.translator.PackageUtil.getPackageName(SelectionTreeRenderer.class);
    ...
    private static final String SCRIPT_POST = "\");\n return false;\n" + "}\n" + "return true;\n}\n" + "function checkall(checked) {\n"
            + "len = document.seltree.elements.length;\n" + "var i;\n" + "for (i=0; i < len; i++) {\n" + "if (document.seltree.elements[i].name.indexOf('"
    ...
    public void render(Renderer renderer, StringOutput target, Component source, URLBuilder ubu, Translator translator, RenderResult renderResult, String[] args) {
        ...
        if (root.getChildCount() != 0) {
            ...
            if (tree.isMultiselect() && atLeastOneIsAccessible) {
                target.append("<div class=\"b_togglecheck\"><a href=\"javascript:checkall(true);setFormDirty('").append(FormRenderer.JSFORMID).append(tree.hashCode())
                        .append("');\">");
        ...
        if (atLeastOneIsAccessible) {
            ...
            target.append(StringHelper.escapeHtmlAttribute(translator.translate(tree.getFormButtonKey())));
        ...
        if (tree.isShowCancelButton()) {
            ...
            target.append(StringHelper.escapeHtmlAttribute(translator.translate("cancel"))).append("\">");
    ...
    private boolean atLeastOneIsAccessible(TreeNode node) {
        ...
        for (int i = 0; i < node.getChildCount(); i++) {
            ...
            if (atLeastOneIsAccessible(child))
                return true;
        }
        return false;
    }
    ...
    private void renderChildNodes(TreeNode root, String indent, int treeID, boolean multiselect, boolean greyOutNonSelectable, boolean showAltTextAsHoverOnTitle,
            StringOutput sb, SelectionTree tree) {
        ...
        for (int i = 0; i < childcnt; i++) {
            ...
            if (!child.isAccessible() && greyOutNonSelectable) {
                ...
                if (child.getAltText() != null && showAltTextAsHoverOnTitle) {
                    ...
                    sb.append(StringHelper.escapeHtmlAttribute(child.getAltText()));
