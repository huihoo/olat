...
/**
 * Description:<br>
 * a fake component which only serves to request additional .js or .css files for a controller/component. In order to activate it, this component here must be inserted
 * into the rendertree, e.g. myVelocityContainer.put("jsAndCss", myJsAndCssComponent);
 * <P>
...
public class JSAndCSSComponent extends Component {
     ...
     * @param baseClass
     *            the class of the controller (or from the package's Manager) from where the resources will be fetched: e.g. org/olat/presentation/demo/_static/js or /css
     *            respectively.
     * @param jsFileNames
     *            maybe <code>null</code> An array of the jsFilenames, one entry is e.g. "script.js", which will then be resolved to baseClass/_static/js/script.js
     * @param cssFileName
     *            maybe <code>null</code> the name of the cssFile to include: e.g. "style.css" will be resolved to loc of baseClass/_static/css/style.css
     * @param forceCssRemove
     *            if true, the given css will be removed if no longer necessary (e.g. for a custom css as in the course), otherwise it will stay in the html-header for
     *            the whole usersession
     * @param rawHeader
     *            -only use if neither jsFileNames nor cssFileName can be used (only for dynamically created jslibs for example)-a string (e.g. "<script...." or "<link
     *            rel=... those entries will be refreshed after each page, that is entries causes an full page reload in ajax mode
     * @param refreshIntervall
     *            the time in miliseconds after which (in ajax mode) a refresh of the screen is needed. -1 means infinity/no refresh. use small times with caution, since
     *            it generates server load. after the given time, a poll (comet, push, hanging get for future release) is issued to the server to collect the dirty
     *            components and rerender them. when more than one interval is requested, then the minimum is taken.
     */
     ...
     * Use a Mapper/MapperRegistry to obtain a cssPath. The css is assumed to be flagged with autoremove, that is, it is removed as soon as no one requires it in the
     * validation phase (since those special css are created by a mapper which in turn is created by a controller so that it makes sense to limit the css lifespan to the
     * lifespan of the controller) <br>
     * Normally use the cssFileName parameter in the constructor for class-based static css files.
     * 
     * @param cssPathName
     *            the path to the css, e.g. /olat/m/1001/mycss.css
     * @param cssLoadIndex
     *            flag to indicate load order of this CSS. See JSAndCSSAdder.CSS_INDEX_*
     */
    ...
    /**
     * @param refreshIntervall
     *            - refresh intervall in ms, -1 no more polling i.e. <=1000 no more polling according to serverpart.html
     */
    ...
    /**
     * requires that a full page reload takes places. sometimes eval'ing a more complex js lib (such as tiny mce) directly into global context does not work (timing
     * issues?) this should be used only rarely when complex js is executed and has errors in it, since a full page refresh is slower than a ajax call. <br>
     * when this component is validated (last cycle before rendering), and a full page refresh is required, then a full page request command is sent via JSON to the
     * browser which then executes it using document.location.replace(...). Since this step involves two calls (JSON+reload), this is slower than a normal full page click
     * (aka known as non-ajax mode).
     */
