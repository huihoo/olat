...
import org.olat.connectors.rest.security.RestSecurityBean;
...
public class RESTDispatcher implements Dispatcher {
    ...
    public void execute(final HttpServletRequest request, final HttpServletResponse response, final String uriPrefix) {
        ...
        if (xOlatToken != null) {
            ...
            final Identity restIdentity = restSecurityBean.getIdentity(xOlatToken);
            //
            if (log.isDebugEnabled()) {
        ...
        // check for valid invitation path regardless of authenticated user session
        // http://bugs.olat.org/jira/browse/OLAT-6419
