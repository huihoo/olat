...
public class MenuTreeRenderer implements ComponentRenderer {
    ...
    private void renderLevel(StringOutput target, int level, TreeNode curRoot, List selPath, URLBuilder ubu, AJAXFlags flags, String componentPrefix, TreeNode markedNode) {
        ...
        /*
         * if (menuitemsDraggable && curSel == curRoot) { //TODO:collect all ids as done in constructor and "[1,435,323].each(function(val){new Draggabl...."
        ...
        // fix needed for firefox bug when fast clicking: the onclick="try{return o2cl()}catch(e){return false}" -> when the document is reloaded, all function js gets
        // unloaded, but the old link can still be clicked.
        ...
        target.append("\" title=\"");
        String altText = curRoot.getAltText() == null ? curRoot.getTitle() : curRoot.getAltText();
        altText = StringHelper.escapeHtmlAttribute(altText);

        target.append(altText);
        target.append("\"");
        if (iframePostEnabled) {
