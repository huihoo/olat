...
public class TableRenderer implements ComponentRenderer {
    ...
    protected static final String TABLE_SINGLESELECT_GROUP = "tb_ss";
    ...
    public void render(final Renderer renderer, final StringOutput target, final Component source, final URLBuilder ubu, final Translator translator,
        ...
        int endRowId = rows;
        // initalize paging
        if (table.isPagingEnabled() && currentPageId != null && !table.isShowAllSelected()) {
        ...
        appendTablePaging(target, translator, table, formName, rows, resultsPerPage, currentPageId, usePaging);
        appendTableSelectionFormActions(target, translator, table);
        appendViewportResizeJsFix(target, source, rows, usePaging);
    ...
    private void appendViewportResizeJsFix(final StringOutput target, final Component source, int rows, boolean usePaging) {
        // JS code to resize table to browser view port and display scrollbars in the table
        // when not in paging mode and more than 1000 results are shown.
    ...
    /**
     * Appends action (submit) buttons for defined actions after a table multi or single selection
     */
    private void appendTableSelectionFormActions(final StringOutput target, final Translator translator, final Table table) {
        target.append("<div class=\"b_table_buttons\">");

        if (table.isMultiSelect() || table.isSingleSelect()) {
            List<String> tableSelectionActionsi18nKeys = null;
            List<String> tableSelectionActionsIdentifiers = null;
            if (table.isMultiSelect()) {
                tableSelectionActionsi18nKeys = table.getMultiSelectActionsI18nKeys();
                tableSelectionActionsIdentifiers = table.getMultiSelectActionsIdentifiers();
            } else {
                tableSelectionActionsi18nKeys = table.getSingleSelectActionsI18nKeys();
                tableSelectionActionsIdentifiers = table.getSingleSelectActionsIdentifiers();
            }

            if (tableSelectionActionsi18nKeys.size() == 0) {
                throw new OLATRuntimeException(
                        null,
                        "Action key in table with selection is undefined. Use addMultiSelectI18nAction(\"i18nkey\", \"action\") or addSingleSelectI18nAction(\"i18nkey\", \"action\") to set an action for this table.",
                        null);
            }

            for (int i = 0; i < tableSelectionActionsi18nKeys.size(); i++) {
                String tableSelectionActionI18nKey = tableSelectionActionsi18nKeys.get(i);
                String tableSelectionActionIdentifer = tableSelectionActionsIdentifiers.get(i);
                target.append("<input type=\"submit\" name=\"" + tableSelectionActionIdentifer + "\" value=\""
                        + StringEscapeUtils.escapeHtml(translator.translate(tableSelectionActionI18nKey)) + "\" class=\"b_button\" />");
            }
        }
    ...
    private void appendTablePaging(final StringOutput target, final Translator translator, final Table table, final String formName, final int rows, int resultsPerPage,
            final Integer currentPageId, final boolean usePaging) {
        if (usePaging && (rows > resultsPerPage)) {
            ...
            appendTablePagingBackLink(target, translator, formName, pageid);
            addPageNumberLinks(target, formName, pageid, maxpageid);
            appendTablePagingNextLink(target, translator, formName, rows, resultsPerPage, pageid);
            appendTablePagingShowallLink(target, translator, table, formName);
    ...
    private void appendTablePagingShowallLink(final StringOutput target, final Translator translator, final Table table, final String formName) {
    ...
    private void appendTablePagingNextLink(final StringOutput target, final Translator translator, final String formName, final int rows, int resultsPerPage, int pageid) {
    ...
    private void appendTablePagingBackLink(final StringOutput target, final Translator translator, final String formName, int pageid) {
        if (pageid > 1) {
