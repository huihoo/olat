...
public class Link extends Component {
    ...
    protected Link(String name, String command, String i18n, int presentation, VelocityContainer vc, Controller listeningController) {
        ...
        if (listeningController == null)
            throw new AssertException("please provide a listening controller, listeningController is null at the moment");
        addListener(listeningController);
        if (vc != null)
            vc.put(getComponentName(), this);
    ...
    protected Link(String name, String command, String i18n, int presentation, Object internalAttachedObj) {
        ...
        if (this.command == null || this.command.equals(""))
            throw new AssertException("command string must be a valid string and not null");
        this.i18n = i18n;
        this.presentation = presentation;
        this.presentationBeforeCustomCSS = presentation;

        if (log.isDebugEnabled()) {
            log.debug("***LINK_CREATED***" + " name: " + getComponentName() + " component: " + getComponentName() + " dispatchId: " + getDispatchID());
        }
    ...
    protected void doDispatchRequest(UserRequest ureq) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("***LINK_CLICKED*** " + " dispatchID: " + ureq.getComponentID() + " commandID: " + cmd);
        }
    ...
    private void dispatch(UserRequest ureq, String cmd) {
        ...
        if (registerForMousePositionEvent)
            setXYOffest(ureq);
     ...
     * 
     * @param the
     *            i18n key or the translated key depending on presentation mode
     */
     ...
     * 
     * @param suppressDirtyFormWarning
     ...
     * 
     * @param sticky
     *            : sets the tooltip sticky, which means the user has to click the tip to disappear
     */
     ...
     * @param comp
     * @param stricky
     *            if true the user has to close the tooltip himself
     */
