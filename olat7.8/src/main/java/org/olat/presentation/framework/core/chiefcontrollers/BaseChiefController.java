...
import org.olat.presentation.framework.core.util.jslogging.JavaScriptTracingController;
import org.olat.presentation.framework.dispatcher.mapper.GlobalMapperRegistry;
...
/**
 * Description: <br>
 * This is the main chief controller for the olat application. It controls the window, implements a windowcontrol and has a header, content, and footer area.
 * <P>
...
public class BaseChiefController extends DefaultChiefController implements ContentableChiefController {
    static final String VELOCITY_ROOT = PackageUtil.getPackageVelocityRoot(BaseChiefController.class);
    ...
    static {
        ...
        /*
         * // check if mandatory jsmath files are unzipped, write error otherwise String fDir = WebappHelper.getContextRoot() + "/static/js/jsMath/"; File jsMathImages =
         * new File(fDir+"/fonts"); if (!jsMathImages.exists() || !jsMathImages.isDirectory() || !(jsMathImages.list().length > 0)) { File fZip = new File(fDir
         * +"/fonts.zip"); if (fZip.exists()) { try { jsMathEnabled = ZipUtil.unzip(fZip, new File(fDir)); } catch (Exception e) {
         * log.error("jsMath fonts are not available. Please unzip the file ``fonts.zip'' before starting olat."); } }
         * 
         * 
         * } else { jsMathEnabled = true; }
         */
        jsMathEnabled = true;
    }
    ...
    /**
     * The OLAT main window controller. This controller is responsible for a OLAT window
     * 
     * @param ureq
     *            The user request
     */
    public BaseChiefController(UserRequest ureq) {
        ...
        translator = PackageUtil.createPackageTranslator(this.getClass(), ureq.getLocale());
        ...
        if (ureq.getUserSession().isAuthenticated() && ureq.getUserSession().getRoles().isOLATAdmin()
        ...
        mainvc.contextPut("classPathStaticBaseURI", resourcePath.substring(0, resourcePath.indexOf("org.olat.presentation")));
    ...
    protected void event(UserRequest ureq, Component source, Event event) {
        if (source == getWindow()) {
            ...
            /*
             * String bc = wbo.getBusinessControlPath(); wbo.getWindowManager(). mainvc.contextPut("o_bc", bc);
             */
            ...
            /*
             * if (!bookmarkWaitOnce) { mainvc.contextPut("bkme", "true"); } else { bookmarkWaitOnce = false; }
             */
