...
import org.olat.lms.commons.validation.ValidationStatusHelper;
...
import org.olat.presentation.framework.core.util.component.FormComponentTraverser;
import org.olat.presentation.framework.core.util.component.FormComponentVisitor;
...
public class Form {
     ...
     * @param name
     * @param translator
     * @param rootFormContainer
     *            if null the default layout is choosen, otherwise the given layouting container is taken.
     * @param listener
     *            the component listener of this form, typically the caller
     * @return
     */
    ...
    private void doInitRequestParameterAndMulipartData(UserRequest ureq) {
        ...
        if (isMultipartEnabled() && ServletFileUpload.isMultipartContent(ureq.getHttpReq())) {
            ...
            try {
                    ...
                    if (item.isFormField()) {
                        ...
                        try {
                            ...
                            if (e.getCause() instanceof MalformedStreamException) {
                                log.warn("Could not read uploaded file >" + fileName
                ...
                log.error("Error while dispatching multipart form: file limit (" + uploadSize + ") exceeded", sizeLimitExceededException);
                requestError = REQUEST_ERROR_UPLOAD_LIMIT_EXCEEDED;
            } catch (IOException e) {
                log.warn("Error while dispatching multipart form: ioexception", e);
                requestError = REQUEST_ERROR_GENERAL;
            } catch (Exception e) {
                log.error("Error while dispatching multipart form: general exception", e);
                requestError = REQUEST_ERROR_GENERAL;
            }
        } else {
            // Get parameters the standard way
            log.debug("Dispatching non-multipart form", null);
    ...
    private class ValidatingFormComponentVisitor implements FormComponentVisitor {
        ...
        List<ValidationStatus> tmp = new ArrayList<ValidationStatus>();
