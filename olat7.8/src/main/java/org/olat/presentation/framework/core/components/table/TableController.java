...
/**
 * <!--**************--> <h3>Responsability:</h3> This controller wraps a table component and offers additional features like column selection. Two constructors are
 * supported: regular table and table with a table filter. Use the TableGuiConfiguration object to configure the various rendering options.
 * <p>
 * <!--**************-->
 * <h3>Events fired:</h3>
 * <ul>
 ...
 * <li><i>{@link org.olat.presentation.framework.components.table.Table Table component events}</i>:<br>
 ...
 * Creates an asynchronously delivered {@link org.olat.lms.commons.mediaresource.media.ExcelMediaResource excel media resource}.</li>
 ...
 * <p>
 * <!--**************-->
 * <h3>Special translators:</h3> Uses a translator provided in the constructor as <i>fallback</i>.
 * <p>
 * <!--**************-->
 * <h3>Hints:</h3> Opens a modal dialog for choosing which columns to hide or show.
 * <p>
 * 
...
public class TableController extends BasicController {
     ...
     * 
     * @param tableConfig
     *            The table GUI configuration determines the tables behavior, may be <code>null</code> to use default table configuration.
     * @param ureq
     *            The user request
     * @param wControl
     *            The window control
     * @param filters
     *            A list of filter objects ({@link ShortName})
     * @param activeFilter
     *            The initially activated filter object
     * @param filterTitle
     *            The translated title of the filter
     * @param noFilterOption
     *            The translated key for the no-filter filter or <code>null</code> if not used
     * @param tableTrans
     *            The translator that is used to translate the table
     */
    public TableController(final TableGuiConfiguration tableConfig, final UserRequest ureq, final WindowControl wControl, final List<ShortName> filters,
            final ShortName activeFilter, final String filterTitle, final String noFilterOption, final Translator tableTrans) {
     ...
     * 
     * @param tableConfig
     *            The table gui configuration determines the tables behaviour, may be <code>null</code> to use default table config.
     * @param ureq
     *            The user request
     * @param wControl
     *            The window control
    ...
    public TableController(final TableGuiConfiguration tableConfigP, final UserRequest ureq, final WindowControl wControl, final Translator tableTrans) {
        ...
        if (tableTrans != null) {
            setTranslator(PackageUtil.createPackageTranslator(I18nPackage.FRAMEWORK_CORE_COMPONENTS_TABLE_, ureq.getLocale(), tableTrans));
        }
        ...
        this.table.setPagingEnabled(tableConfig.isPagingEnabled());
    ...
    private void applyAndCheckChangedColumnsChoice(final UserRequest ureq) {
        List<Integer> selCols = colsChoice.getSelectedRows();
        if (selCols.size() == 0) {
            ...
            if (table.isSortableColumnIn(selCols)) {
                // ok
                table.updateConfiguredCols(selCols);
                // update user preferences, use the given preferences key
                if (prefs == null) {
                    prefs = new TablePrefs();
                }
                prefs.setActiveColumnsRef(selCols);
    ...
    protected void event(final UserRequest ureq, final Controller source, final Event event) {
        ...
        if (event instanceof EntriesChosenEvent) {
            ...
            if (!filterList.isEmpty()) {
                this.table.setSearchString(filterList.get(0));
    ...
    /**
     * @return The currently active filter object or <code>null</code> if no filter is applied
     */
    ...
    /**
     * @param activeFilter
     *            The currently applied filter or <code>null</code> if no filter is applied
     */
    ...
    /**
     * Notifies the controller about a changed table data model. This will check if the table data model has any values and show a message instead of the table when the
     * model has no rows.
     */
    public void modelChanged(final boolean resetSearchString) {
        ...
        if (tableModel != null) {
            ...
            if (table.isTableFiltered()) {
                ...
                resetLink = LinkFactory.createCustomLink(LINK_NUMBER_OF_ELEMENTS, LINK_NUMBER_OF_ELEMENTS, String.valueOf(table.getUnfilteredRowCount()),
                        Link.NONTRANSLATED, contentVc, this);
            } else {
    ...
    public void setTableDataModel(final TableDataModel tableDataModel) {
        ...
        if (!tablePrefsInitialized) { // first time
            if (prefs != null) {
                // if there are stored table prefs display columns according to it
                try {
                    table.updateConfiguredCols(prefs.getActiveColumnsRef());
    ...
    /**
     * Sets the sortColumn to a specific column id (zero based). Check if the column can be accessed and if it is sortable.
     * 
     * @param sortColumn
     *            The sortColumn to set
     * @param isSortAscending
     *            true: sorting is ascending
     */
    public void setSortColumn(final int sortColumn, final boolean isSortAscending) {
        if ((table.getColumnCount() > sortColumn) && table.getColumnDescriptor(sortColumn).isSortingAllowed()) {
    ...
    /**
     * Sets whether user is able to select a single row via radio button.
     * 
     * @param isSingleSelect
     */
    public void setSingleSelect(final boolean isMultiSelect) {
        table.setSingleSelect(isMultiSelect);
    }
     ...
     * @param actionIdentifier
     */
    public void addSingleSelectAction(final String actionKeyi18n, final String actionIdentifier) {
        table.addSingleSelectAction(actionKeyi18n, actionIdentifier);
    }
