...
package org.olat.presentation.framework.core.components.form;
...
import org.olat.presentation.framework.core.formelements.HTMLTextAreaElement;
import org.olat.presentation.framework.core.formelements.LinkElement;
import org.olat.presentation.framework.core.formelements.MultipleSelectionElement;
import org.olat.presentation.framework.core.formelements.PasswordElement;
import org.olat.presentation.framework.core.formelements.PopupData;
import org.olat.presentation.framework.core.formelements.RadioButtonGroupElement;
import org.olat.presentation.framework.core.formelements.SelectionElement;
import org.olat.presentation.framework.core.formelements.SingleSelectionElement;
...
import org.olat.presentation.framework.core.formelements.StaticTextElement;
...
import org.olat.presentation.framework.core.formelements.TitleElement;
import org.olat.presentation.framework.core.formelements.VisibilityDependsOnSelectionRule;
import org.olat.presentation.framework.core.formelements.WikiMarkupTextAreaElement;
import org.olat.presentation.framework.core.formelements.WikiMarkupTextAreaElementAutoSize;
...
public class FormRenderer implements ComponentRenderer {
    ...
    /**
     * This is a singleton! so it may not change instance variables Constructor for FormRenderer. There must be an empty contructor for the Class.forName() call
     */
    public FormRenderer() {
        super();
    }
    ...
    private void renderFormSubmit(Form f, StringOutput sb, Translator translator) {
        // Submit buttons
        if (f.getSubmitKeysi18n().size() == 0)
            throw new OLATRuntimeException(null, "Submit key in form is undefined. Use setSubmitKey(\"save\"); to set key in the form.", null);
        int counter = 0;
    ...
    private void renderFormElement(Form f, FormElement fe, StringOutput sb, Translator translator, URLBuilder ubu, String[] args) {
        ...
        if (fe instanceof TitleElement) {
            ...
            if (spe.isBr())
                sb.append("<div class=\"b_form_spacer\" />");
    ...
    private void renderWikiMarkupTextAreaElementAutoSize(Form f, TextAreaElement tea, StringOutput sb, boolean resizable) {
        ...
        if (f.isDisplayOnly()) {
            ...
            sb.append("<textarea onchange=\"return setFormDirty('").append(JSFORMID).append(f.hashCode()).append("')\" onclick=\"return setFormDirty('").append(JSFORMID)
                    .append(f.hashCode()).append("')\" cols=\"").append(tea.getCols()).append("\" rows=\"").append(tea.getRows()).append("\" name=\"")
                    .append(tea.getName()).append("\" id=\"").append(uniqueId).append("\"");
            sb.append(">").append(StringEscapeUtils.escapeHtml(val)).append("</textarea>");
            ...
            if (resizable) {
                sb.append("<script type=\"text/javascript\">").append("b_form_resizeTextarea('").append(uniqueId).append("');").append("</script>");
            }
        }
    }
    ...
    private void renderTextAreaElement(Form f, TextAreaElement tea, StringOutput sb, boolean allowHTML, String[] args) {
        ...
        if (f.isDisplayOnly()) {
            ...
            if (wrapoff)
                sb.append(" wrap=\"off\"");
            sb.append(">").append(allowHTML ? val : StringEscapeUtils.escapeHtml(val).toString()).append("</textarea>");
        }
    }
    ...
    private void renderTextElement(Form f, TextElement textElement, StringOutput sb, URLBuilder ubu) {
        ...
        if (f.isDisplayOnly()) {
            ...
            sb.append("<input type=\"text\" id=\"").append(textElement.getName()).append("\" name=\"").append(textElement.getName()).append("\" value=\"")
                    .append(htmlVal).append("\" size=\"").append(textElement.getSize()).append("\" maxlength=\"").append(textElement.getMaxLength()).append("\" ")
                    .append("onkeypress=\"return setFormDirty('").append(JSFORMID).append(f.hashCode()).append("')\" ").append("onclick=\"return setFormDirty('")
                    .append(JSFORMID).append(f.hashCode()).append("')\" ").append("id=\"").append(JSELEMENTID).append(cId).append("\" />");
            if (textElement.isUseDateChooser()) {
                // date chooser button
                sb.append("<span class=\"b_form_datechooser\" id=\"trigger_").append(textElement.getName()).append("\" title=\"")
                        .append(StringEscapeUtils.escapeHtml(f.getTranslator().translate("calendar.choose"))).append("\">&nbsp;</span>");
                // date chooser javascript
                sb.append("<script type=\"text/javascript\">").append("Calendar.setup({").append("inputField:\"").append(textElement.getName()).append("\",")
                        .append("ifFormat:\"");
                if (textElement.getDateChooserDateFormat() == null) {
                    ...
                    if (textElement.isDateChooserTimeEnabled())
                        sb.append(formatter.getSimpleDatePatternForDateAndTime());
                    else
                        sb.append(formatter.getSimpleDatePatternForDate());

                } else {
                    // use custom date format
                    sb.append(textElement.getDateChooserDateFormat());
                }
                sb.append("\",").append("button:\"trigger_").append(textElement.getName()).append("\",").append("align:\"Tl\",").append("singleClick:false,");
                if (textElement.isDateChooserTimeEnabled()) {
            ...
            if (popupData != null) {
                ...
                sb.append("','olatpopup','toolbar=no,location=no,directories=0,status=no,menubar=0,scrollbars=yes,resizable=yes,width=")
                        .append(popupData.getPopupwidth()).append(",height=").append(popupData.getPopupheight()).append("');win.focus();}\" title=\"").append(text)
                        .append("\" class=\"b_form_genericchooser\"></a>");
            }
        }
    }
    ...
    private void renderPasswordElement(Form f, PasswordElement pe, StringOutput sb) {
        ...
        if (f.isDisplayOnly()) {
            ...
            sb.append("<input type=\"password\" name=\"").append(pe.getName()).append("\" size=\"").append(pe.getSize()).append("\" id=\"").append(uniqueId)
                    .append("\" maxlength=\"").append(pe.getMaxLength()).append("\" onchange=\"return setFormDirty('").append(JSFORMID).append(f.hashCode())
                    .append("')\" onclick=\"return setFormDirty('").append(JSFORMID).append(f.hashCode()).append("')\" />");
        }
    }
    ...
    private void renderLink(Form f, LinkElement le, Translator translator, StringOutput sb) {
        ...
        if (f.isDisplayOnly()) {
            ...
            sb.append("<input name=\"").append(le.getName()).append("\" size=\"").append(32).append("\" maxlength=\"").append(265)
                    .append("\" onchange=\"return setFormDirty('").append(JSFORMID).append(f.hashCode()).append("')\" onclick=\"return setFormDirty('").append(JSFORMID)
                    .append(f.hashCode()).append("')\" value=\"");
            sb.append(url);
    ...
    private void renderRadioButtonGroup(Form f, RadioButtonGroupElement rbge, Translator translator, StringOutput sb) {
        ...
        if (rbge.getSelected() == -1)
            throw new AssertException("no selection at render time");
        sb.append("<div id=\"").append(JSELEMENTID + rbge.hashCode()).append("\">");
        if (f.isDisplayOnly()) {
            int sel = rbge.getSelected();
            String val = rbge.getValue(sel);
            if (val == null)
                throw new AssertException("value in readonly mode of radiobutton group (" + rbge.getName() + ") was null, selPos = " + rbge.getSelected());
    ...
    private void renderDropDown(Form f, SingleSelectionElement sse, StringOutput sb) {
        ...
        if (f.isDisplayOnly()) {
            ...
            if (val == null)
                throw new AssertException("value in readonly mode of drop-down list (" + sse.getName() + ") was null, selPos = " + sse.getSelected());
            sb.append(StringEscapeUtils.escapeHtml(val).toString());
    ...
    /*
     * private void renderRadioButtons(SelectionElement se, StringOutput sb) { String name = se.getName(); int cnt = se.getSize(); for (int i = 0; i < cnt; i++) { String
     * key = se.getKey(i); String value = se.getValue(i); boolean selected = se.isSelected(i); sb.append("<input type=\"radio\" name=\""); sb.append(name);
     * sb.append("\" value=\""); sb.append(key); sb.append("\""); if (selected) { sb.append(" checked=\"checked\""); } sb.append("> "); sb.append(value);
     * sb.append("<br />"); } }
     */
    ...
    private void renderCheckBoxes(Form f, MultipleSelectionElement me, Translator translator, StringOutput sb) {
        ...
        if (me.enableCheckAll() && !me.isReadOnly()) { // add check/uncheck link
            ...
            sb.append(", true);setFormDirty('").append(JSFORMID).append(f.hashCode())
                    .append("')}\"><input type=\"checkbox\" class=\"b_checkbox\" checked=\"checked\" disabled=\"disabled\" />&nbsp;");
            sb.append(translator.translate("form.checkall"));
            sb.append("</a>");

            sb.append("<br /><a href=\"#\" onclick=\"javascript:{b_form_toggleCheck(document.");
            sb.append(f.getComponentName() + "." + name);
            sb.append(", false);setFormDirty('").append(JSFORMID).append(f.hashCode())
                    .append("')}\"><input type=\"checkbox\" class=\"b_checkbox\" disabled=\"disabled\" />&nbsp;");
     ...
     * org.olat.presentation.framework.components.Component, org.olat.presentation.framework.render.URLBuilder, org.olat.presentation.framework.translator.Translator)
     */
    public void renderHeaderIncludes(Renderer renderer, StringOutput sb, Component source, URLBuilder ubu, Translator transll, RenderingState rstate) {
        /*
         * no longer needed here as js gets included by Form component for ajax compatibilty // FIXME:fj:a remove translator from interface, use source.getTranslator //
         * instead Form f = (Form) source; // add date chooser java script when in edit only mode if (!f.isDisplayOnly()) { Iterator it_formelemnames =
         * f.getNameIterator(); while (it_formelemnames.hasNext()) { String name = (String) it_formelemnames.next(); FormElement fe = f.getFormElement(name); // only
         * check on elements that are not set to readonly and // are of type text element if (!fe.isReadOnly() && fe instanceof TextElement) { TextElement tel =
         * (TextElement) fe; // if this text element uses a date chooser add date chooser java // script libraries if (tel.isUseDateChooser()) { // only include this css
         * and the .js once (loading the same .js file // more than once is unnecessary and gives javascript errors Object alreadyIncluded = rstate.getRenderInfo(this,
         * "date_js"); if (alreadyIncluded == null) { // the first time -> include rstate.putRenderInfo(this, "date_js", new Object());
         * sb.append("<link rel=\"stylesheet\" type=\"text/css\" media=\"all\" href=\""); String fontSize = renderer.getGlobalSettings().getFontSize();
         * Renderer.renderNormalURI(sb, "raw/css/jscalendar-" + fontSize + ".css");
         * sb.append("\" title=\"jscalendarcss\">\n").append("<script type=\"text/javascript\" src=\""); Renderer.renderNormalURI(sb, "raw/js/jscalendar/calendar.js");
         * sb.append("\"></script>\n").append("<script type=\"text/javascript\" src=\""); Renderer.renderNormalURI(sb, "raw/js/jscalendar/lang/calendar-");
         * sb.append(source.getTranslator().getLocale().getLanguage()).append(".js\"></script>\n").append( "<script type=\"text/javascript\" src=\"");
         * Renderer.renderNormalURI(sb, "raw/js/jscalendar/calendar-setup.js"); sb.append("\"></script>\n"); // finished, date chooser is initialized, skip all other
         * elements break; } // else omit output of css and js includes } } } if (!f.isDisplayOnly()) { sb.append("<script type=\"text/javascript\">\n");
         * sb.append("function checkform").append(f.getComponentName()).append("(){\n"); renderElementVisibilityDependencyRules(f, sb); sb.append("}</script>\n"); } }
         */
    }
    ...
    public void renderBodyOnLoadJSFunctionCall(Renderer renderer, StringOutput sb, Component source, RenderingState rstate) {
        ...
        if (!f.isValid() && !f.isDisplayOnly()) {
            ...
            if (focusElem != null) {
                ...
                if (focusElem instanceof RadioButtonGroupElement)
                    tmp.append("[0]");
    ...
    private void renderElementVisibilityDependencyRules(Form f, StringOutput sb) {
        ...
        while (iter.hasNext()) {
            ...
            sb.append("b_form_updateFormElementVisibility('").append(f.getComponentName()).append("','").append(selection.getName()).append("','")
                    .append(dependentElement.getName()).append("','").append(rule.getVisibilityRuleValue()).append("',")
                    .append(String.valueOf(rule.isVisibilityRuleResult())).append(",\"").append(resetValue).append("\",")
                    .append(String.valueOf(rule.isHideDisabledElements())).append(",").append(String.valueOf(rule.isPreventOppositeAction())).append(");\n");
        }
    }
