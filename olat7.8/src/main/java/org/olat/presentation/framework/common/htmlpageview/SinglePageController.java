...
package org.olat.presentation.framework.common.htmlpageview;
...
import org.olat.presentation.framework.core.components.htmlsite.ExternalSiteEvent;
 ...
 * <P/>
 * Initial Date: Dec 16, 2004 EVENTS: to listening controllers: - OlatCmdEvent (which has to be accepted by calling accept() on the event) - NewInlineUriEvent if the user
 * changed the page by clicking on a link
 * 
 * @author gnaegi
 */
public class SinglePageController extends BasicController implements CloneableController {
     ...
     * @param showHomeLink
     */
    public SinglePageController(UserRequest ureq, WindowControl wControl, final VFSContainer rootContainer, final String fileName, String currentUri,
    ...
    /**
     * Constructor for the generic single page controller. displays the html page (or any file if in iframe mode) and, if not on the first page and not in iframe mode,
     * offers a button to return to the start page. (useful for a "home" button)
     * <p>
     ...
     * @param inIframe
     *            if true, the contents are rendered within an iframe
     * @param fileName
     *            the relative filePath in the material folder starting with a slash, e.g. /welcome.html or /docu/info.html
     * @param rootContainer
     *            the root from which to resolve the files (like "the htdocs directory")
     * @param currentUri
     *            if not null, the start page is set to this uri (instead of the fileName arg). relative to the -corrected- rootcontainer if !allowRelativeLinks
     * @param allowRelativeLinks
     *            if true, an initial uri of /folder/a.html allows navigating till "/", if false, only navigating in /folder/ and subfolders of this folder is allowed
     * @param showHomeLink
     *            true enables the home link icon and link which is added to the SP, false removes icon and link.
     */
    public SinglePageController(UserRequest ureq, WindowControl wControl, VFSContainer rootContainer, String fileName, String currentUri, boolean allowRelativeLinks,
    ...
    /**
     * When you call this method the edit mode will be enabled. By default no edit is possible, you have to call this method after construction time explicitly
     */
    ...
    protected void event(UserRequest ureq, Controller source, Event event) {
        ...
        // issue OLAT-7090    
            ...
            if (htmlEditorController instanceof HTMLEditorController && CloseableModalController.CLOSE_MODAL_EVENT.equals(event)) {
                ((HTMLEditorController) htmlEditorController).releaseLock();
            }
        }
    }
        ...
        if (source == editLink) {
            if (event.getCommand().equals(COMMAND_EDIT)) {
                ...
                if (customLinkTreeModel == null) {
                    ...
                    htmlEditorController = WysiwygFactory.createWysiwygControllerWithInternalLink(ureq, getWindowControl(), g_new_rootContainer, g_curURI, true,
                            customLinkTreeModel);
                }
    ...
    /**
     * Set a scale factor to enlarge / shrink the entire page. This is handy when a preview of a page should be displayed. E.g: 2 will scale the page by factor 2, 0.5
     * will shrink the page by factor 2
     * 
     * @param scaleFactor
     *            > 0 or 1: don't scale; < 1: shrink page; > 1 enlarge page.
     * @param displayHeight
     *            > 0: size to fit; > 0: fixed size in pixel
     * @param hideOverflow
     *            true: don't show scroll-bars; false: default behavior with scroll-bars
     */
