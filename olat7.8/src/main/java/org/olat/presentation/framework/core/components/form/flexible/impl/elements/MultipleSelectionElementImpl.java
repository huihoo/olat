...
public class MultipleSelectionElementImpl extends FormItemImpl implements MultipleSelectionElement {
    ...
    private static final String PACKAGE = PackageUtil.getPackageName(MultipleSelectionElementImpl.class);
    protected static final String VELOCITY_ROOT = PackageUtil.getPackageVelocityRoot(PACKAGE);
    ...
    public void setSelectedValues(String[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            if (!foundKey) {
                log.warn("submitted key '" + key + "' was not found in the keys of formelement named " + this.getName() + "  , keys=" + Arrays.asList(keys));
            } else {
                selected.add(key);
            }
        }
    ...
    protected Component getFormItemComponent() {
        /**
         * FIXME:pb dirty hack or not? to allow singleselection subcomponents being added to surrounding formlayouters -> e.g. language chooser selectbox we have to
         * return the formLayoutContainer.Component if it was not a "custom" formlayouter. -> detection via ..endsWith() bad not beautyful but functional so far.
         */
    ...
    /**
     * Tells the component that the values are already escaped, this is only used for rendering. <br>
     * The <code>value</code> contains html tags, so we want to preserve them.
     * <p>
     * Call this before <code>setKeysAndValues</code>.
     */
