...
/**
 * <h3>Description:</h3> This controller serves as the main entry point of the context help display controller. The workflow is intended to run in a separate window.
 * <p>
 * The controller can be triggered in two modes:
 * <ul>
 * <li>/help/de/org.olat.presentation/mychelp_page.html : the new, preferred style</li>
 * <li>/help/de/mychelp_page.html : legacy mode to support old external links. Don't use this when creating new pages!</li> </li> Those are only the entry URL's. As soon
 * as the window is build up, everything works via the framework.
 * <p>
 * To be delievered context help files must be in a _chelp directory in the classpath. Links to the page are made using the velocity helper method
 * $r.contextHelp("org.olat.presentation", "mychelp_page.html", "help to mypage")
 * <p>
 * Each context help page must provide a translated title in the same package as the page is located using the syntax 'chelp.PAGENAME.title': Example: <br>
 * <code>page: my.package._chelp.my_help.html </code><br>
 * <code>chelp.my_help.title translated in my.package._i18n</code>
 * <p>
 * Context help files can reference static resources, e.g. images. Those images must be in the _static directory within the _chelp directory:<br>
 * <code>resource: my.package._chelp._static.myimage_de.png</code> <br>
 * The resource must be in the static directory for each locale. A fallback mechanism checks if the resource exists and uses the fallback image when no image is found. <br>
 * In the html template the resource must be addressed without the locale, e.g: <code>&lt;img src='chelpStaticDirUrl/myimage.png' &gt;</code> <br>
 * This will load the resource from my.package._chelp._static.myimage_de.png
 * <p>
 * The controller listens to LanguageChangedEvent and changes the locale for is children when such an event occures (fired by ContextHelpTopNavController)
 * <p>
 * The workflow features a table of contents and a page details view. The navigation is implemented using a bread crumb controller. Sub-pages are also supported. Links to
 * those sub-pages must be made with<br>
...
class ContextHelpMainController extends MainLayoutBasicController implements GenericEventListener {
    ...
    public ContextHelpMainController(UserRequest ureq, WindowControl control) {
        ...
        if (ureq.getUserSession().getSessionInfo() != null) {
            ...
            ThreadLocalUserActivityLogger.log(OlatLoggingAction.CS_HELP, getClass(),
                    CoreLoggingResourceable.wrapNonOlatResource(StringResourceableType.csHelp, "-1", ArrayUtils.toString(uriParts)));
        }
    }
