...
/**
 * <h2>Summary:</h2> <b>setting panels content</b> is optional and <tt>params</tt> can always be <tt>null</tt>
 * <ul>
 ...
 * </ul>
 * find out about <b>form item attributes</b> by querying it from the velocity page with:
 * <ul>
 ...
 * <p>
 * <h2>Description:</h2> Each implemented form item consists of 4 panels appearing as div's in the html rendered page. They belong together in a <i>logical way</i>, but
 * are <i>layoutable</i>, e.g. placeable anywhere within a velocity page. The following figure illustrates the logical composition.<br>
 * <img src="doc-files/fi_mainlayout.png"/><br>
 * <ul>
 * <li>the Element panel<br>
 * The form element panel holds for example one or more checkboxes, a dropdownbox, an input field, ... any other html form element, a link!, a custom element!</li>
 * <li>the Label panel<br>
 * textlabel for the element</li>
 * <li>the Error panel<br>
 * Either a textlabel as error text or another form element. This form element can then start a subworkflow explaining the error or for fixing the error.</li>
 * <li>the Example panel<br>
 * example text, explaing an input format or how to proceed with the shown element.</li>
 * </ul>
 * The following figure shows a concrete example of a yes-no switch, the form element is labeled "Depending on date". The screenshot is taken in <i>debug mode mini</i>
 * which is available in development mode only. SingleSelectionImpl is the FormItem implemenation in this case.<br>
 ...
 * code: <img src="doc-files/fi_dateswitchcode.png"/><br>
 * <ul>
 * <li>519: <tt>dateSwitch</tt> is the name of the element in the velocity container</li>
 * <li>521/522: <tt>form.easy.dateSwitch</tt> is the i18n key for labeling, and the label is made visible.</li>
 * <li>523/524: sets the i18n keys for yes and no, and selects no as default</li>
 * <li>528: add an onclick listener, so you get informed if yes or no is clicked</li>
 * <li>529: final step of adding the form element to form (layout), it is then available in the velocity page layouting the form under the name <tt>dateSwitch</tt></li>
 * </ul>
 * <p>
 * The form item goes to a velocity page under defined name, e.g. <tt>dateSwitch</tt>, and the corresponding logical panels are accessed with the following convention:
 * <ul>
 ...
 * It is very common that a layout needs information about whether a form item has an error, example, label or is mandatory. This information is available through a
 * {@link org.olat.presentation.framework.components.form.flexible.FormDecorator} which can be accessed for example as <tt>$f.hasError("dateSwitch")</tt>.
 * <P>
 * The {@link FormItem} is aggregated within {@link FormItemContainer} and both together form the <i>composite</i> pattern.<br>
...
public interface FormItem extends FormBaseComponentIdProvider {
    ...
    /**
     * reset the data in the field to a initial/predefined value. This method is called if in a form a reset request is issued. It is the counterpart to the validate call
     * which
     */
    public void reset();
     ...
     * <p>
     * Please note, that the caller is not the GUI Framework but the form items manageing class. A default and exemplary implemention is {@link Form}.
     * <p>
     * This method must be implemented by a specialised form item provider.
     * 
     * @param ureq
    ...
    /**
     * The form item's name within an manageing form container. This has nothing to do with a name displayed on the screen to the user.<br>
     * The default implementation {@link FormItemImpl} uses the name to make the element available in the velocity container.
     * <p>
    ...
    /**
     * label panel of the form item, <code>null</code> if no label provided/rendered
     * <p>
     * This method is used by a form infrastructure provider.
     * 
     * @return
    ...
    /**
     * Shows a <i>mandatory</i> icon next to this form item. This is only a GUI cue, it does not activate any validators. It is in the developer's responsibility to
     * validate the form.
     * 
     * @param isMandatory
     *            Whether this form item should have a <i>mandatory</i> icon.
     */
     ...
     * @param params
     *            Additional error message contents.
     */
    public void setErrorKey(String errorKey, String... params);

    /**
     * a complex "error" message, or a helper wizard to fix the error.<br>
     * It must be a form item itself, that starting the fixing workflow let the surrounding form evaluate (intermediate save values).<br>
     * Typically a {@link FormLink} is used here to start a workflow in a modal dialog.<br />
     * Use showError(true) for the visibility of the error.
     * 
     * @param errorFormItem
     * @param container
     *            the container containing the form item which has the error
     */
