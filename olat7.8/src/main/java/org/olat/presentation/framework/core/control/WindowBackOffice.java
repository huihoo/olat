...
public interface WindowBackOffice extends Disposable {
    ...
    /**
     * @param wco
     *            the Command to be sent to the client (all requests are queued and sent in a batch at the end of the request)
     */
    public void sendCommandTo(Command wco);

    /**
     * together with getData and registerForCycleEvents, this serves to exchange request-transient render-related data amongst parties, e.g. layoutcontrollers telling the
     * implementor of the windowcontrol/guimessage receiver where to visually put the guimessage. <br>
     * all the data is clear before the inline-rendering-about-to-start event, since after that point no state change must occur.
     * 
     * @param key
     *            the key (to be agreed upon by the inter-communicating parties)
     * @param value
     *            any object
     */
     ...
     * use when "critical" use cases/controllers are in progress, such as a qti-test where a (unintended) back-button-click should not leave the current workflow. <br>
     * this method should be called from the constructor of the controller that wishes to control the back-handling. <br>
     * an existing backhandler will be overridden, that is, only the latest set has any effect TODO: check if boolean (done/skip) instead of void makes sense here to
     * backevent-bubble. or if the back-handlers should be stacked (what would be real use cases for such a thing?)
     * 
     * @param backHandler
     */
