...
var translator;
if (mainWin) {
	translator = mainWin.b_jsTranslatorFactory.getTranslator(mainWin.o_info.locale, 'org.olat.presentation.framework.core.components.form.flexible.impl.elements.richText.plugins.olatmovieviewer')	
} else {
	// implement dummy-translator
	translator = {	translate : function(key) { return key; } }
}

var oldWidth, oldHeight, oldProvider;
...
var MovieViewerDialog = {
	...
	init: function() {
		...
		if (pl != "") {
			...
			oldWidth = oldHeight = undefined;
			
			var domIdentity = "olatFlashMovieViewer";
			var placeHolders = tinyMCEPopup.editor.dom.select("img.mceItemOlatMovieViewer");
			do {
				// Issue 7057: removed count and used uniqueid to generate unique id (simple algorithm could be improved when needed) for olat movie viewer HTML element.
				// That allows to use olatmovie viewer also on pages, where more as one instance tiny editor exists (problem with Question/Answer)
				var uniqueid = Date.now();
				domIdentity = "olatFlashMovieViewer" + uniqueid;
...
function setDefaultSize() {
	...
	if(pl.provider === "sound") {
		...
		if(oldWidth === undefined && (oldProvider === "sound" || oldProvider === undefined)) {
			document.forms[0].width.value = 420;
		} 
		if(oldHeight === undefined && (oldProvider === "sound" || oldProvider === undefined)) {
			document.forms[0].height.value = 300;
		}	
	}
	oldProvider = pl.provider;
}
