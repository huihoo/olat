 ...
 * <p>
 */
package org.olat.presentation.framework.core.control.generic.ajax.tree;
...
/**
 * <h3>Description:</h3> The ajax tree controller provides a dynamic tree view with drag and drop support. The datamodel is loaded dynamically on request for each
 * hierarchy level. <br>
 * You must provide a AjaxTreeModel object that implements such a dynamic data model.
 * <p>
...
public class TreeController extends BasicController {
     ...
     *            Javascript method used as callback when a node is draged and moved around over the tree. The method must be defined somewhere else in your code, in this
     *            case you provide the method name. Alternatively you cen define the method right here in the string with function(event){}. Use NULL if not used.
     */
    public TreeController(UserRequest ureq, WindowControl wControl, final String titleRootNode, final AjaxTreeModel dataModel, String nodeDragOverCallback) {
        super(ureq, wControl);
        ...
        treeDataMapper = new Mapper() {
            ...
            public MediaResource handle(String relPath, HttpServletRequest request) {
                ...
                if (StringHelper.containsNonWhitespace(nodeId)) {
                    ...
                    log.error("Got a tree data mapper request but node parameter is missing. Root node title was::" + titleRootNode, null);
                    data = "";
                }
    ...
    protected void event(UserRequest ureq, Component source, Event event) {
        if (source == mainVC) {
            ...
            try {
                ...
                log.warn("Could not generate URIHelper for URI::" + getUri, e);
                return;
            }
            ...
            if (event.getCommand().equals(CMD_MOVE)) {
                ...
                if (moveEvent.isResultSuccess()) {
                    ...
                    smr.setData("b_amt_status_title=\"" + moveEvent.getResultFailureTitle() + "\", b_amt_status_msg=\"" + moveEvent.getResultFailureMessage()
                            + "\"; b_amt_status=false;");
                }
                ...
                for (String oldPath : oldPathes) {
                    ...
                    if (!oldPath.startsWith(collapsedPath))
                        newPathes.add(oldPath);
                }
                guiPrefs.putAndSave(TreeController.class, guiPrefsKey, newPathes);
                // return empty resource
     ...
     * @param foldersFirst
     *            true: show sorted folder items first (items with attribute 'isTypeLeaf') and then sorted leafs
     */
     ...
     *            Javascript method used as callback when a node editor is started. The method must be defined somewhere else in your code, in this case you provide the
     *            method name. Alternatively you cen define the method right here in the string with function(event){}. Use NULL if not used. The function can return
     *            false to cancel the edit request (e.g. to implement non-editable nodes)
     * @param beforeCompleteCallback
     *            Javascript method used as callback when a node is edited and the user closes the editor to save the changes. The method must be defined somewhere else
     *            in your code, in this case you provide the method name. Alternatively you can define the method right here in the string with function(event){}. Use
     *            NULL if not used. The function can return false to stop the edit request and revert to the original state (e.g. to check for valid new values)
     */
    public void setTreeInlineEditing(boolean treeEditingEnabled, String beforeStartEditCallback, String beforeCompleteCallback) {
        mainVC.contextPut("treeEditingEnabled", Boolean.valueOf(treeEditingEnabled));
    ...
    /**
     * Select a certain node in the tree by the node path. When the tree is already rendered, the activation of the node in the GUI is only performed when the next AJAX
     * render cycle is performed. <br />
     * Example path: /rootNodeId/firstLevelNodeId/secondLevelNodeId This will select the node with the ID secondLevelNodeId
     * 
     * @param selectedPath
    ...
    /**
     * Remove a certain node in the tree by the node path. This is only a rendering helper method to remove a node in the GUI that has already been removed in the
     * datamodel. It does not change the datamodel whatsoever. <br />
     * Example path: /rootNodeId/firstLevelNodeId/secondLevelNodeId This will remove the node with the ID secondLevelNodeId and all its children from the rendered tree
     * 
     * @param removedPath
     *            The path of the node or NULL
     */
    ...
    /**
     * Reload a certain node in the tree by the node path. This will force the AJAX tree to reload the datamodel for the given node. This can be used e.g. when on the
     * server side a new node has been added to the model and you don't want to reload the entire tree. <br />
     * Example path: /rootNodeId/firstLevelNodeId/secondLevelNodeId This will reload the node with the ID secondLevelNodeId
     * 
     * @param reloadedPath
     *            the path of the node or NULL
     */
    ...
    protected void doDispose() {
        ...
        if (mainVC != null) {
            ...
            sb.append("if (o_info.objectMap.containsKey('").append(treePanelName).append("')) {var oldTree = o_info.objectMap.removeKey('").append(treePanelName)
