...
import org.olat.lms.activitylogging.ILoggingResourceable;
import org.olat.lms.activitylogging.IUserActivityLogger;
...
import org.olat.presentation.framework.core.control.state.StateConstants;
import org.olat.presentation.framework.core.control.winmgr.ControllerStateImpl;
import org.olat.presentation.framework.core.translator.PackageUtil;
import org.olat.presentation.framework.core.translator.Translator;
import org.olat.system.commons.RunnableWithException;
...
public abstract class DefaultController implements Controller, ControllerEventListener {
     ...
     */
    /* TODO: STATIC_METHOD_REFACTORING refactoring this method must be discussed */
    ...
    /**
     * @param wControl
     *            should always be provided except for basechiefcontroller etc. which provide their own impl. of a windowcontrol
     */
    ...
    protected void setOrigWControl(WindowControl wControl) {
        if (this.origWControl != null)
            throw new AssertException("can only set origWControl once!");
        if (wControl == null)
            throw new AssertException("can not accept a null Windowcontrol here");
        this.origWControl = wControl;
        this.newWControl = new LocalWindowControl(wControl, this);
    }
    ...
    public void addControllerListener(ControllerEventListener el) {
        ...
        if (listeners.contains(el))
            throw new AssertException("controllerEventListener '" + el.toString() + "' was already added to controller '" + toString());
        listeners.add(el);
    }
    ...
    protected void fireEvent(UserRequest ureq, Event event) {
        ...
        for (Iterator<ControllerEventListener> iter = listeners.iterator(); iter.hasNext();) {
            ...
            if (log.isDebugEnabled())
                log.debug("Controller event: " + this.getClass().getName() + ": fires event to: " + listener.getClass().getName());
    ...
    public final void dispatchEvent(UserRequest ureq, Component source, Event event) {
        if (!disposed) {
            ...
            if (disposedMessageController != null && wrapperPanel != null) {
                ...
                Translator pT = PackageUtil.createPackageTranslator(DefaultController.class, locale);
     ...
     * 
     * @param req
     *            the UserRequest from which the session is fetched
     */
     ...
     * 
     * @param loggingResourceable
     *            the loggingResourceable to be set on this Controller's IUserActivityLogger
     */
     ...
     * 
     * @param asynchronous
     *            if true, then this method is invoked by a different thread than the current user-gui-thread ("mouse-click-thread"). this means if set to true, then you
     *            should inform the user by replacing the current render subtree of your controller's component with e.g. a velocitycontainer stating a message like 'this
     *            object has been disposed by an other process/user. please click some other link to continue...
     */
    @Override
     ...
     * 
     * @param cstate
     *            the new state to change to. the concrete controller must adjust its own state to a state it once created. normally controllers will only remember the
     *            "important" states - the ones that must be accessible using a permalink and which can be reached using browser-back-button
     * @param ureq
     *            the UserRequest: using as normal, but calling ureq.getParameter(...) doesn't make sense here, since those are the parameters of a call in the past.
     */
    @SuppressWarnings("unused")
     ...
     */
    protected final void setDisposedMsgController(Controller disposeMsgController) {
        if (disposedMessageController != null) {
            ...
            // an external event.
            final String clazz = disposedMessageController.getClass().getName();
            final Component comp = disposeMsgController.getInitialComponent();
            final String compName = comp != null ? comp.getComponentName() : "Component is null";
