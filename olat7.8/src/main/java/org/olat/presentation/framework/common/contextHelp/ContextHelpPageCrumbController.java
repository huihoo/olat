...
/**
 * <h3>Description:</h3> The context help page crumb controller reads context help files from the classpath and embedds them into a context help viewer.
 * <p>
...
class ContextHelpPageCrumbController extends CrumbBasicController {
    ...
    /**
     * Constructor for a context help details page. This renders a crumb with the help page for the given bundle and page
     * 
     * @param ureq
     * @param lwControl
     * @param bundleName
    ...
    /**
     * Internal helper to initialize the mapper that provides context help resources. The mapper tries it first with the language of the user, then with the default
     * language and last but not least with the fallback language.
     */
    ...
    /**
     * Internal helper to add the page rating elements. If the user is not authenticated or the feature is disabled in the configuration, no such elements are added.
     * 
     * @param ureq
     */
    private void addPageRating(UserRequest ureq) {
        if (!ContextHelpModule.isContextRatingEnabled() || !ureq.getUserSession().isAuthenticated() || contextHelpPageVC == null)
            return;
        ...
        if (commentAndRatingService != null) {
            ...
            String key = calculateCombinedKey(pageTranslator.getLocale(), bundleName, page);
    ...
    private String calculateCombinedKey(Locale locale, String bundleName, String page) {
