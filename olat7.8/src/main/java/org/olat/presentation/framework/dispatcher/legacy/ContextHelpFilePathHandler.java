 ...
 * html files encoded in utf-8. This handler here adds the handling of non-existing files (for translations not ready yet). It shows a message in the current locale that
 * the file is not translated yet and offers a fallback to the default locale.
 * <P/>
 * 
 * @author Felix Jost
 */
public class ContextHelpFilePathHandler implements PathHandler {
    ...
    public void init(final String path) {
        ...
        if (path != null) {
            ...
            throw new StartupException("ContextHelpFilePathHandler did not find mandatory <root> element:" + path);
        }
    }
    ...
    public ResourceDescriptor getResourceDescriptor(final HttpServletRequest request, final String relPath) {
        try {
            ...
            final Locale loc = UserSession.getUserSession(request).getLocale();
