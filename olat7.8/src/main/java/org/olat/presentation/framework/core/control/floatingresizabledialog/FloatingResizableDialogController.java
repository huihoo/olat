 ...
 * <p>
 */
package org.olat.presentation.framework.core.control.floatingresizabledialog;
...
public class FloatingResizableDialogController extends BasicController {
    ...
    public FloatingResizableDialogController(UserRequest ureq, WindowControl wControl, Component content, String title, int initialWidth, int initialHeight, int offsetX,
        ...
        wrapper.contextPut("title", StringHelper.escapeJavaScript(StringHelper.escapeHtml(title)));
        wrapper.contextPut("collabsibleContentPanelTitel", StringHelper.escapeJavaScript(StringHelper.escapeHtml(collabsibleContentPanelTitel)));
    ...
    protected void event(UserRequest ureq, Component source, Event event) {
        ...
        if (event.getCommand().equals("geometry")) {
            ...
            if (p != null && Pattern.compile("^(\\d+),(\\d+):(\\d+),(\\d+)$").matcher(p).matches()) {
                ...
                if (m.find())
                    offsetX = Integer.parseInt(m.group());
                if (m.find())
                    offsetY = Integer.parseInt(m.group());
                if (m.find())
