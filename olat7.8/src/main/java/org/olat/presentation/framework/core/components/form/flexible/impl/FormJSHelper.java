...
public class FormJSHelper {
    ...
    /**
     * create for example an <code>onclick="o_ffEvent('ofo_1377','ofo_1377_dispatchuri','o_fi1399','ofo_1377_eventval','1')"</code>
     * 
     * @param form
     * @param id
     * @param actions
     ...
     */
    public static void appendRenderInstructions(StringOutput sb, String jsonRenderInstruction, Set acceptedInstructions) {
        JSONObject instr;
        try {
            instr = new JSONObject(jsonRenderInstruction);
            sb.append(" ");// ensure blank before appending instructions -> '
                           // '...
            for (Iterator iter = acceptedInstructions.iterator(); iter.hasNext();) {
            ...
            throw new OLATRuntimeException("error retrieving JSON style render instruction", e);
        }
    }
     ...
     *            where to append
     * @param id
     *            formItemId of the InlineEdit FormItem
     * @param oldHtmlValue
     *            escaped HTML value TODO:2009-09-26:pb: escaped values appear as &apos; and are not escaped back.
     * @param rootForm
     *            to extract the ID of the Form where to submit to.
     */
    ...
    /**
     * submits a form when the enter key is pressed. TextAreas are handeled special and do not propagate the enter event to the outer world
     * 
     * @param formName
     * @return
     */
    public static String submitOnKeypressEnter(String formName) {
        ...
        sb.append("Ext.get(document.forms['").append(formName).append("']).on('keypress', function(event) {if (13 == event.keyCode) {if (document.forms['")
                .append(formName).append("'].onsubmit()) {document.forms['").append(formName).append("'].submit();}}})");
        sb.append(getJSEnd());
        return sb.toString();
    }
