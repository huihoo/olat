...
package org.olat.presentation.framework.core.control.generic.dialog;
...
/**
 * Description:<BR>
 * Generic controller that displays a dialogue with two buttons, e.g. to to make a delete confirmation dialogue. Fires the following events:
...
public class DialogController extends DefaultController {
    private static final String VELOCITY_ROOT = PackageUtil.getPackageVelocityRoot(DialogController.class);
    private static final String PACKAGE = PackageUtil.getPackageName(DialogController.class);
     ...
     * @param secondButtonText
     * @param text
     *            the (non html) text to be rendered
     * @param userObject
     *            any UserObject to be retrieved later if the dialog has been answered by the user (convenience)
     */
    public DialogController(Locale locale, String firstButtonText, String secondButtonText, String text, Object userObject, boolean displayCloseIcon, String title) {
        ...
        if (secondButtonText != null)
            myContent.contextPut("secondButtonText", secondButtonText);
    ...
    /**
     * @param customHeader
     *            set a custom hader. if set to null, the default header is used
     */
