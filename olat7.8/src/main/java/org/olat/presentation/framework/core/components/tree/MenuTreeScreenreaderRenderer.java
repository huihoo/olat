...
public class MenuTreeScreenreaderRenderer implements ComponentRenderer {
    ...
    public void render(Renderer renderer, StringOutput target, Component source, URLBuilder ubu, Translator translator, RenderResult renderResult, String[] args) {
        ...
        INode cur = selNode;
        if (cur == null)
            cur = root; // if no selection, select the first node to
        // expand the children
        ...
        if (tree.isDirtyForUser() && selNode != null) {
            ...
            for (int i = 0; i < cnt; i++) {
                ...
                so.append("\" title=\"").append(StringHelper.escapeHtmlAttribute(tn.getAltText())).append("\">").append(tn.getTitle()).append("</a>");
    ...
    private void renderLevel(StringOutput target, int level, TreeNode curRoot, List selPath, URLBuilder ubu, AJAXFlags flags, String componentPrefix, TreeNode markedNode) {
        ...
        target.append("\" title=\"");
        target.append(curRoot.getAltText() == null ? title : StringHelper.escapeHtmlAttribute(curRoot.getAltText()).toString());
        target.append("\"");
        if (iframePostEnabled) {
