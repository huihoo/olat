 ...
 * @author Felix Jost
 */
public class Table extends Component implements Comparator<Integer> {
     ...
     */
    public static final String COMMAND_SINGLESELECT = "ss";
    ...
    // The commands are internal to the table and affect functionality such as sorting and paging.
    ...
    private List<String> multiSelectActionsI18nKeys = new ArrayList<String>();
    private List<String> multiSelectActionsIdentifiers = new ArrayList<String>();
    private BitSet multiSelectSelectedRows = new BitSet();
    private BitSet multiSelectReadonlyRows = new BitSet();

    private List<String> singleSelectActionsI18nKeys = new ArrayList<String>();
    private List<String> singleSelectActionsIdentifiers = new ArrayList<String>();

    private int selectedRowId = NO_ROW_SELECTED;
    ...
    protected void modelChanged() {
        ...
        // sorting before the comparator calls start
        ...
        // now sort
        resort();
        updatePaging(getCurrentPageId());
        if (pagingEnabled) {
            currentPageId = Integer.valueOf(1);
        }

        if (isMultiSelect()) {
            // Reset multi/single selected row(s)
    ...
    /**
     * @param position
     *            The position of the column descriptor. Set to -1 to add this CD at the end.
     * @param visible
     * @param cd
     */
    ...
    protected void doDispatchRequest(final UserRequest ureq) {
        ...
        if (formCmd != null && formCmd.length() > 0) {
            ...
            // check for multi/single select actions
            if (isMultiSelect()) {
                for (final String actionIdentifier : multiSelectActionsIdentifiers) {
                    if (ureq.getParameter(actionIdentifier) != null) {
                        formCmd = COMMAND_MULTISELECT;
                ...
                for (final String actionIdentifier : singleSelectActionsIdentifiers) {
                    if (ureq.getParameter(actionIdentifier) != null) {
                        formCmd = COMMAND_SINGLESELECT;
                        value1 = actionIdentifier;
                        updateSingleSelectState(ureq);
                        break;
                    }
                ...
                throw new AssertException("Multiple or SingleSelect mode expected.");
            }
        }
        dispatchRequest(ureq, formCmd, value1, value2);
    }
     ...
     * @param theValue
     */
    private void dispatchRequest(final UserRequest ureq, final String cmd, final String value1, final String value2) {
        if (COMMAND_SORTBYCOLUMN.equals(cmd)) {
            ...
            setDirty(true);
            resort();
        } else if (COMMAND_MOVECOLUMN_RIGHT.equals(cmd)) { // move column right
            ...
            setDirty(true);
        } else if (COMMAND_MOVECOLUMN_LEFT.equals(cmd)) { // move column left
            int col = Integer.parseInt(value1);
            int swapCol = (col - 1) % (getColumnCount());
            ColumnDescriptor cdMove = getColumnDescriptor(col);
            ColumnDescriptor cdSwap = getColumnDescriptor(swapCol);
        ...
        } else if (COMMAND_PAGEACTION.equals(cmd)) {
            if (value1.equals(COMMAND_PAGEACTION_SHOWALL)) {
                // updatePaging(null); (see OLAT-1340)
                ...
                if (currentPageId != null) {
                    updatePaging(Integer.valueOf(currentPageId.intValue() + 1));
                    setDirty(true);
                }
                ...
                if (currentPageId != null) {
                    updatePaging(Integer.valueOf(currentPageId.intValue()));
                } else {
                    updatePaging(Integer.valueOf(1));
                }
                setDirty(true);
            } else {
                updatePaging(Integer.valueOf(value1));
                setDirty(true);
            }

        } else if (COMMANDLINK_ROWACTION_CLICKED.equals(cmd)) {
            ...
            return;
        } else if (COMMAND_MULTISELECT.equals(cmd)) {
            setDirty(true);
            fireEvent(ureq, new TableMultiSelectEvent(COMMAND_MULTISELECT, value1, getMultiSelectSelectedRows()));
        } else if (COMMAND_SINGLESELECT.equals(cmd)) {
            setDirty(true);
            fireEvent(ureq, new TableSingleSelectEvent(COMMAND_SINGLESELECT, value1, selectedRowId));
        }
    }
    ...
    private void updateMultiSelectState(final UserRequest ureq) {
        ...
        int endRowId = rows;
        // initalize paging
        if (isPagingEnabled() && currentPageId != null && !isShowAllSelected()) {
    ...
    private void updateSingleSelectState(final UserRequest ureq) {
        selectedRowId = NO_ROW_SELECTED;
        final String selectedRowParam = ureq.getHttpReq().getParameter(TableRenderer.TABLE_SINGLESELECT_GROUP);
        if (selectedRowParam == null) {
            return;
        }
        int newSelectedRowId = NO_ROW_SELECTED;
        try {
            newSelectedRowId = Integer.parseInt(selectedRowParam);
        } catch (NumberFormatException nfe) {
            throw new OLATRuntimeException("Invalid rowID submitted as table singleselect parameter", nfe);
        }
        ...
        // we need this because user might have unchecked a row.
        for (int i = startRowId; i < endRowId; i++) {
            int sortedRow = getSortedRow(i);
            if (sortedRow == newSelectedRowId) {
                selectedRowId = sortedRow;
                break;
            }
        }
    }
    ...
    protected void resort() {
        if (isSortingEnabled()) {
            ...
            if (currentSortingCd == null) {
                throw new RuntimeException("cannot find columndesc for column " + sortColumn
            ...
            if (log.isDebugEnabled()) {
                ...
                log.debug("sorting time for " + (model == null ? "null" : model.getRowCount()) + " rows:" + (stop - start) + " ms");
            }
    ...
    @Override
    public int compare(Integer rowNumA, Integer rowNumB) {
        return (sortAscending ? currentSortingCd.compareTo(rowNumA, rowNumB) : currentSortingCd.compareTo(rowNumB, rowNumA));
    }
     ...
     */
    protected void setSortColumn(final int sortColumn, final boolean sortAscending) {
     ...
     * 
     * @param selectedRowUnselectable
     *            The selectedRowUnselectable to set
     */
     ...
     *            true: allow table sorting, false: never sort table, not even when adding a new table data model.
     */
    ...
    protected TableDataModel createChoiceTableDataModel() {
        return new ChoiceTableDataModel((isMultiSelect() || isSingleSelect()), this.allCDs, this.columnOrder, this.getTranslator());
    }

    /**
     * @param selRows
     */
    protected void updateConfiguredCols(final List<Integer> selCols) {
        setDirty(true);
        columnOrder.clear();
        for (int col : selCols) {
            // if multi/single select, skip the first cd (which is the multi/single select CD)
            if (isMultiSelect() || isSingleSelect()) {
                col += 1;
            }
            columnOrder.add(allCDs.get(col));
        }

        // make sure sorting is smooth in all cases
        if (isMultiSelect() || isSingleSelect()) {
            // add multi/single select CD at the beginning
            columnOrder.add(0, allCDs.get(0));
            sortColumn = 1;
        } else {
            sortColumn = 0;
        }
    ...
    public boolean isSortableColumnIn(final List<Integer> selRows) {
        for (final Integer row : selRows) {
            ColumnDescriptor cd = allCDs.get(row);
    ...
    /**
     * @param newPageId
     *            new id used as active page
     */
    protected void updatePaging(final Integer newPageId) {
        if (newPageId == null) {
     ...
     * @param enabledFlag
     */
    protected void setPagingEnabled(final boolean enabledFlag) {
        this.pagingEnabled = enabledFlag;
    }
    ...
    protected void setMultiSelect(final boolean multiSelect) {
        if (this.multiSelect != multiSelect) {
            if (multiSelect) {
                // add a CD to render an extra checkbox column
                addColumnDescriptor(new MultiSelectColumnDescriptor(), 0, true);
                // adjust sort column
                sortColumn++;
            } else {
                // remove extra checkbox column
    ...
    protected void setSingleSelect(final boolean singleSelect) {
        if (this.singleSelect != singleSelect) {
            if (singleSelect) {
                // add a CD to render an extra radio button column
                addColumnDescriptor(new SingleSelectColumnDescriptor(), 0, true);
                // adjust sort column
                sortColumn++;
            } else {
                // remove extra radio button column
                removeColumnDescriptor(0);
                // adjust sort column
                sortColumn--;
            }
    ...
    protected List<String> getMultiSelectActionsI18nKeys() {
        return multiSelectActionsI18nKeys;
    }

    protected List<String> getMultiSelectActionsIdentifiers() {
    ...
    protected void addSingleSelectAction(final String actionKeyi18n, final String actionIdentifier) {
        singleSelectActionsI18nKeys.add(actionKeyi18n);
        singleSelectActionsIdentifiers.add(actionIdentifier);
    }

    protected List<String> getSingleSelectActionsI18nKeys() {
        return singleSelectActionsI18nKeys;
    }

    protected List<String> getSingleSelectActionsIdentifiers() {
        return singleSelectActionsIdentifiers;
    }
...
/**
 * Table model for visible column selection dialog
 */
class ChoiceTableDataModel extends BaseTableDataModelWithoutFilter {

    private boolean tableSelectionEnabled;
    private List<ColumnDescriptor> allCDs;
    private List<ColumnDescriptor> columnOrder;
    private Translator translator;

    protected ChoiceTableDataModel(final boolean tableSelectionEnabled, final List<ColumnDescriptor> allCDs, final List<ColumnDescriptor> columnOrder,
            final Translator translator) {
        this.tableSelectionEnabled = tableSelectionEnabled;
        this.allCDs = allCDs;
    ...
    public int getRowCount() {
        // if this is a multi/single select table, we do not want the checkboxes of
        // the multi/single select to be disabled. therefore we simply exclude the entire
        // checkbox/radiobutton row (which is at the very beginning of the CD array).
    ...
    public Object getValueAt(final int row, final int col) {
        ColumnDescriptor cd = allCDs.get(tableSelectionEnabled ? (row + 1) : row);
        switch (col) {
