...
public class URLBuilder {
    ...
    /**
     * builds a java script command that executes in background iframe or as document.location.replace depending on the AJAX mode. This version also includes safety
     * checks for not breaking other running ajax requests TODO: may only offer one method but check for usage of o_beforeserver first!
     * 
     * @param buf
     *            the buffer to write to
     * @param keys
     ...
     * @param mode
     * @param singe
     *            or double quote as string termination
     */
    ...
    /*
     * public boolean isAjaxOn() { return true; }
     */

    /*
     * public void buildNonAjaxURI(StringOutput buf, String[] keys, String[] values, String modURI) { buildURI(buf, keys, values, modURI, AJAXFlags.MODE_TOBGIFRAME); }
     */

    /**
     * builds an uri. neither key nor values may contain the character UserRequest.PARAM_DELIM which is a ":" (colon). in case you think you should transfer a filename
     * using a param, then may be it is better to use a model anyway to avoid having special characters in the url (even though it is utf-8 encoded, but some
     * browsers/webservers do not seem to handle special chars in the url pathinfo part)
     * 
     * @param buf
     *            the buffer to write to
     * @param keys
     ...
     * @param modURI
     * @param mode
     *            indicates what kind of link it is (0 normal, 1 into background-iframe, ...)
     */
    ...
    private StringBuilder encodeParams(int mode) {
        ...
        /*
         * TODO bookmarking via hash part of url or via below result.append(UserRequest.PARAM_DELIM); // avoid empty string for stringtokenizer result.append('a'); if
         * (businessControlPath != null) { result.append(businessControlPath); }
         */
        return result;
    }
