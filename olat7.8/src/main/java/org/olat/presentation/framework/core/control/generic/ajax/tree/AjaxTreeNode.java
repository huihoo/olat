...
/**
 * <h3>Description:</h3> The AjaxTreeNode is a wrapper object for tree nodes used in the dynamic tree model. There are various configuration flags that can be set for
 * this node. Use the treeNode.put(key,value) method and the static Strings to set those flags.
 * <p>
 * The actual node from your data model is not in this node, the only reference to the real data object is the node identifyer. This AjaxTreeNode is only used for
 * displaying purposes.
 * <p>
 * Because it is only for display, we escape the untrusted text.
 * <p>
...
public class AjaxTreeNode extends JSONObject {
     ...
     * 
     * @param id
     *            The ID of this tree node. Must be unique within the entire tree
     * @param attributes
     ...
     */
    public AjaxTreeNode(String id, String text) throws JSONException {
        super.put(CONF_ID, id);
        super.put(CONF_TEXT, StringHelper.escapeHtml(text));
    }

    @Override
    public JSONObject put(String key, Object value) throws JSONException {
        if (CONF_QTIP.equals(key)) {
            return super.put(key, StringHelper.escapeHtml((String) value));
        }
