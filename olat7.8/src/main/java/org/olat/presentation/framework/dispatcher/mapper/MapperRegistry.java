...
/**
 * Description:<br>
 * Allows to register so called mappers, which map a certain session-unique url to a mapper which can dispatch requests to that url.
 * <P>
 * As of the 7.0 release, an additional cacheable mapper register method has been added that allows registering of mappers with a permanent mapper ID. This allows
 * browsers to cache the resources delivered by the mapper using the last-modified date.
 * <P>
...
public class MapperRegistry implements Dispatcher {
    ...
    public static final String CNAME = MapperRegistry.class.getName();
    ...
    /**
     * Register a non-cachable mapper. The mapper will have a new URL every time it is registered.
     * <p>
     * If your resources are static files, use the cacheable register version to allow browsers using the last-modified date to cache the resources.
     * <p>
     * 
     * @param mapper
    ...
    /**
     * Register a cacheable mapper by giving a mapper path ID. When setting the same ID every time for the same usage context, the browser can use the last modified date
     * to cache the resources. The mapper ID will be use as MD5 hash to make it URL save and shorter. In debug mode, all special characters are replaced with the '_'
     * character.
     * <p>
    ...
    public String registerCacheable(String cacheableMapperID, Mapper mapper) {
        ...
        if (log.isDebugEnabled()) {
            saveMapperID = cacheableMapperID.replaceAll("[^a-zA-Z0-9-_.:]", "_");
        } else {
            saveMapperID = Encoder.encrypt(cacheableMapperID);
        }
        if (map.containsKey(saveMapperID)) {
            log.warn("Trying to add a cacheable mapper with the cacheableMapperID::" + cacheableMapperID
                    + " that is already registered for another mapper. Using a non-cachable mapper instead. Check your code and use unique mapper ID's everywhere!", null);
            return register(mapper);
        } else {
            map.put(saveMapperID, mapper);
            return WebappHelper.getServletContextPath() + DispatcherAction.PATH_MAPPED + saveMapperID;
        }
    }
    ...
    public void execute(HttpServletRequest hreq, HttpServletResponse hres, String pathInfo) {
        ...
        if (m == null) {
            log.warn("Call to mapped resource, but mapper does not exist for path::" + pathInfo, null);
            DispatcherAction.sendNotFound(pathInfo, hres);
            return;
        }
        String mod = subInfo.substring(slashPos);
        if (mod.indexOf("..") != -1) {
            log.warn("Illegal mapper path::" + mod + " contains '..'", null);
