 ...
 * No need to escape the value string if would be static text (that is trusted text - from *.properties) - <br>
 * BUT sometimes it is not static text, so we escape it.
 * 
 * 
 * @author patrickb
 */
class StaticTextElementRenderer implements ComponentRenderer {
    ...
    public void render(Renderer renderer, StringOutput sb, Component source, URLBuilder ubu, Translator translator, RenderResult renderResult, String[] args) {
        ...
        // sb.append("\" ");
        sb.append(" >");
        sb.append(getValueAsSafeHtml(steC)); //
        sb.append("</span>");

    }

    private String getValueAsSafeHtml(StaticTextElementComponent staticTextElementComponent) {
        if (staticTextElementComponent.isTrustedText()) {
            return staticTextElementComponent.getValue();
        }
        return StringHelper.escapeHtml(staticTextElementComponent.getValue());
    }
