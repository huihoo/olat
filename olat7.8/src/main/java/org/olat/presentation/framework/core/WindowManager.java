...
public interface WindowManager extends Disposable {
     ...
     * ajax will be enable if all of the following criterion meet: a) it is globally enabled in olat.properties b) the browser of the user supports it c) the given
     * argument "enabled" is true
     * 
     * @param ureq
     * @param enabled
     *            if true, ajax should be enabled if possible
     */
    ...
    /**
     * locates and returns the resource found under <package-of-baseclass>/relpath; e.g. for baseClass org.olat.presentation.demo.MyDemo and relPath /js/myfunc.js ->
     * org/olat/presentation/demo/_static/js/myfunc.js
     * 
     * @param baseClass
     *            the class which packages denotes the base path for the lookup
     * @param relPath
     *            e.g. /js/myfunc.js
    ...
    /**
     * whether or not ajax mode ("web 2.0") is enabled. should only called by controllers to determine whether they can offer additional ui capabilites due to ajax turned
     * on. e.g. provide an autocompletion
     * 
     * @return
     */
    ...
    /**
     * a new browser window with content (controller) provided through the controller creator. If the different layouts in popup windows are needed, the application must
     * provide wrapping layouting controller containing the content. Feed in the wrapping layouting controller. <br>
     * please see the {@link org.olat.presentation.framework.components.link.Link} on how to define a link to act as a popup window link. Once such a link is clicked the
     * listening controller uses the
     * <code>popupBrowserWindow = getWindowControl().getWindowBackOffice().getWindowManager().createNewPopupBrowserWindowFor(ureq, this);</code> to create the popup
     * window (<code>this</code> may be any ControllerCreator!). Just call the method {@link PopupBrowserWindow#open(UserRequest)} to activate/open/render the content in
     * the new browserwindow. <br>
     * 
     * @param ureq
