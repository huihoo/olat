...
import org.olat.lms.webfeed.ItemPublishDateComparator;
...
import org.olat.presentation.webfeed.navigation.Dated;
import org.olat.presentation.webfeed.navigation.NavigationEvent;
import org.olat.presentation.webfeed.navigation.YearNavigationController;
...
public class ItemsController extends BasicController {
    ...
    static Event HANDLE_NEW_EXTERNAL_FEED_DIALOG_EVENT = new Event("cmd.handle.new.external.feed.dialog");
    static Event FEED_INFO_IS_DIRTY_EVENT = new Event("cmd.feed.info.is.dirty");
    private final SubscriptionContext subscriptionContext;
    private final BlogNotificationTypeHandler blogNotificationTypeHandler;
    private final PodcastNotificationTypeHandler podcastNotificationTypeHandler;
    ...
    public ItemsController(final UserRequest ureq, final WindowControl wControl, final Feed feed, final FeedViewHelper helper, final FeedUIFactory uiFactory,
            final FeedSecurityCallback callback, final VelocityContainer vcRightColumn, FeedItemDisplayConfig displayConfig, SubscriptionContext subscriptionContext) {
        super(ureq, wControl);
        ...
        blogNotificationTypeHandler = CoreSpringFactory.getBean(BlogNotificationTypeHandler.class);
        podcastNotificationTypeHandler = CoreSpringFactory.getBean(PodcastNotificationTypeHandler.class);
    }
    ...
    protected void event(final UserRequest ureq, final Component source, final Event event) {
        ...
        if (source == addItemButton) {
            currentItem = feedManager.createItem();
                ...
                if (event.equals(Event.CHANGED_EVENT)) {
                    ...
                    if (feedManager.getItemContainer(currentItem, feed) == null) {
                        ...
                        if (!feed.getItems().contains(currentItem)) {
                            ...
                            PublishEventTO publishEventTO = createFeedPublishEventTO(ureq, feed, PublishEventTO.EventType.NEW);
                            feedManager.addItem(currentItem, mediaFile, feed, publishEventTO);
                            ...
                            // Write item file
                            PublishEventTO publishEventTO = createFeedPublishEventTO(ureq, feed, PublishEventTO.EventType.CHANGED);
                            feedManager.updateItem(currentItem, mediaFile, feed, publishEventTO);
    ...
    private PublishEventTO createFeedPublishEventTO(final UserRequest ureq, final Feed feed, final PublishEventTO.EventType eventType) {
        PublishEventTO publishEventTO = PublishEventTO.getNoPublishInstance();
        if (PodcastFileResource.TYPE_NAME.equals(feed.getResourceableTypeName())) {
            publishEventTO = podcastNotificationTypeHandler.createPublishEventTO(subscriptionContext, feed.getResourceableId(), ureq.getIdentity(), currentItem,
                    eventType);
        } else if (BlogFileResource.TYPE_NAME.equals(feed.getResourceableTypeName())) {
            publishEventTO = blogNotificationTypeHandler.createPublishEventTO(subscriptionContext, feed.getResourceableId(), ureq.getIdentity(), currentItem, eventType);
        }
