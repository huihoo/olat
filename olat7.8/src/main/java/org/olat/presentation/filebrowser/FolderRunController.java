...
public class FolderRunController extends BasicController implements Activateable {
    ...
    protected void event(UserRequest ureq, Controller source, Event event) {
        if (source == folderCommandController) {
            if (event == FolderCommand.FOLDERCOMMAND_FINISHED) {
                ...
                enableDisableQuota(ureq);
                ((FolderEvent) event).setFolderName(folderComponent.getCurrentContainerPath());
                fireEvent(ureq, event);
            }
        } else if (source == cmc) {
    ...
    protected void event(UserRequest ureq, Component source, Event event) {
        if (source == folderComponent || source == folderContainer || source == editQuotaButton) {
            ...
            if (!isExistFolder()) {
                // showError("msg.error.folderdeleted");
                // return;
            }
    ...
    protected boolean isExistFolder() {
        VFSContainer currentContainer = folderComponent.getCurrentContainer();
        try {
            if (currentContainer != null && currentContainer.getParentContainer() != null && currentContainer.getParentContainer().getPath() != null
                    && currentContainer.exists()) {
                return true;
            }
        } catch (Exception e) {
            log.info("Folder doesn't exists: " + currentContainer + " one element is missing in:" + folderComponent.getCurrentContainerPath(), e);
        }
        return false;
    }
    ...
    protected void doDispose() {
        // child controllers are registerd with listenTo and get disposed in BasicController
    }
