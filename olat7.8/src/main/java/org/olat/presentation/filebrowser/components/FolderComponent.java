...
public class FolderComponent extends Component {
    ...
    private void setSortAsc(String col) {
        if (col == null)
            col = SORT_NAME; // "clicked" column not existent
    ...
    private void sort(String col) {
        if (col.equals(SORT_NAME)) { // sort after file name?
            ...
            comparator = new Comparator<VFSItem>() {
                ...
                public int compare(VFSItem o1, VFSItem o2) {
                    ...
                    if (leaf1 == null && leaf2 != null)
                        return -1; // folders are always smaller
            ...
            comparator = new Comparator<VFSItem>() {
                ...
                public int compare(VFSItem o1, VFSItem o2) {
                    ...
                    if (o1 instanceof VFSLeaf) {
                        if (!FolderHelper.isKnownFileType(type1))
                            type1 = translator.translate("UnknownFile");
                    } else {
                        type1 = (sortAsc) ? "" : "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ"; // it's a folder
                    }
                    if (o2 instanceof VFSLeaf) {
                        if (!FolderHelper.isKnownFileType(type2))
                            type2 = translator.translate("UnknownFile");
                    } else {
        ...
        if (currentContainerChildren != null)
            updateChildren(); // if not empty the update list
    }
    ...
    public void updateChildren() {
        ...
        while (it.hasNext()) {
            VFSItem item = it.next();
            if (!exclFilter.accept(item)) {
                it.remove();
            }
        }
    ...
    public boolean setCurrentContainerPath(String relPath) {
        ...
        if (vfsItem == null || !(vfsItem instanceof VFSContainer)) {
            // unknown path, reset to root container...
            currentContainer = rootContainer;
            log.info("unknown path, reset to root container: " + relPath);
            relPath = "";
            return false;
        }
     ...
     * 
     * @param customLinkTreeModel
     *            The link tree model or NULL to not use this feature in the editor
     */
