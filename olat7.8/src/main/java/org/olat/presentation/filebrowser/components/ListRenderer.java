...
import org.olat.data.commons.vfs.AbstractVirtualContainer;
...
public class ListRenderer {
    ...
    public String render(FolderComponent fc, URLBuilder ubu, Translator translator, boolean iframePostEnabled) {
        ...
        sb.append(" ext:qtip=\"").append(StringEscapeUtils.escapeHtml(translator.translate("header.Name"))).append("\">").append(translator.translate("header.Name"))
                .append("</a>");
        ...
        sb.append(" ext:qtip=\"").append(StringEscapeUtils.escapeHtml(translator.translate("header.Size"))).append("\">").append(translator.translate("header.Size"))
                .append("</a>");
        ...
        sb.append(" ext:qtip=\"").append(StringEscapeUtils.escapeHtml(translator.translate("header.Type"))).append("\">").append(translator.translate("header.Type"))
                .append("</a>");

        if (canVersion) {
            sb.append("</th><th class=\"b_briefcase_col_rev\">").append("<a href=\""); // file size column
            ubu.buildURI(sb, new String[] { PARAM_SORTID }, new String[] { FolderComponent.SORT_REV }, iframePostEnabled ? AJAXFlags.MODE_TOBGIFRAME
        ...
        sb.append(" ext:qtip=\"").append(StringEscapeUtils.escapeHtml(translator.translate("header.Status"))).append("\">").append(translator.translate("header.Status"))
                .append("</a>");
    ...
    private void appendRenderedFile(FolderComponent fc, VFSItem child, String currentContainerPath, StringOutput sb, URLBuilder ubu, Translator translator,
        ...
        if (child instanceof OlatRelPathImpl) {
            ...
            metaInfo = metaInfoService.createMetaInfoFor((OlatRelPathImpl) child);
        }
        boolean lockedForUser = FileMetadataInfoHelper.isLocked(metaInfo, fc.getIdentityEnvironnement());
        ...
        if (metaInfo != null) {
            ...
            if (StringHelper.containsNonWhitespace(metaInfo.getTitle())) {
                String title = StringHelper.escapeHtml(metaInfo.getTitle()); // TODO: LD: 1024: NOK
            ...
            String author = StringHelper.escapeHtml(metaInfo.getAuthor());
        ...
        if (metaInfo != null) {
            if (metaInfo.isLocked()) {
                ...
                if (lockedBy != null) {
                    String firstName = getUserService().getUserProperty(lockedBy.getUser(), UserConstants.FIRSTNAME, translator.getLocale());
                    String lastName = getUserService().getUserProperty(lockedBy.getUser(), UserConstants.LASTNAME, translator.getLocale());
                    String date = "";
        ...
        if (canWrite) {
            ...
            // show content edit action if HTML, text or CSS file which is not locked
            String nameLowerCase = name.toLowerCase();
            if (!isContainer && !lockedForUser
                    && (nameLowerCase.endsWith(".html") || nameLowerCase.endsWith(".htm") || nameLowerCase.endsWith(".txt") || nameLowerCase.endsWith(".css"))) {
            ...
            boolean canMetaData = FileMetadataInfoHelper.canMetaInfo(child);
            if (canMetaData) {
                if (lockedForUser) {
