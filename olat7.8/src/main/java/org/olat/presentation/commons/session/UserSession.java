...
public class UserSession implements HttpSessionBindingListener, GenericEventListener, Serializable {
    ...
    // this has to be initialized lazily => retrieve it via getter
    // initialization in a static initializer is not possible since this could
    // be called (Tomcat session serialization) before Spring initialization takes place
    private static int sessionTimeoutInSec;
    ...
    private void init() {
        ...
        log.trace("UserSession<init> END");
        clearSession = true;
    }
    ...
    public static UserSession getUserSession(HttpSession session) {
        ...
        synchronized (session) {// o_clusterOK by:fj
            ...
            if (us == null) {
                us = new UserSession();
                // triggers the valueBoundEvent => nothing more to do here
    ...
    public void valueBound(HttpSessionBindingEvent be) {
        if (log.isTraceEnabled()) {
            log.trace("Bind UserSession: " + this.toString() + " to HTTPSession: " + be.getSession().getId());
        }
    }
    ...
    public void valueUnbound(HttpSessionBindingEvent be) {
        try {
            ...
            if (ident == null) {
                // TODO this causes a stacktrace on session timeout
    ...
    private void signOffAndClearWithout() {
        log.trace("signOffAndClearWithout() START");
        ...
        synchronized (authUserSessions) { // o_clusterOK by:fj
            if (authUserSessions.remove(this)) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("UserSession already removed! for [" + ident + "]");
                }
            }
        }
        log.trace("signOffAndClearWithout() END");
    }
    ...
    public synchronized void signOffAndClear() { // o_clusterOK by:fj
        log.trace("signOffAndClear() START");
        //
    ...
    public void event(Event event) {
        ...
        log.trace("event() is SignOnOffEvent. isSignOn=" + se.isSignOn());
    ...
    public static int invalidateAllSessions() {
        log.trace("invalidateAllSessions() START");
        ...
        log.trace("invalidateAllSessions() END");
    ...
    public static int getGlobalSessionTimeout() {
        if (UserSession.sessionTimeoutInSec == 0) {
            UserSession.sessionTimeoutInSec = Settings.getSessionTimeout();
            log.info("HTTP session global timeout set to  " + UserSession.sessionTimeoutInSec + "s by configuration.");
        }
        return UserSession.sessionTimeoutInSec;
    }
    ...
    public static void setGlobalSessionTimeout(int sessionTimeoutInSec) {
        ...
        log.info("HTTP session global timeout set to " + UserSession.sessionTimeoutInSec + "s in admin console.");
