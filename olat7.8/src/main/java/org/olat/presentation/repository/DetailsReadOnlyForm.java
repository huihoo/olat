...
public class DetailsReadOnlyForm extends FormBasicController {
    ...
    final public static int MAX_DISPLAYNAME = 140;
    ...
    /**
     * The details form is initialized with data collected from entry and typeName. Handler is looked-up by the given handlerName and not by the entry's resourceableType.
     * This is to allow for an entry with no resourceable to initialize correctly (c.f. RepositoryAdd workflow). The typeName may be null.
     * 
     * @param name
     * @param t
     * @param entry
     ...
     */
    public DetailsReadOnlyForm(final UserRequest ureq, final WindowControl wControl, final RepositoryEntry entry, final String typeName, final boolean enableAuthorView) {
        super(ureq, wControl);
        this.entry = entry;
    ...
    protected void initForm(final FormItemContainer formLayout, final Controller listener, final UserRequest ureq) {
        ...
        descriptionReadOnly = uifactory.addStaticTextElementWithTrustedText("cif_description", "cif.description", description, formLayout);

        resourceName = uifactory.addStaticTextElement("cif_resourcename", "cif.resourcename", (entry.getResourcename() == null) ? "-" : entry.getResourcename(),
                formLayout);

        final StringBuilder typeDisplayText = new StringBuilder(100);
        if (typeName != null) { // add image and typename code
            final RepositoryEntryIconRenderer reir = new RepositoryEntryIconRenderer(getLocale());
        ...
        type = uifactory.addStaticTextElementWithTrustedText("cif_type", "cif.type", typeDisplayText.toString(), formLayout);
