...
public class PropPupForm extends FormBasicController {
     ...
     */
    public PropPupForm(final UserRequest ureq, final WindowControl wControl, final RepositoryEntry entry) {
        super(ureq, wControl);
        this.entry = entry;

        final String typeName = entry.getOlatResource().getResourceableTypeName();
        if (typeName != null) {
            handler = RepositoryHandlerFactory.getInstance().getRepositoryHandler(typeName);
        }

        keys = new String[] { "" + RepositoryEntry.ACC_OWNERS, "" + RepositoryEntry.ACC_OWNERS_AUTHORS, "" + RepositoryEntry.ACC_USERS,
                "" + RepositoryEntry.ACC_USERS_GUESTS };

        values = new String[] { translate("cif.access.owners"), translate("cif.access.owners_authors"), translate("cif.access.users"),
