...
public class Wiki implements WikiContainer, Serializable {
    ...
    public static final String NEW_PAGE = "O_new_page";
    ...
    protected Wiki(final VFSContainer wikiRootContainer) {
        if (wikiRootContainer == null) {
            throw new AssertException("null values are not allowed for the wiki constructor!");
        }
        wikiPages = Collections.synchronizedMap(new HashMap<String, WikiPage>());
        diffService = WikiManager.getInstance().getDiffService();
     ...
     * 
     * @param pageId
     *            use either the page name or the pageID
     ...
     * @param pageId
     * @param loadContent
     *            loads the page content (article) lazy (performance issues) when set true
    ...
    public WikiPage getPage(final String pageId, final boolean loadContent) {
        ...
        if (page.getContent() == null) {
            log.error("I-130606-0080: content==null for WikiPage[id=" + pageId + ", pageContainerPath=" + pageContainer.getPath() + "]");
        }
        ...
        if (loadContent) {
            ...
            if (leaf == null) {
                // ticket I-130422-0138 reported NPE here => dump context information
                throw new AssertException("I-130422-0138: No data found for wiki page '" + page.getPageId() + "." + WikiManager.WIKI_FILE_SUFFIX + "' [pageName="
                        + page.getPageName() + ", wikiPageContainer=" + pageContainer.getPath() + "]");
            }
    ...
    public void addPage(final WikiPage page) {
    ...
    /**
     * @param pageId
     *            , or filename for media and image files
     * @return
     */
    @Override
    ...
    public void removePage(final WikiPage page) {
    ...
    public List<String> getDiff(final WikiPage page, final int version1, final int version2) {
    ...
    public List<WikiPage> getHistory(final WikiPage page) {
    ...
    public WikiPage loadVersion(final WikiPage page, final int version) {
        ...
        if (version == page.getVersion() && !page.getContent().equals("")) {
            return page;
        }
        if (version == 0) {
            return new WikiPage("dummy");
        }
        if (page.getPageName().equals("dummy")) {
            return page;
        }
    ...
    public List<VFSItem> getMediaFileListWithMetadata() {
        return mediaContainer.getItems();
    }

    public List<VFSItem> getMediaFileList() {
    ...
    public String getRecentChanges(final Locale locale) {
        if (locale == null) {
        ...
        for (final Iterator<WikiPage> iter = pages.iterator(); iter.hasNext();) {
            ...
            if (!page.getPageName().startsWith("O_") && !page.getPageName().startsWith(WikiPage.WIKI_MENU_PAGE)) {
                ...
                if (author != 0) {
                    sb.append(getBaseSecurity().loadIdentityByKey(Long.valueOf(page.getModifyAuthor())).getName());
                }
                sb.append("\n");
                counter++;
            }
    ...
    public WikiPage findPage(final String name) {
        WikiPage page = null;
        if (pageExists(WikiManager.generatePageId(FilterUtil.normalizeWikiLink(name)))) {
     ...
     */
    public List<WikiPage> getPagesByDate() {
        final ArrayList<WikiPage> pages = new ArrayList<WikiPage>(wikiPages.values());
        Collections.sort(pages, WikiPageSort.MODTIME_ORDER);
        return pages;
    }
     ...
     */
    public List<String> getListOfAllPageNames() {
        final ArrayList<WikiPage> pages = new ArrayList<WikiPage>(wikiPages.values());
    ...
    public boolean mediaFileExists(final String imageName) {
        ...
        for (final Iterator<VFSItem> iter = mediaFiles.iterator(); iter.hasNext();) {
            final VFSLeaf leaf = (VFSLeaf) iter.next();
            if (leaf.getName().equals(Utilities.encodeForURL(imageName))) {
                return true;
            }
        }
        return false;
