...
import org.olat.presentation.instantmessaging.rosterandchat.XMPPConnListener;
...
public class ConnectToServerTask implements Runnable {
    ...
    private final InstantMessagingClient client;
    private static final Logger log = LoggerHelper.getLogger();

    private final InstantMessaging im;

    public ConnectToServerTask(final InstantMessagingClient client) {
        this.client = client;
    ...
    public void run() {
        for (int i = 0; i < 2; i++) {
            try {
                final XMPPConnection connection = client.getConnection();
                connection.connect();
                ...
                if (client.isConnected() && connection.isAuthenticated()) {
                    ...
                    final String defaultStatus = client.getDefaultRosterStatus();
                    if (defaultStatus.equals(Presence.Type.unavailable.toString())) {
                        client.sendPresenceUnavailable();
                    } else {
                        client.sendPresenceAvailable(Presence.Mode.valueOf(defaultStatus));
                    }
                    break;
                    ...
                    if (!im.hasAccount(client.getChatUsername())) {
                        final boolean success = im.createAccount(client.getChatUsername(), client.getPassword(), client.getFullName(), client.getEmail());
                        if (success) {
                            log.info("Audit:New instant messaging authentication account created for user:" + client.getChatUsername());
                ...
                if (e.getXMPPError() != null && e.getXMPPError().getCode() == 401) { // 401 -->
                                                                                     // "not authorised",
                                                                                     // normally this
                                                                                     // users do not
                                                                                     // have an
                                                                                     // account on
                                                                                     // the server
                    log.info("User is not authorized to connect to Instant Messaging server (username, server): " + client.getChatUsername() + ", "
                            + client.getServerName() + ". Make sure this users have an account on the IM server. I will try to recreate the account now");

                    if (!im.hasAccount(client.getChatUsername())) {
                        final boolean success = im.createAccount(client.getChatUsername(), client.getPassword(), client.getFullName(), client.getEmail());
                        if (success) {
                            log.info("Audit:New instant messaging account created for user:" + client.getChatUsername());
                            client.closeConnection(true);
                            continue;
                        }
                    }
            ...
            } catch (final Exception e) { // also catch
