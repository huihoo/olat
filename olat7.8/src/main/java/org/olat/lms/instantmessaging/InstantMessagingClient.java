...
import org.olat.lms.instantmessaging.task.ConnectToServerTask;
...
public class InstantMessagingClient {
     ...
     * @param password
     */
    protected InstantMessagingClient(IMUserInfo userInfo, final TaskExecutorService taskExecutorService, InstantMessaging imService) {
        this.userInfo = userInfo;
        this.taskExecutorService = taskExecutorService;
        jabberServer = imService.getConfig().getServername();
    ...
    protected void doConnect() {
        ...
        log.debug("Connecting to server by thread pool with user: " + userInfo.getUsername());

        taskExecutorService.runTask(new ConnectToServerTask(this));
    ...
    /**
     * @param xmppAddress
     *            jabber jid like guido@swissjabber.org
     ...
     *            like guido@swissjabber.org/office
    ...
    protected String parseJid(final String xmppAddressWithRessource) {
        if (xmppAddressWithRessource == null) {
            return null;
        }
     ...
     * 
     * @param reason
     *            The reason why this user is not allowd to chat e.g. doing test By setting the third param to 10 all messages should be send to this client even if other
     *            clients are up. So we have full control over them.
     */
    protected void disableCollaboration(final String reason) {
        ...
        imService.getClientManager().sendPresenceEvent(Presence.Type.available, userInfo.getUsername());
    ...
    protected void enableCollaboration() {
        ...
        if (recentStatusMode.equals(Presence.Type.unavailable.toString())) {
            sendPresence(Presence.Type.unavailable, null, 0, null);
            imService.getClientManager().sendPresenceEvent(Presence.Type.unavailable, userInfo.getUsername());
        } else {
    ...
    public String getFullName() {
        return userInfo.getFullUserName();
    }
    ...
    public String getChatUsername() {
        return imService.getIMUsername(userInfo.getUsername());
    }
    ...
    public String getDefaultRosterStatus() {
        return userInfo.getRosterDefaultStatus();
    }
