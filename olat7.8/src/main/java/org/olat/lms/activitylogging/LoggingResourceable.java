 ...
 * combined.
 * <p>
 * Besides the above (container for the triple type/id/name) it serves the purpose of doing checks between the businessPath/contextEntries and the
 * ThreadLocalUserActivityLogger's LoggingResourceables which have been collected all the way from the initial request creating a particular Controller to the actual
 * event handling method calling into IUserActivityLogger.log() - optionally passing additional LoggingResourceables.
 * <p>
 ...
 * <p>
 * If a comparison with the businessPath fails, a simple (technical) log.WARN is issued. This should then be noticed by the system administrator hence feeding back into a
 * patch or a fix for the next release.
 * <P>
...
public class LoggingResourceable implements ILoggingResourceable {
    ...
    /**
     * type of this LoggingResourceable - contains the OlatResourceable's type in the OlatResourceable case, or the enum name() of the StringResourceableType otherwise
     */
    private final String type_;

    /**
     * the id of this LoggingResourceable - contains the OlatResource or RepositoryEntry's ID in those cases, or -1 in the StringResourceableType case.
     */
    private final String id_;

    /**
     * the name of this LoggingResourceable - this can be the title in case of a course - or the html name of a page in case of cp
     */
    private final String name_;

    /**
     * the ILoggingResourceableType corresponding to this LoggingResourceable - this is used for checks against the businessPath
     */
     ...
     * 
     * @param arg
     *            the string to be size-restricted
     * @param maxBytes
     *            the maximum number of bytes the string should result to when converting into UTF-8
     * @return a string matching into the given number of bytes
     */
    public static String restrictStringLength(final String arg, final int maxBytes) {
     ...
     * 
     * @param arg
     *            the String to be restricted to the maxBytes length
     * @param maxBytes
     *            the max length allowed
     * @param argNameForLogging
     *            the name of the arg value - used in case there's an error to give more accurate logging details
     * @return
     */
    private static String restrictStringLength(final String arg, final int maxBytes, final String argNameForLogging, final boolean doLog) {
        if (arg == null) {
        ...
        try {
            ...
            if (doLog) {
                log.error("restrictStringLength: " + argNameForLogging + " too long. Allowed " + maxBytes + ", actual: " + arg.getBytes().length + ", value=" + arg);
            }
            ...
            log.error("restrictStringLength: unsupported encoding: ", uee);
    ...
    /**
     * Internal constructor to create a LoggingResourceable object with the given mandatory parameters initialized.
     * <p>
     * This method also does length checks to catch oversized parameters as early as possible (versus later in the hibernate/mysql handling)
     * <p>
     * 
     * @param resourceable
     *            the OlatResourceable if available - can be null
     * @param resourceableType
     *            the type which is used for comparison later during businessPath checks
     * @param type
     *            the type to be stored to the database
     * @param id
     *            the id to be stored to the database
     * @param name
     *            the name to be stored to the database
     */
    private LoggingResourceable(final OLATResourceable resourceable, final ILoggingResourceableType resourceableType, final String type, final String id,
            final String name) {
    ...
    public static LoggingResourceable wrapScormRepositoryEntry(final RepositoryEntry scormRepoEntry) {
     ...
     */
    public static LoggingResourceable wrapWikiOres(final OLATResourceable olatResourceable) {
     ...
     * @return a LoggingResourceable representing the given wiki
     */
    public static LoggingResourceable wrapPortfolioOres(final OLATResourceable olatResourceable) {
        if (olatResourceable == null) {
            throw new IllegalArgumentException("olatResourceable must not be null");
        }
    ...
    /**
     * General wrapper for an OlatResourceable - as it's not obvious of what type that OlatResourceable is (in terms of being able to later compare it against the
     * businessPath etc) an ILoggingResourceableType needs to be passed to this method as well.
     * 
     * @param olatResourceable
     *            a general OlatResourceable
     * @param type
     *            the type of the olatResourceable
     * @return a LoggingResourceable wrapping the given olatResourceable type pair
     */
    public static LoggingResourceable wrap(final OLATResourceable olatResourceable, final ILoggingResourceableType type) {
        RepositoryEntry repoEntry = null;
        if (olatResourceable instanceof RepositoryEntry) {
            repoEntry = (RepositoryEntry) olatResourceable;
        } else {
            repoEntry = RepositoryServiceImpl.getInstance().lookupRepositoryEntry(olatResourceable, false);
        }
        if (repoEntry != null) {
            return new LoggingResourceable(repoEntry, type, repoEntry.getOlatResource().getResourceableTypeName(), String.valueOf(repoEntry.getOlatResource()
                    .getResourceableId()), repoEntry.getDisplayname());
        } else if (olatResourceable instanceof OLATResource) {
            final OLATResource olatResource = (OLATResource) olatResourceable;
            return new LoggingResourceable(olatResource, type, olatResource.getResourceableTypeName(), String.valueOf(olatResource.getResourceableId()),
                    String.valueOf(olatResource.getKey()));
        } else {
            return new LoggingResourceable(olatResourceable, type, olatResourceable.getResourceableTypeName(), String.valueOf(olatResourceable.getResourceableId()), "");
        }
    }
     ...
     * 
     * @param type
     *            the ILoggingResourceableType which corresponds the given id/name information
     * @param idForDB
     *            the id - to be stored to the database
     * @param nameForDB
     *            the name - to be stored to the database
     ...
     */
    public static LoggingResourceable wrapNonOlatResource(final StringResourceableType type, final String idForDB, final String nameForDB) {
        return new LoggingResourceable(null, type, type.name(), idForDB, nameForDB);
    }
     ...
     *            the filename - to be stored to the database in the name field
     * @return a LoggingResourceable wrapping the given filename as type StringResourceableType.uploadFile
     */
    public static LoggingResourceable wrapUploadFile(final String uploadFileName) {
     ...
     */
    public static LoggingResourceable wrapBCFile(final String bcFileName) {
     ...
     * 
     * @param cpNodeName
     *            the node name - to be stored to the database in the name field
     * @return a LoggingResourceable wrapping the given node name as type StringResourceableType.cpNode
     */
    public static LoggingResourceable wrapCpNode(final String cpNodeName) {
     ...
     * 
     * @param spUri
     *            the single page uri - to be stored to the database in the name field
     * @return a LoggingResourceable wrapping the given uri as type StringResourceableType.spUri
     */
    public static LoggingResourceable wrapSpUri(final String spUri) {
     ...
     * 
     * @param right
     *            the name of the businessgroup right - to be stored to the database in the name field
     * @return a LoggingResourceable wrapping the given right name as type StringResourceableType.bgRight
     */
    public static LoggingResourceable wrapBGRight(final String right) {
     ...
     * @return a LoggingResourceable wrapping the given filename as type StringResourceableType.uploadFile
     */
    public static LoggingResourceable wrap(final BGArea bgArea) {
     ...
     * 
     * @param identity
     *            the identity - to be stored to the database in the name field
     * @return a LoggingResourceable wrapping the given identity as type StringResourceableType.targetIdentity
     */
    public static LoggingResourceable wrap(final Identity identity) {
     ...
     * 
     * @param forum
     *            the forum to be wrapped
     * @return a LoggingResourceable wrapping the given Forum
     */
    public static LoggingResourceable wrap(final Forum forum) {
        final String name;
        final List<Message> forumMessages = getForumService().getMessagesByForum(forum);
        if (forumMessages == null || forumMessages.size() == 0) {
            name = null;
        } else {
            name = forumMessages.get(0).getTitle();
        }
        return new LoggingResourceable(forum, OlatResourceableType.forum, forum.getResourceableTypeName(), String.valueOf(forum.getResourceableId()), name);
    }
     ...
     */
    public static LoggingResourceable wrap(final Message forumMessage) {
        return new LoggingResourceable(OresHelper.createOLATResourceableInstance(Message.class, forumMessage.getKey()), OlatResourceableType.forumMessage,
                OlatResourceableType.forumMessage.name(), String.valueOf(forumMessage.getKey()), forumMessage.getTitle());
    }
     ...
     * 
     * @param feed
     *            the feed to be wrapped
     * @return a LoggingResourceable wrapping the given feed
     */
    public static LoggingResourceable wrap(final Feed feed) {
        String title = feed.getTitle();
        // truncate title after 230 chars
        if (title.length() > 230) {
            title = title.substring(0, 229);
        }
        return new LoggingResourceable(feed, OlatResourceableType.feed, feed.getResourceableTypeName(), String.valueOf(feed.getResourceableId()), title);
    }
     ...
     */
    public static LoggingResourceable wrap(final Item item) {
        if (item.getExternalLink() != null) {
            ...
            if (guid.length() > 230) {
                guid = guid.substring(guid.length() - 230);
            }
     ...
     * 
     * @param bgContext
     *            the bgContext to be wrapped
     * @return a LoggingResourceable wrapping the given BGContext
     */
    public static LoggingResourceable wrap(final BGContext bgContext) {
        ...
        return new LoggingResourceable(bgContext, OlatResourceableType.bgContext, bgContext.getGroupType(), String.valueOf(bgContext.getResourceableId()), name);
    }
     ...
     */
    public static LoggingResourceable wrap(final BusinessGroup group) {
        return new LoggingResourceable(group, OlatResourceableType.businessGroup, group.getResourceableTypeName(), String.valueOf(group.getKey()), group.getName());
    }
     ...
     * 
     * @param course
     *            the course to be wrapped
     * @return a LoggingResourceable wrapping the given ICourse
     */
    public static LoggingResourceable wrap(final ICourse course) {
        return new LoggingResourceable(course, OlatResourceableType.course, course.getResourceableTypeName(), String.valueOf(course.getResourceableId()),
     ...
     */
    public static LoggingResourceable wrap(final CourseNode node) {
        final String name = node.getShortTitle();
        final String ident = node.getIdent();

        final String typeForLogging = node.getType();
        try {
            final Long id = Long.parseLong(ident);

            return new LoggingResourceable(OresHelper.createOLATResourceableInstance("CourseNode", id), OlatResourceableType.node, typeForLogging, node.getIdent(), name);
        } catch (final NumberFormatException nfe) {
            return new LoggingResourceable(null, OlatResourceableType.node, typeForLogging, node.getIdent(), name);
        }
    }
     ...
     * 
     * @param calendar
     *            the calendar to be wrapped
     * @return a LoggingResourceable wrapping the given calendar
     */
    public static LoggingResourceable wrap(final OlatCalendar calendar) {
     ...
     * @return
     */
    private static String createUniqueId(final String type, final String name) {
    ...
    @Override
    public int hashCode() {
        return type_.hashCode() + (id_ != null ? id_.hashCode() : 1)
                + (resourceable_ != null ? resourceable_.getResourceableTypeName().hashCode() + (int) resourceable_.getResourceableId().longValue() : 0)
                + (resourceableType_ != null ? resourceableType_.hashCode() : 0);
    }
    ...
    public boolean equals(final Object obj) {
        ...
        final LoggingResourceable lri = (LoggingResourceable) obj;
     ...
     */
    @Override
    public boolean correspondsTo(final ContextEntry ce) {
        if (ce == null) {
            return false;
        }
        final OLATResourceable ceResourceable = ce.getOLATResourceable();
        ...
        if (resourceable_ != null) {
            ...
            if (ceResourceable instanceof RepositoryEntry) {
                final RepositoryEntry re = (RepositoryEntry) ceResourceable;

                final OLATResource ores = re.getOlatResource();
                if (ores != null && ores.getResourceableTypeName().equals(resourceable_.getResourceableTypeName())
                ...
                final RepositoryEntry re = RepositoryServiceImpl.getInstance().lookupRepositoryEntry(ceResourceable.getResourceableId());
                if (re != null) {
                    final OLATResource ores = re.getOlatResource();
                    if (ores != null && ores.getResourceableTypeName().equals(resourceable_.getResourceableTypeName())
                            && ores.getResourceableId().equals(resourceable_.getResourceableId())) {
        ...
        // we still try...
        if (type_.equals(StringResourceableType.targetIdentity.name()) && ceResourceable.getResourceableTypeName() == "Identity") {
