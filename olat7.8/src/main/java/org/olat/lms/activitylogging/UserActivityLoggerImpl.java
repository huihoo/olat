 ...
 * <p>
 * The UserActivityLoggerImpl gathers all information during the course of time to be ready for the log() call later - i.e. to have all mandatory information at that
 * time. This includes such things as session/identity, the businessPath and the list of LoggingResourceables.
 * <p>
 * Also, this class implements the simpleDuration calculation - which is a simple time difference between log calls within a session. Note that this is really simple, as
 * it does not distinguish between having multiple log calls per click and having no log calls for a number of clicks - hence the simpleDuration can only be useful to
 * some extend. The plan is to introduce a semanticDuration or some other concept later. Anyway, for the simpleDuration the UserActivityLoggerImpl stores the last
 * LoggingObject (the actual hibernate object) into the session for comparison later.
 * <p>
 * <P>
...
public class UserActivityLoggerImpl implements IUserActivityLogger {
    ...
    /**
     * the list of LoggingResourceable objects which have been added to this UserActivityLoggerImpl yet - all used later for logging into the database
     */
     ...
     * <p>
     * The idea is to eventually achieve a complete match between LoggingResourceable and businessPath. While we are currently miles away from this, when it eventually
     * matches we could get rid of one or the other and only use for example the businessPath, i.e. the underlying contextEntries (which contain the actual
     * OlatResourceable objects). The problem though always seems to remain that not all information which we would like to log is actually a OlatResourceable - sometimes
     * it's as simple as a String (i.e. number of qti attemts etc) which we'll never convert to an OlatResourceable. Anyway, once we'd have a 100% correct businessPath
     * match with the loggingResourceables (at least, lets say the OlatResourceables thereof) we could simplify things here a lot
     */
    ...
    /**
     * whether or not this UserActivityLoggerImpl should propagate setters to businessPath and bcContextEntries_ to the runtimeParent.
     * <p>
     * This is needed for the following reason: there are two kinds of usages for the ThreadLocalUserActivityLogger:
     * <ul>
     * <li>It can happen that during an event() call a LoggingResourceable is set on the ThreadLocalUserActivityLogger which is needed lateron in the rendering - or in
     * other parts which are outside the original event() method. I.e. there should be a propagation mechanism where LoggingResourceables propagate to the current
     * parent-chaing of ThreadLocalUserActivityLoggers</li>
     * <li>During the simplevm eventbus handling though, we want to make sure that nothing propagates back to the caller - since we're simulating basically a new thread
     * calling an event handler - but of course that event handler could make changes to the ThreadLocalUserActivityLogger - for example by adding a LoggingResourceable -
     * this information we do not want to propagate</li>
     * </ul>
     * Hence this field controls whether or not information propagates to runtime parent ThreadLocalUserActivityLoggers or not
     */
     ...
     * <p>
     * The runtime parent is used to propagate LoggingResourceables and businessPath/contextEntries. The reason for this structure is to be able to take a Controller's
     * IUserActivityLogger during the event call, set it as ThreadLocalUserActivityLogger and still have the parent (i.e. the IUserActivityLogger which was set by the
     * time the event method was called) also be informed about changes. This is because there are situations where we do logging after the event method (i.e. during
     * rendering) and need to have the LoggingResourceables as well as the businessPath/contextEntries.
     * <p>
    ...
    private I18nManager i18nManager = I18nManager.getInstance();

    /**
     * package protected constructor of the UserActivityLoggerImpl based on a HttpServletRequest - out of which the session and identity are extracted.
     * <p>
     * This is used to set up a UserActivityLoggerImpl initially by OLATServlet.doPost and OLATServlet.init()
     * <p>
     * 
     * @param hReq
     *            the HttpServletRequest out of which the session and identity are extracted
     */
     ...
     * [for testing only]
     */
    UserActivityLoggerImpl(UserSession userSession, ActivityLoggingDao activityLoggingDao, boolean isLogAnonymous, Set<String> userProperties, I18nManager i18nManager) {
        propageToThreadLocal_ = false;
        runtimeParent_ = null;
        this.session_ = userSession;
        this.activityLoggingDao = activityLoggingDao;
        this.isLogAnonymous_ = isLogAnonymous;
     ...
     * 
     * @param wControl
     *            the WindowControl from which the businessPath and contextEntries are extracted
     ...
     *            the "blue print" UserActivityLogger which should be copied for runtime use
    ...
    /**
     * The special thing about this 'logger for EventBus' is, that it doesn't propagate resourceInfos/businessPath etc to the parent nor the
     * ThreadLocalUserActivityLogger.
     * <p>
     * 
     * @param controller
     *            the Controller from which the IUserActivityLogger and the WindowControl (businessPath) should be extracted
     * @return a new UserActivityLoggerImpl purpose built for use by the EventBus during fireEvent
     */
    public static UserActivityLoggerImpl newLoggerForEventBus(GenericEventListener listener) {
        if (!(listener instanceof Controller)) {
            throw new AssertException("Wrong argument UserActivityLoggerImpl.newLoggerForEventBus(), listener must be type of Controller; listener=" + listener);
        }
        Controller controller = (Controller) listener;
    ...
    public void addLoggingResourceInfo(ILoggingResourceable loggingResourceable) {
        ...
        if (existingPos != -1) {
            ...
            if (existingRI.getName() != null && loggingResourceable.getName() != null && existingRI.getName().equals(loggingResourceable.getName())) {
                // ignore - already set
                return;
    ...
    public void frameworkSetBusinessPath(String businessPath) {
        if (businessPath == businessPath_ || businessPath == null || businessPath.length() == 0
                || (businessPath_ != null && businessPath.length() < businessPath_.length())) {
            return;
        }
    ...
    /**
     * Returns the combined and orderd list of LoggingResourceables which are set on this UserActivityLoggerImpl and are matching the contextEntries.
     * <p>
     * Note that this method fails if there is a contextEntry which doesn't have a corresponding LoggingResourceable: This would be a situation where the
     * businessPath/contextEntry has a resource defined which is unknown to this UserActivityLoggerImpl - i.e. which has not been set by the Controller or not been passed
     * via the log() call.
     * <p>
     ...
     * @param lriOrNull
     *            an
     * @return the combined and ordered list of LoggingResourceables which should go right to the database in the corresponding fields
     */
                ...
                if (!foundIt) {
                    ...
                    if (ce != null && ce.getOLATResourceable() != null) {
                        try {
                            ...
                            log.error("SecurityException while retrieving getOlatResource() Method from " + ce.getOLATResourceable().getClass());
                        } catch (NoSuchMethodException e) {
                            log.info("(OK) ContextEntry's OLATResourceable had no further getOlatResource() method: " + ce.getOLATResourceable().getClass());
                        } catch (IllegalArgumentException e) {
                            log.error("IllegalArgumentException while calling getOlatResource() Method from " + ce.getOLATResourceable().getClass(), e);
                        } catch (IllegalAccessException e) {
                            log.error("IllegalAccessException while calling getOlatResource() Method from " + ce.getOLATResourceable().getClass(), e);
                        } catch (InvocationTargetException e) {
                            log.error("IllegalAccessException while calling getOlatResource() Method from " + ce.getOLATResourceable().getClass(), e);
                        }
                    }
                    log.info("Could not find any LoggingResourceable corresponding to this ContextEntry: " + ce.toString() + ", ce.getOLATResourceable()="
                            + ce.getOLATResourceable() + ", ce.getOLATResourceable().getOlatResource()=" + oresourceableOres + ", dump of resource infos:");
                    for (Iterator<ILoggingResourceable> it2 = inputCopy.iterator(); it2.hasNext();) {
                        ILoggingResourceable resourceInfo = it2.next();
                        log.info("id: " + resourceInfo.getId() + ", name=" + resourceInfo.getName() + ", type=" + resourceInfo.getType() + ", toString: "
                                + resourceInfo.toString());
                    }
                    // removed throwable from error logging due to overflow of the log files with stack traces - probably relate to OLAT-4653
                    log.error("Could not find any LoggingResourceable corresponding to this ContextEntry: " + ce.toString());
                }
            }
        }
     ...
     * @return
     */
    private ActivityLoggingDao getActivityLoggingDao() {
        // Do it lazy because reference to dao can be resolved after spring startup
        if (activityLoggingDao != null) {
            return activityLoggingDao;
        } else {
            synchronized (this) {
                if (activityLoggingDao == null) {
                    activityLoggingDao = (ActivityLoggingDao) CoreSpringFactory.getBean(ActivityLoggingDao.class);
                }
                return activityLoggingDao;
            }
        }
