 ...
 * as (greatGrandParent,grandParent,parent,target) resources in the logging table.
 * <p>
...
public class CoreLoggingResourceable implements ILoggingResourceable {
    ...
    private CoreLoggingResourceable(OLATResourceable resourceable, ILoggingResourceableType resourceableType, String type, String id, String name) {
        if (type != null && type.length() > 32) {
            log.error("<init> type too long. Allowed 32, actual: " + type.length() + ", type=" + type);
            type = type.substring(0, 32);
        }
        if (id != null && id.length() > 64) {
            log.error("<init> id too long. Allowed 64, actual: " + id.length() + ", id=" + id);
            id = id.substring(0, 64);
        }
        if (name != null && name.length() > 230) {
            log.error("<init> name too long. Allowed 230 (to have some margin to 256), actual: " + name.length() + ", name=" + name);
    ...
    public static CoreLoggingResourceable wrapNonOlatResource(StringResourceableType type, String idForDB, String nameForDB) {
        return new CoreLoggingResourceable(null, type, type.name(), idForDB, nameForDB);
    }
    ...
    public static CoreLoggingResourceable wrap(OLATResourceable olatResourceable, ILoggingResourceableType type) {
        return new CoreLoggingResourceable(olatResourceable, type, olatResourceable.getResourceableTypeName(), String.valueOf(olatResourceable.getResourceableId()), "");
    }
    ...
    @Override
    public int hashCode() {
        return type_.hashCode() + id_.hashCode()
