 ...
 * <p>
 * There are two groups of methods in this class: those used for running a piece of code with a guaranteed and managed IUserActivityLogger set in the
...
public class ThreadLocalUserActivityLoggerInstaller {
     ...
     * @param logger
     *            the logger to be set during the runnable.run call
     * @throws Exception
     *             thrown by runnable.run
     */
    ...
    public static void runWithUserActivityLogger(Runnable runnable, IUserActivityLogger logger) {
        ...
        if (logger == null) {
            if (originalLogger == null) {
                log.warn("runWithUserActivityLogger: no logger available. Reinitializing...");
        ...
        if (logger == null)
            throw new IllegalStateException("PostCondition logger != null violated:");
        ...
        try {
            ...
            // make sure to reset the original logger in the end
            if (log.isDebugEnabled() && originalLogger == null)
                log.debug("reset original logger back with originalLogger == null!");
            ThreadLocalUserActivityLogger.userActivityLogger_.set(originalLogger);
        }
    }
