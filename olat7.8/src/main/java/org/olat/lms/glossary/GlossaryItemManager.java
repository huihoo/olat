...
import org.olat.presentation.framework.core.control.generic.textmarker.TextMarker;
import org.olat.presentation.framework.core.control.generic.textmarker.TextMarkerManager;
...
/**
 * Description:<br>
 * holds the glossary item list in a cache, to deliver it faster and to prevent from filesystem access or multiple loading into ram. each glossary will only be loaded
 * once, until it changes and cache is invalidated. read and writes the enhanced docBook-glossary format from/to filesystem.
 * 
...
public class GlossaryItemManager extends BasicManager {
    ...
    protected void upgradeAndDeleteOldGlossary(VFSContainer folderContainingGlossary, VFSLeaf textMarkerFile) {
        ...
        if (folderContainingGlossary.resolve(GLOSSARY_FILENAME) != null) {
            log.error("Upgrading Glossary in " + folderContainingGlossary.toString()
                    + ": There is already a new glossary-file. There can't be an old and a new version in the same directory!", null);
        } else { // upgrade it
    ...
    public boolean isFolderContainingGlossary(VFSContainer folderContainingGlossary) {
        ...
        if (glossaryFileNew == null && glossaryFileOld == null)
            return false;
        else
            return true;
    }
    ...
    private ArrayList<GlossaryItem> loadGlossaryItemListFromFile(VFSLeaf glossaryFile) {
        ...
        if (glossaryFile == null) {
            return new ArrayList<GlossaryItem>();
        }
