...
package org.olat.lms.scorm.manager;
...
import org.olat.lms.scorm.contentpackaging.NavigationViewer;
...
public class ScormManager extends BasicManager {
    ...
    /**
     * instance of the navigationviewer - used to lookup values/elements from the manifest.
     */
    ...
    /**
     * The manager's instance of the scorm sequence manager - used for sequencing and prerequisites, and cmi updating models
     */
    ...
    /**
     * Used to keep track of how many items there are in a organization (this is array based, so initially set to -1)
     */
    ...
    /**
     * Alternate constructor - used to pass the the package name (i.e. - used to construct a path to disk and find the files needed. It also allows us to specify if we
     * want to show the tree widget and the next and previous buttons in the navigation
     * 
     * @param packagePath
     * @param repoId
     * @param courseId
    ...
    public ScormManager(final String packagePath, final boolean showTreeWidget, final boolean showNavigation, final boolean autoProgression,
            final ISettingsHandler settings) {
    ...
    private void init() {
        ...
        if (scormSettingsHandler.getManifestFile().exists()) {
            ...
            final Element manifestRoot = (Element) _navViewer.getRootElement().clone();
            // now get the organizations node
            final Element orgs = manifestRoot.getChild(CP_Core.ORGANIZATIONS, manifestRoot.getNamespace());
            final Element[] tmp = _navViewer.getOrganizationList();
    ...
    /**
     * A method to do some initialising before we actually play the package. Here we get the default organization and then pass that to a method which will parse that
     * organization (JDOM) and figure out what we need, in terms of items, sequence, prerequisites, etc
     */
    ...
    /**
     * A method to start the parse of the manifest and write the javascript strings which the tree widget needs and also to populate the sequencer
     * 
     * @param useRelativePaths
     * @return String[]
     */
    protected String[] parse(final boolean useRelativePaths) {
        // New Vector
        ...
        final String[] javascriptStrings = new String[v.size()];
        v.copyInto(javascriptStrings);
        return javascriptStrings;
    }

    protected void createNavLinksForAllOranizations(final Vector javascriptStrings, final String menuParent, final boolean useRelativePaths) {
        _itemCount = -1;
     ...
     * @param useRelativePaths
     */
    protected void createNavLinks(final Vector javascriptStrings, final Element element, String menuParent, final boolean useRelativePaths) {
        final String name = element.getName();
        // ORGANIZATION
        if (name.equals(CP_Core.ORGANIZATION) && _navViewer.isDocumentNamespace(element)) {
            ++_orgCount;
            final String orgId = element.getAttributeValue(CP_Core.IDENTIFIER);
            menuParent = "menu";
            String title = "Organization";
            // Display Title if there is one
            final Element titleElement = element.getChild(CP_Core.TITLE, element.getNamespace());
            ...
            final String _defaultOrganization = _defaultorg.getAttributeValue(CP_Core.IDENTIFIER);
        ...
        // ITEM
            ...
            ++_itemCount;
            final String itemId = element.getAttributeValue(CP_Core.IDENTIFIER);
            String hyperLink = "";
            String url = "";
            String scoType = "";
            // Display Title if there is one
            ...
            final String isVisibleAttrib = element.getAttributeValue(CP_Core.ISVISIBLE);
            ...
            if (ref_element != null) {
                final String ref_name = ref_element.getName();
                // A RESOURCE
                if (ref_name.equals(CP_Core.RESOURCE)) {
                    ...
                    // Absolute Paths for Previewing in-situ
                        ...
                        final String turl = _navViewer.getLaunch(element);
                        url = turl;
                    }
                    if (url != null) {
                        hyperLink = url;
                ...
                // A sub-MANIFEST
                    ...
                    final Element orgsElement = ref_element.getChild(CP_Core.ORGANIZATIONS, ref_element.getNamespace());
                    ...
                    // clones
                    if (ref_element != null) {
                        final Iterator it = ref_element.getChildren().iterator();
                        while (it.hasNext()) {
                            final Element ref_child = (Element) it.next();
     ...
     * @param value
     *            - the value of the item to set (String)
     */
    protected void writePackageSettings(final Vector javascriptStrings, final String name, final String value) {
     ...
     *            - the name of the item to set
     * @param value
     *            - the value of the item to set (int)
     */
    protected void writePackageSettings(final Vector javascriptStrings, final String name, final int value) {
        javascriptStrings.add("CPAPI." + name + " = " + value + ";");
    }
     ...
     */
    protected void writeOrganization(final Vector javascriptStrings, final String title, final String orgId) {
     ...
     * @param url
     *            - the launch url of this item
     * @param itemId
     *            - the item identifier
     * @param parentMenu
     *            - the tree node to attach this to.
     */
    protected void writeItem(final Vector javascriptStrings, final String title, final String url, final String itemId, String parentMenu) {
