...
public class Path {
    ...
    // Class variables
    private static final String TOKEN_PROVIDER = BaseSecurityEBL.FEED_TOKEN_PROVIDER;
    ...
    public static final String PODCAST_URI_PREFIX = FeedManager.KIND_PODCAST;
    public static final String BLOG_URI_PREFIX = FeedManager.KIND_BLOG;

    public static Hashtable<String, String> uriPrefixes;
    ...
    public static String getFeedBaseUri(final Feed feed, final Identity identity, final Long courseId, final String nodeId) {
        ...
        uri.append(slash);
        uri.append(uriPrefixes.get(feed.getResourceableTypeName()));
        uri.append(slash);
        ...
        if (identity != null) {
            ...
            Authentication authentication = getBaseSecurityEBL().findOrCreateAuthenticationWithRandomCredential(identity, TOKEN_PROVIDER);
            boolean isPrivate = isAccessibleToRegisteredUsers(feed);

            if (isPrivate) {
                // identity key
                uri.append(identity.getKey().toString());
                uri.append(slash);
                // token
     ...
     * @return
     */
    private static boolean isAccessibleToRegisteredUsers(final Feed feed) {
        // If the repository entry allows guest access it is public, thus not
        // private.
        boolean isPrivate = true;
        final RepositoryEntry entry = RepositoryServiceImpl.getInstance().lookupRepositoryEntry(feed, false);
