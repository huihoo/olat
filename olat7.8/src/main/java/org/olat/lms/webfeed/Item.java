...
public class Item implements Serializable, Dated {
    ...
    /**
     * @param authorKey
     *            The authorKey to set. Used for OLAT-made feeds.
     */
    ...
    /**
     * @param author
     *            The author to set. Used for external feeds.
     */
    ...
    public String getAuthor() {
        ...
        if (authorKey > 0) {
            final Identity identity = getBaseSecurity().loadIdentityByKey(authorKey, false);
            if (identity != null) {
                final User user = identity.getUser();
                if (user == null) {
                    authorName = author = identity.getName();
                } else {
                    authorName = author = getUserService().getFirstAndLastname(user);
                }
            }
        }
    ...
    public String getModifier() {
        ...
        if (modifierKey > 0) {
            final Identity identity = getBaseSecurity().loadIdentityByKey(modifierKey, false);
            if (identity != null) {
                final User user = identity.getUser();
                if (user == null) {
                    modifierName = modifier = identity.getName();
                } else {
                    modifierName = modifier = getUserService().getFirstAndLastname(user);
                }
            }
        }
