...
public class CourseCreationHelper {
    ...
    public void finalizeWorkflow(final UserRequest ureq) {
        ...
        CourseTitleHelper.saveCourseTitleInCourseModel(course, addedEntry.getDisplayname());

        // --------------------------
        // 3.2 publish the course
        // --------------------------
        CoursePublishHelper.publish(course, ureq.getLocale(), ureq.getIdentity(), getPublishNodeIds(course.getEditorTreeModel()));
    ...
    private List<String> getPublishNodeIds(CourseEditorTreeModel cetm) {
