...
/**
 * Description:<BR/>
 * The structure is ordered tree like hierarchical structure of course nodes. It is used in OLAT to represent the course structure.
 * <P/>
...
public class Structure implements Serializable {
    ...
    /**
     * @param node
     *            The node that should be the new root node
     */
    public void setRootNode(final CourseNode node) {
        rootNode = node;
    }
     ...
     */
    public CourseNode getNode(final String nodeId) {
        return findNode(rootNode, nodeId);
    }

    /**
     * Searches for a course node with the given node id starting at the given course node
     * 
     * @param node
     * @param nodeId
     * @return The found course node or null if no such node exists
     */
    private CourseNode findNode(final CourseNode node, final String nodeId) {
        if (node.getIdent().equals(nodeId)) {
            return node;
        }
        final int childcnt = node.getChildCount();
        for (int i = 0; i < childcnt; i++) {
            final CourseNode element = (CourseNode) node.getChildAt(i);
            final CourseNode foo = findNode(element, nodeId);
            if (foo != null) {
                return foo;
            }
        }
