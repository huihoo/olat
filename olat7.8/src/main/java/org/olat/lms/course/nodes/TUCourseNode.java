...
import org.olat.presentation.course.nodes.tu.TUEditController;
import org.olat.presentation.course.nodes.tu.TURunController;
 ...
 */
public class TUCourseNode extends AbstractAccessableCourseNode implements UsedByXstream {
    ...
    public TabbableController createEditController(final UserRequest ureq, final WindowControl wControl, final ICourse course, final UserCourseEnvironment euce) {
        ...
        final TUEditController childTabCntrllr = new TUEditController(getModuleConfiguration(), ureq, wControl, this, course, euce);
    ...
    public StatusDescription isConfigValid() {
        ...
        final String host = (String) getModuleConfiguration().get(TUConfigForm.CONFIGKEY_HOST);
        final boolean isValid = host != null;
        StatusDescription sd = StatusDescription.NOERROR;
        if (!isValid) {
            // FIXME: refine statusdescriptions
            final String[] params = new String[] { this.getShortTitle() };
            final String translPackage = PackageUtil.getPackageName(TUConfigForm.class);
    ...
    public StatusDescription[] isConfigValid(final CourseEditorEnv cev) {
        ...
        // error messages
        final String translatorStr = PackageUtil.getPackageName(TUEditController.class);
