 ...
 */
@Component
public class TopicCandidateGroupConfirmationEmailBuilder extends ConfirmationEmailBuilder<TopicCandidateGroupConfirmationInfo> {
    ...
    @Override
    protected String getSubjectForConfirmation(Locale recipientsLocale, TopicCandidateGroupConfirmationInfo confirmationInfo) {
        String[] subjectVariables = new String[] { confirmationInfo.getTopicName() };
        String subjectTranslationKey = "";
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_TO_CANDIDATES.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.add.topic.candidate";
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_FROM_CANDIDATES.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.remove.topic.candidate";
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.MOVE_USER_FROM_CANDIDATES.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.move.topic.candidate";
        } else {
     ...
     */
    @Override
    protected ConfirmationMailBody getMailBody(Locale recipientsLocale, TopicCandidateGroupConfirmationInfo confirmationInfo) {
        String content;
        String greeting;
        String greetingFrom;
        String footer;
        ...
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_TO_CANDIDATES.equals(confirmationInfo.getGroupConfirmationType())) {
            content = translator.translate("mail.body.confirmation.add.topic.candidate", variables);
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_FROM_CANDIDATES.equals(confirmationInfo.getGroupConfirmationType())) {
            content = translator.translate("mail.body.confirmation.remove.topic.candidate", variables);
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.MOVE_USER_FROM_CANDIDATES.equals(confirmationInfo.getGroupConfirmationType())) {
            content = translator.translate("mail.body.confirmation.move.topic.candidate", variables);
        } else {
