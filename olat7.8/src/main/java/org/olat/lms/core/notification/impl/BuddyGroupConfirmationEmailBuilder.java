 ...
 */
@Component
public class BuddyGroupConfirmationEmailBuilder extends ConfirmationEmailBuilder<BuddyGroupConfirmationInfo> {
    ...
    @Override
    protected String getSubjectForConfirmation(Locale recipientsLocale, BuddyGroupConfirmationInfo confirmationInfo) {
        String[] subjectVariables = new String[] { confirmationInfo.getGroupName() };
        String subjectTranslationKey = "";
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_BUDDY_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.add.to.group";
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_BUDDY_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
     ...
     */
    @Override
    protected ConfirmationMailBody getMailBody(Locale recipientsLocale, BuddyGroupConfirmationInfo confirmationInfo) {
        String content;
        String greeting;
        String greetingFrom;
        String footer;
        ...
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_BUDDY_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
            content = translator.translate("mail.body.confirmation.add.to.buddy.group", variables);
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_BUDDY_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
            variables[3] = getQuotedString(confirmationInfo.getGroupName());
            content = translator.translate("mail.body.confirmation.remove.from.buddy.group", variables);
        } else {
