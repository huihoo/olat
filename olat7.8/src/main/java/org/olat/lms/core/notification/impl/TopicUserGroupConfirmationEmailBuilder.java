 ...
 */
@Component
public class TopicUserGroupConfirmationEmailBuilder extends ConfirmationEmailBuilder<TopicUserGroupConfirmationInfo> {

    @Override
    public CONFIRMATION_TYPE getConfirmationInfoType() {
        return CONFIRMATION_TYPE.TOPIC_USERS;
    }

    @Override
    protected String getSubjectForConfirmation(Locale recipientsLocale, TopicUserGroupConfirmationInfo confirmationInfo) {
        String[] subjectVariables = new String[] { confirmationInfo.getTopicName() };
        String subjectTranslationKey = "";
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_TO_TOPIC.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.add.topic.user";
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_FROM_TOPIC.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.remove.topic.user";
        } else {
     ...
     */
    @Override
    protected ConfirmationMailBody getMailBody(Locale recipientsLocale, TopicUserGroupConfirmationInfo confirmationInfo) {
        String content;
        String greeting;
        String greetingFrom;
        String footer;
        ...
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_TO_TOPIC.equals(confirmationInfo.getGroupConfirmationType())) {
            content = translator.translate("mail.body.confirmation.add.topic.user", variables);
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_FROM_TOPIC.equals(confirmationInfo.getGroupConfirmationType())) {
            content = translator.translate("mail.body.confirmation.remove.topic.user", variables);
        } else {
