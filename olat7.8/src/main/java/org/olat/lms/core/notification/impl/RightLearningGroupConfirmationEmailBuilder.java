 ...
 */
@Component
public class RightLearningGroupConfirmationEmailBuilder extends ConfirmationEmailBuilder<RightLearningGroupConfirmationInfo> {
    ...
    @Override
    protected String getSubjectForConfirmation(Locale recipientsLocale, RightLearningGroupConfirmationInfo confirmationInfo) {
        String[] subjectVariables = new String[] { confirmationInfo.getGroupName() };
        String subjectTranslationKey = "";
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_RIGHT_LEARNING_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.add.to.group";
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_RIGHT_LEARNING_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
            subjectTranslationKey = "mail.subject.confirmation.remove.from.group";
        } else {
     ...
     */
    @Override
    protected ConfirmationMailBody getMailBody(Locale recipientsLocale, RightLearningGroupConfirmationInfo confirmationInfo) {
        String content;
        String greeting;
        String greetingFrom;
        String footer;
        ...
        if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.ADD_USER_RIGHT_LEARNING_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
            ...
            content = translator.translate("mail.body.confirmation.add.to.group", variables);
        } else if (AbstractGroupConfirmationInfo.GROUP_CONFIRMATION_TYPE.REMOVE_USER_RIGHT_LEARNING_GROUP.equals(confirmationInfo.getGroupConfirmationType())) {
            variables[3] = getQuotedString(confirmationInfo.getGroupName());
            content = translator.translate("mail.body.confirmation.remove.from.group", variables);
        } else {
