...
/**
 * Description:<BR>
 * Objects of this class contain configuration flags for the various business group controllers. See the BGControllerFactory to see which group type has which
 * configuration.
 * <P>
...
public class BGConfigFlags {
    ...
    /**
     * true if the current admin user is allowed to see all userdata. If set to true the user can change the table columns of the user tables and for example see the
     * users institutional id. If set to false, only the name, firstname, login and email is revealed
     */
    ...
    public boolean isEnabled(final String flag) {
        final Boolean result = flags.get(flag);
        if (result == null) {
            return false;
        }
    ...
    public static BGConfigFlags createBuddyGroupDefaultFlags() {
        final BGConfigFlags bgFlags = new BGConfigFlags();
