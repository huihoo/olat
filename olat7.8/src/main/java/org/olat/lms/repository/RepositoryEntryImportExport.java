...
public class RepositoryEntryImportExport {
    ...
    private final boolean propertiesLoaded = false;
     ...
     * @param baseDirectory
     */
    public RepositoryEntryImportExport(final RepositoryEntry re, final File baseDirectory) {
        this.re = re;
     ...
     * @param baseDirecotry
     */
    public RepositoryEntryImportExport(final File baseDirecotry) {
    ...
    /**
     * Export metadata of a repository entry to a file. Only one repository entry's metadata may be exported into a directory. The file name of the properties file will
     * be the same for all repository entries!
     */
    public void exportDoExportProperties() {
        final Dom4jConfiguration root = new Dom4jConfiguration(PROP_ROOT);
    ...
    public boolean exportDoExportContent() {
        // export resource
        final RepositoryHandler rh = RepositoryHandlerFactory.getInstance().getRepositoryHandler(re);
        final MediaResource mr = rh.getAsMediaResource(re.getOlatResource());
        FileOutputStream fOut = null;
        try {
            // TODO:MK
    ...
    /**
     * Returns the exported repository file. <br/>
     * Used at import.
     * 
     * 
    ...
    public String getSoftkey() {
        return getProperty(PROP_SOFTKEY);
    }
    ...
    public String getDisplayName() {
        return getProperty(PROP_DISPLAYNAME);
    }
    ...
    public String getResourceName() {
        return getProperty(PROP_RESOURCENAME);
    }
    ...
    public String getDescription() {
        return getProperty(PROP_DECRIPTION);
    }
    ...
    public String getInitialAuthor() {
        return getProperty(PROP_INITIALAUTHOR);
    }
