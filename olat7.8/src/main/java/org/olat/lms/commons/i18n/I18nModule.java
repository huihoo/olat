...
/**
 * <h3>Description:</h3> The I18nModule initializes the localization infrastructure. It offers configuration options to define the default language, the available and
 * enabled languages etc.
 * <p>
...
public class I18nModule extends AbstractOLATModule implements Destroyable {
    ...
    private static final String APPLICATION_FALLBACK_BUNDLE_NAME = "org.olat.presentation";
    ...
    private static String TESTDATA_BUNDLE = "org.olat.lms.commons.i18n.junittestdata";
    ...
    private I18nModule() {
        super();
        if (INSTANCE != null) {
            throw new OLATRuntimeException("Tried to construct I18nModule, but module was already loaded!", null);
        }
        INSTANCE = this;
    }
    ...
    protected void initDefaultProperties() {
        ...
        // Make translation server stuff work in 7.3.x
        // Slow OLATNG hypothese: translatableLanguages.add("en");
        // Make translation server stuff work in 7.3.x
    }
    ...
    private void doInit() {
        ...
        if (transToolEnabled) {
            ...
            if (!transToolEnabled) {
                log.warn(
                        ...
                        null);
                log.warn(" transToolApplicationSrcPath::" + appSrc + " transToolApplicationI18nSrcPath::" + optAppSrc, null);
            }
        }
        ...
        initBundleNames();
        // Set the base bundles for the application. When a key is
        // not found, the manager looks it up in the application base before it gives up
        applicationFallbackBundle = getStringConfigParameter(CONFIG_APPLICATION_FALLBACK_BUNDLE, APPLICATION_FALLBACK_BUNDLE_NAME, false);
        ...
        log.info("Configured i18nModule with default language::" + getDefaultLocale().toString() + " and the reference languages '" + referenceLanguagesConfig
                + "' and the following enabled languages: " + enabledLanguagesKeys.toString(), null);
    }
    ...
    private void doInitAvailableLanguages() {
        ...
        for (String languageCode : i18nMgr.searchForAvailableLanguages(libDir)) {
            if (availableLanguages.contains(languageCode)) {
                log.warn("Skipping duplicate or previously loaded  language::" + languageCode + " found in " + libDir.getAbsolutePath(), null);
                continue;
            }
            ...
            // Make translation server stuff work in 7.3.x
            // Slow OLATNG hypothese: translatableLangAppBaseDirLookup.put(languageCode, libDir);
            // Slow OLATNG hypothese: translatableLanguages.add(languageCode);
            // Make translation server stuff work in 7.3.x
        ...
        for (String languageCode : i18nMgr.searchForAvailableLanguages(LANG_PACKS_DIRECTORY)) {
            if (availableLanguages.contains(languageCode)) {
                log.warn("Skipping duplicate or previously loaded  language::" + languageCode + " found in " + LANG_PACKS_DIRECTORY.getAbsolutePath(), null);
                continue;
            }
        ...
        if (availableLanguages.size() == 0 || !availableLanguages.contains(Locale.ENGLISH.toString())) {
            throw new OLATRuntimeException("Did not find any language files, not even 'en'! At least 'en' must be available.", null);
        }
        ...
        for (String langKey : availableLanguages) {
            ...
            if (locale == null) {
                log.error("Could not create locale for lang::" + langKey + ", skipping language and remove it from list of available languages", null);
    ...
    private void doInitLanguageConfiguration() {
        ...
        if (newDefaultLocale == null) {
            log.error("Could not set default locale to value::" + defaultLanguageKey + " - no such language found. Using fallback locale instead", null);
            ...
            log.error("Did not find the default language::" + newDefaultLocale.toString() + " in the available availableLanguages files! Using fallback locale instead",
                    null);
            newDefaultLocale = allLocales.get(transToolReferenceLanguages.get(0));
        }
        ...
        if (!enabledLanguagesKeys.contains(getDefaultLocale().toString())) {
            ...
            log.warn("The configured default language::" + defLang + " is not in the list of enabled languages. Enabling language::" + defLang, null);
        }
    }
    ...
    /**
     * @return The locale that is used when a string is not found in any other locale
     */
    ...
    /**
     * @return as keys: a Set of Strings with the supported languages (e.g. de, de_CH, en, ...). those are the languages that are installed. See the enabled languages to
     *         get the list of languages that are enabled to be used
     */
    ...
    /**
     * @return A set of language keys that can be translated by the translation tool. Theses are the languages that are available in the source form. Languages embedded
     *         in jars can't be edited.
     */
    ...
    /**
     * @return the map (with dummy value) of all languages including overlayLocales (as a String)
     */
    ...
    /**
     * @return The lookup map of the overlay locales. Key: the locale; value: the corresponding overlay
     */
    ...
    /**
     * @return as keys: a List of Strings with the supported languages (e.g. de, de_CH, en, ...). those are the languages which can be chosen by the user
     */
    ...
    /**
     * @return The bundle name that contains the commonly used translations from the application
     */
    ...
    /**
     * @return true: enable the translation tool; false: disable the translation tool
     */
    ...
    /**
     * search for bundles that contain i18n files. Searches in the org.olat package
     */
    ...
    public static File getPropertyFilesBaseDir(Locale locale, String bundleName) {
        ...
        if (Settings.isJUnitTest() && bundleName.startsWith(TESTDATA_BUNDLE)) {
            return new File(transToolApplicationLanguagesDir, "/../../test/java");
        }
        // 2) Metadata file: metadata file is always on the application
        // source path
        if (locale == null) {
            return transToolApplicationLanguagesDir;
        }
        // 3) Locale files from application
    ...
    /**
     * Private helper that implements the reinitialization of all caches. This is decoupled from the reInitialize() to prevent endless firing of events in the cluster.
     */
    ...
    /**
     * Get the language dir for the applications DE and EN files. Only available when in translation server mode
     * 
     * @return
     */
    ...
    /**
     * Get the language dir for the applications i18n different than DE and EN files. Only available when in translation server mode
     * 
     * @return
     */
