...
public class Render_choice extends GenericQTIElement {
     ...
     * @param el_element
     */
    public Render_choice(final Element el_element) {
        super(el_element);

        // fetch shuffle
        final String sShuffle = el_element.attributeValue("shuffle");
        if (sShuffle != null) {
            if (sShuffle.equals("Yes")) {
                shuffle = true;
            } else if (sShuffle.equals("No")) {
                shuffle = false;
            } else {
                throw new AssertException("Attribute shuffle has invalid value.");
            }
        }
        ...
        final List<?> ident = el_element.selectNodes("../../../@ident");
        if (ident != null && ident.size() == 1) {
            try {
                if (((AbstractAttribute) ident.get(0)).getValue().startsWith(ItemParser.ITEM_PREFIX_KPRIM)) {
                    kprim = true;
                }
    ...
    public void render(final StringBuilder buffer, final RenderInstructions ri) {
        ...
        /*
         * // Display min/max values if (minnumber != -1 || maxnumber != -1) { Translator translator = new PackageTranslator("org.olat.lms.ims.qti",
         * (Locale)ri.get(RenderInstructions.KEY_LOCALE)); buffer.append("<p class=\"o_qti_minmax\">"); if (minnumber != -1) {
         * buffer.append("<i>").append(translator.translate("render.item.minanswers")).append("&nbsp;") .append(minnumber).append("</i><br />"); } if (maxnumber != -1) {
         * buffer.append("<i>").append(translator.translate("render.item.maxanswers")).append("&nbsp;") .append(maxnumber).append("</i>"); } buffer.append("</p>"); }
         */
