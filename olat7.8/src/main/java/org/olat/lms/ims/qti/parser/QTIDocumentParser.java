...
public class QTIDocumentParser implements IParser {
    ...
    public Object parse(final Element element) {
        ...
        final QTIDocument qtiDocument = new QTIDocument();

        Object tmp = null;
        final Element qticomment = element.element("qticomment");
        qtiDocument.setQticomment((QTIObject) parserManager.parse(qticomment));

        final Element objectbank = element.element("objectbank");
        tmp = parserManager.parse(objectbank);
        if (tmp != null) {
            qtiDocument.setObjectbank((QTIObject) tmp);
        }
        final Element assesment = element.element("assessment");
