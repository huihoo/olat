...
/**
 * @author Potable Shop restrictions on items so far: - only one resprocessing element is evaluated. duration: from ims qti 1.2.1 best practice: duration The duration
 *         element is used within the item, section and assessment elements to define the permitted duration for the enclosed activity. The duration is defined as the
 *         period between the activity's activation and completion. The ISO 8601 format is used: PnYnMnDTnHnMnS. in mchc_smimr_104.xml example: 0000:00:00T:01:00:00d in
 *         ...101.xml: pTH02 iso website: http://www.iso.org/iso/en/prods-services/popstds/datesandtime.html and http://www.w3.org/TR/NOTE-datetime from some page:
 *         specified in numbers of years [Y], months [M] or weeks [W], days [D], hours [H] and so on, introduced by a ?P?, for example: P6W2D -> not a date, but only a
 *         duration makes sense: -> I shall take the ISO 8601 Time of the day hh:mm:ss format for now
 */
public class ItemContext implements Serializable {
    ...
    private boolean feedback = false; // flags
    ...
    private Element shuffle(Element item) {
        ...
        List<Element> respList = new ArrayList<Element>(shusize);
        List<Element> parentList = new ArrayList<Element>(shusize);
     ...
     * 
     * @param theItemInput
     *            The itemInput to set. max be null for "unanswered"
     * @return the status of the add operation, e.g. ok
     */
    public int addItemInput(final ItemInput theItemInput) {
        final boolean underMax = isUnderMaxAttempts();
        final boolean onTime = isOnTime();
        if (underMax && onTime) {
            start();
            timesAnswered++;
    ...
    public boolean isOpen() {
        ...
        final boolean ok = isOnTime() && isUnderMaxAttempts();
        return ok;
    }
    ...
    public void eval() {
        ...
        if (evalNeeded) {
            final QTI_item qtiItem = QTIHelper.getQtiItem();
    ...
    public float getMaxScore() {
        final Variable var = getVariables().getSCOREVariable();
    ...
    public float getScore() {
        ...
        if (var == null) {
            ...
            final float sc = var.getTruncatedValue();
            return sc;
        }
    }
    ...
    public void resetVariables() {
        final Element el_outcomes = (Element) getEl_item().selectSingleNode("resprocessing/outcomes");
     ...
     * 
     * @param hintLevel
     *            The hintLevel to set
     */
    public void setHintLevel(final int hintLevel) {
