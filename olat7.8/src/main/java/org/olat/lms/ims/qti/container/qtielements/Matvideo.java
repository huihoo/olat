...
public class Matvideo extends GenericQTIElement {
     ...
     * @param el_matimage
     */
    public Matvideo(final Element el_matimage) {
        super(el_matimage);
    ...
    public void render(final StringBuilder buffer, final RenderInstructions ri) {
        ...
        buffer.append("<object width=\"").append(width).append("\"").append(" height=\"").append(height).append("\" ");
        ...
        if (videotype.equals("application/x-shockwave-flash")) {
            ...
            // quicktime
            buffer.append("classid=\"clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B\" codebase=\"http://www.apple.com/qtactivex/qtplugin.cab\">").append(
            ...
            buffer.append("\"><param name=\"controller\" value=\"true\"><param name=\"autoplay\" value=\"true\">")
                    .append("<embed  type=\"video/quicktime\" pluginspage=\"http://www.apple.com/quicktime/download/\" width=\"").append(width).append("\" height=\"")
                    .append(height).append("\" src=\"");
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                buffer.append(uri);
            } else {
                buffer.append((String) ri.get(RenderInstructions.KEY_STATICS_PATH)).append(uri);
            ...
            buffer.append("<embed type=\"application/vnd.rn-realmedia\"").append(" width=\"").append(width).append("\" height=\"").append(height).append("\" src=\"");
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                buffer.append(uri);
            } else {
                buffer.append((String) ri.get(RenderInstructions.KEY_STATICS_PATH)).append(uri);
            ...
            buffer.append("<embed ").append(" width=\"").append(width).append("\" height=\"").append(height).append("\" src=\"");
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                buffer.append(uri);
            } else {
                buffer.append((String) ri.get(RenderInstructions.KEY_STATICS_PATH)).append(uri);
