 ...
 * 
 * @author gnaegi Comment: Representatio of an essay question type. An essay consists of a question (material) and a response (response_str) element. The essay question
 *         type has no respconditions at all, it can only be used in the context of a survey. </pre>
 */
public class EssayQuestion extends Question implements QTIObject {
     ...
     * @return
     */
    public static EssayQuestion getInstance(final Element item) {

        final EssayQuestion instance = new EssayQuestion();

        // Parsing presentation element
        final Element presentationXML = item.element("presentation");

        // Question
        final Element material = (Element) presentationXML.selectSingleNode(".//material"); // export uses flow
        ...
        final String ident = response_str.attribute("ident").getValue();
        response.setIdent(ident);

        final Element render_fib = response_str.element("render_fib");
    ...
    public void addToElement(final Element root) {
        ...
        final EssayResponse response = this.getEssayResponse();

        final Element response_str = presentationXML.addElement("response_str");
        ...
        final Element response_label = render_fib.addElement("response_label");
    ...
    public EssayResponse getEssayResponse() {
        ...
        for (final Iterator iter = getResponses().iterator(); iter.hasNext();) {
