...
public interface BaseSecurity {
    ...
    /**
     * @param secGroup
     * @return list of Identities in this security group (NOT sorted by a certain attribute).
     */
    public List<Identity> getIdentitiesOfSecurityGroup(SecurityGroup secGroup);

    /**
     * @param secGroup
     * @return list of Identities in this security group (sorted by date added to it).
     */
     ...
     * 
     * @param identityKey
     *            the key of the identity to load; may not be null or zero
    ...
    /**
     * Retrieves all expired invitations created earlier than <code>limitDate</code>
     */
     ...
     *            Map of user properties that needs to be matched.
     * @param userPropertiesAsIntersectionSearch
     *            true: user properties and login name are combined with an AND query; false: user properties and login name are combined with an OR query
     * @param groups
     *            Array of SecurityGroups the user participates in. Search machtches if user is in any of the groups (OR query)
     * @param permissionOnResources
     *            Array of resource permissions the user has. Search machtches if user has any of the permissions (OR query)
     * @param authProviders
     *            Array of authenticaton providers the user has. Search machtches if user has any of the authProviders (OR query)
     * @param createdAfter
     *            date after which the user has been created
     * @param createdBefore
     *            date before which the user has been created
     ...
     *            date before the user has logged in the last time
     * @param userLoginAfter
     *            date after the user has logged in the last time
     * @param status
     *            identity status, define in interface Identity e.g. ACTIV, LOGIN_DENIED, DELETED
     * @return List of identities
     */
    ...
    /**
     * @return ID list of all active identities
     */
