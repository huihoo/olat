...
public class MergeSource extends AbstractVirtualContainer {
     ...
     * @param container
     * @param enableWrite
     *            If true, writes to the root of this MergeSource are directed to this container.
     */
    ...
    public List getItems(VFSItemFilter filter) {
        ...
        if (filter == null && defaultFilter == null) {
            ...
            for (Iterator iter = mergedContainers.iterator(); iter.hasNext();) {
                ...
                if (defaultFilter != null && !defaultFilter.accept(mergedContainer))
                    passedFilter = false;
                // check for custom filter
                if (passedFilter && filter != null && !filter.accept(mergedContainer))
                    passedFilter = false;
                // only add when both filters passed the test
                if (passedFilter)
                    all.add(mergedContainer);
            }
        }
