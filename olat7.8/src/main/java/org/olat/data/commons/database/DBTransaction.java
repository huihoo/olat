...
final class DBTransaction {
    ...
    protected void setErrorAndRollback(Exception exception) {
        error = exception;
        log.debug("Hibernate Error , rolling back");
        try {
            rollback();
        } catch (DBRuntimeException e) {
            // in case of error it's not guaranteed that rollback works properly => report but don't throw this error
            log.warn("Transaction rollback failed after error.", e);
        }
    }
