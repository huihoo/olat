...
package org.olat.data.commons.filter;

import org.olat.data.commons.filter.impl.AddBaseURLToMediaRelativeURLFilter;
import org.olat.data.commons.filter.impl.ConditionalHTMLCommentsFilter;
import org.olat.data.commons.filter.impl.NekoHTMLFilter;
import org.olat.data.commons.filter.impl.OWASPAntiSamyXSSFilter;
import org.olat.data.commons.filter.impl.SimpleHTMLTagsFilter;
import org.olat.data.commons.filter.impl.XMLValidCharacterFilter;
...
public class FilterFactory {
    ...
    /**
     * Unescapes the original string and filters all HTML tags out. <br>
     * Is not the same as calling <code>getHtmlTagAndDescapingFilter().filter()</code>.
     */
    public static String unescapeAndFilterHtml(String str) {
        Filter filter = getHtmlTagsFilter();
        String unescaped = StringHelper.unescapeHtml(str);
        return filter.filter(unescaped);
    }
     ...
     * 
     * @param set
     *            the maximum length allowed by the xss filter, -1 take the default value from the policy file
     * @return
     */
     ...
     * <p>
     * The reason why this method is not into <code>StringHelper</code> is that the <code>org.olat.data.commons.filter</code> package depends on
     * <code>org.olat.data.commons.vfs.VFSManager</code>.
     */
    public static final String filterXSS(String str) {
