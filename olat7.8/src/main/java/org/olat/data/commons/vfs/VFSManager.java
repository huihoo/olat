...
public class VFSManager extends BasicManager {
    ...
    public static boolean isSelfOrParent(VFSContainer descendant, VFSContainer parent) {
        if (parent.isSame(descendant))
            return true;
        VFSContainer parentContainer = descendant.getParentContainer();
        if (parentContainer != null && parentContainer.isSame(parent))
            return true;
    ...
    public static VFSItem resolveFile(VFSContainer rootContainer, String path) {
        ...
        // On unix-like systems, just put together the paths of both arguments
        ...
        if (File.separator.equals("/") && rootContainer instanceof LocalFolderImpl) {

            String bcroot = FolderConfig.getCanonicalRoot();
            LocalFolderImpl l = (LocalFolderImpl) rootContainer;
            String base = l.getBasefile().getAbsolutePath();
            String fsPath = base + path;
            ...
            boolean isBCroot = base.startsWith(bcroot);
            if (isBCroot) {
                fsPath = fsPath.replace(bcroot, "");
            }

            // create a properly chained parent for the VFSItem to be returned
            VFSContainer rvParent = rootContainer;
            String s[] = path.split("/");
            for (int i = 1; s.length > 1 && i < s.length; i++) {
                File sf = new File(base + "/" + s[i]);
                if (sf.isDirectory()) {
                    if (isBCroot) {
                        rvParent = new OlatRootFolderImpl(sf.getAbsolutePath().replace(bcroot, ""), rvParent);
                    } else {
                        rvParent = new LocalFolderImpl(sf, rvParent);
                    }
                    base += "/" + s[i];
                }
            }
            if (t.isDirectory()) {
                if (isBCroot) {
                    return new OlatRootFolderImpl(fsPath, rvParent);
                } else {
                    return new LocalFolderImpl(t, rvParent);
                }
            } else {
                if (isBCroot) {
                    return new OlatRootFileImpl(fsPath, rvParent);
                } else {
                    return new LocalFileImpl(t, rvParent);
                }
            }
        }
    ...
    public static VFSSecurityCallback findInheritedSecurityCallback(VFSItem vfsItem) {
        ...
        if (inheritingItem != null)
            return inheritingItem.getLocalSecurityCallback();
        return null;
    }
    ...
    public static VFSContainer findInheritingSecurityCallbackContainer(VFSItem vfsItem) {
        ...
        if (vfsItem instanceof NamedContainerImpl)
            return findInheritingSecurityCallbackContainer(((NamedContainerImpl) vfsItem).delegate);
    ...
    public static Quota isTopLevelQuotaContainer(VFSContainer container) {
        ...
        if (callback != null && callback.getQuota() != null)
            return callback.getQuota();
        ...
        if (container instanceof MergeSource) {
            ...
            if (rwContainer != null && rwContainer.getLocalSecurityCallback() != null && rwContainer.getLocalSecurityCallback().getQuota() != null)
    ...
    public static long getQuotaLeftKB(VFSContainer container) {
        ...
        long quotaKB = inheritingItem.getLocalSecurityCallback().getQuota().getQuotaKB().longValue();
        long usageKB = getUsageKB(inheritingItem);
        long quotaRemainingKB = quotaKB - usageKB;
        if (quotaRemainingKB < 0) {
            log.warn("Quota exceeded on folder: " + container + " [quota=" + quotaKB + "KB, used=" + usageKB + "KB].");
        }
        return quotaRemainingKB;
    }
    ...
    public static long getUsageKB(VFSItem vfsItem) {
        // TODO enable/disable completely in QuotaManager
     ...
     * 
     * @param rootDir
     *            the container that should be checked
     * @param relFilePath
     *            The valid file path within this container
     ...
     * @param target
     * @param closeInput
     *            set to false if it's a ZipInputStream
     * @return True on success, false on failure
     */
