...
public class NodeInfo implements Serializable {
     ...
     */
    public synchronized void update(final ClusterInfoEvent cie) {// cluster_ok is per vm only
     ...
     */
    public synchronized boolean update(final JMSWrapper jmsWrapper) {// cluster_ok is per vm only
        numOfReceivedMessages++;
        final long currentId = jmsWrapper.getMsgId();
        final long expected = latestReceivedMsgId + 1;
