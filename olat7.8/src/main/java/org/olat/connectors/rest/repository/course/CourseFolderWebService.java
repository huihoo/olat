...
public class CourseFolderWebService {
    ...
    public Response attachFileToFolderPost(@PathParam("courseId") final Long courseId, @PathParam("nodeId") final String nodeId,
    ...
    @Path("files")
    @Consumes(MediaType.MULTIPART_FORM_DATA)
    public Response attachFileToFolder(@PathParam("courseId") final Long courseId, @PathParam("nodeId") final String nodeId,
            @FormParam("filename") final String filename, @FormParam("file") final InputStream file, @Context final HttpServletRequest request) {
        if (!isAuthor(request)) {
        ...
        final CourseNode node = course.getEditorTreeModel().getCourseNode(nodeId);
        if (node == null) {
            return Response.serverError().status(Status.NOT_FOUND).build();
        ...
        final BCCourseNode bcNode = (BCCourseNode) node;
        final OlatNamedContainerImpl container = BCCourseNode.getNodeFolderContainer(bcNode, course.getCourseEnvironment());
        if (container.resolve(filename) != null) {
            // already exist
        } else {
            final VFSLeaf newFile = container.createChildLeaf(filename);
            final OutputStream out = newFile.getOutputStream(false);
