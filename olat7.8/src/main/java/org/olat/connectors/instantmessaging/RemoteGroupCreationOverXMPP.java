...
public class RemoteGroupCreationOverXMPP implements InstantMessagingGroupSynchronisation {
	 ...
	 */
    @Override
    public boolean addUserToSharedGroup(final String groupId, final String username) {
        final boolean statusOk = sendPacket(new AddUserToGroup(username, groupId));
        if (!statusOk) {
	 ...
	 */
    @Override
    public boolean addUsersToSharedGroup(final String groupId, final List<String> users) {
        boolean statusOk = true;
        for (final Iterator<String> iterator = users.iterator(); iterator.hasNext();) {
	 ...
	 */
    @Override
    public boolean createSharedGroup(final String groupId, final String displayName, final List<String> initialMembers) {
        ...
        if (initialMembers != null) {
            int cnt = 0;
            for (final Iterator<String> iterator = initialMembers.iterator(); iterator.hasNext();) {
                final String username = iterator.next();
                if (!sendPacket(new AddUserToGroup(username, groupId))) {
                    log.error("adding user to shard group failed. group: " + groupId + " and user: " + username);
                    statusOk = false;
    ...
    @Override
    public boolean createSharedGroup(final String groupId, final String displayName) {
	 ...
	 */
    @Override
    public boolean deleteSharedGroup(final String groupId) {
        final boolean statusOk = sendPacket(new GroupDelete(groupId));
        if (!statusOk) {
	 ...
	 */
    @Override
    public boolean removeUserFromSharedGroup(final String groupId, final String username) {
        final boolean statusOk = sendPacket(new RemoveUserFromGroup(username, groupId));
        if (!statusOk) {
	 ...
	 */
    @Override
    public boolean removeUsersFromSharedGroup(final String groupId, final String[] users) {
	 ...
	 */
    @Override
    public boolean renameSharedGroup(final String groupId, final String displayName) {
