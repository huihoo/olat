 ...
 * also used for chat/groupchat stuff. With this class we use the IM server to count all users clusterwide.
 * <P>
...
public class SessionCount extends IQ {
    ...
    public static class Provider implements IQProvider {
        ...
        public IQ parseIQ(final XmlPullParser parser) throws Exception {
            ...
            final SessionCount sCount = new SessionCount();
            boolean done = false;
            while (!done) {
                final int eventType = parser.next();
                if (eventType == XmlPullParser.START_TAG) {
                    if (parser.getName().equals("sessioncount")) {
                        final String sessions = parser.nextText();
    ...
    void setNumberOfSessions(final int numberOfSessions) {
