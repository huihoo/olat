...
package org.olat.connectors.webdav.util;
...
public class VFSDirContext extends BaseDirContext {
     ...
     *                if the specified value is not supported by this implementation
     */
    ...
    public Object lookup(String name) throws NamingException {
        ...
        if (item == null)
            throw new NamingException(smgr.getString("resources.notFound", name));
    ...
    public boolean canCopy(String name) {
        VFSItem item = resolveFile(name);
        if (item != null && VFSConstants.YES.equals(item.canCopy()))
            return true;
        else
            return false;
    }
    ...
    public boolean canDelete(String name) {
        ...
        if (item != null && VFSConstants.YES.equals(item.canDelete())) {
            return !FileMetadataInfoHelper.isLocked(item, userSession);
        } else
            return false;
    }
     ...
     * <p>
     * This method is idempotent. It succeeds even if the terminal atomic name is not bound in the target context, but throws NameNotFoundException if any of the
     ...
     * <p>
     * This method is idempotent. It succeeds even if the terminal atomic name is not bound in the target context, but throws NameNotFoundException if any of the
     * intermediate contexts do not exist. In a federated naming system, a context from one naming system may be bound to a name in another. One can subsequently look up
     * and perform operations on the foreign context using a composite name. However, an attempt destroy the context using this composite name will fail with
     * NotContextException, because the foreign context is not a "subcontext" of the context in which it is bound. Instead, use unbind() to remove the binding of the
     * foreign context. Destroying the foreign context requires that the destroySubcontext() be performed on a context from the foreign context's "native" naming system.
     * 
     * @param name
    ...
    /**
     * Retrieves the named object, following links except for the terminal atomic component of the name. If the object bound to name is not a link, returns the object
     * itself.
     * 
     * @param name
     *            the name of the object to look up
     ...
     * <p>
     * Many naming services have a notion of a "full name" for objects in their respective namespaces. For example, an LDAP entry has a distinguished name, and a DNS
     * record has a fully qualified name. This method allows the client application to retrieve this name. The string returned by this method is not a JNDI composite name
     * and should not be passed directly to context methods. In naming systems for which the notion of full name does not make sense, OperationNotSupportedException is
     * thrown.
     * 
    ...
    /**
     * Retrieves selected attributes associated with a named object. See the class description regarding attribute models, attribute type names, and operational
     * attributes.
     * 
     ...
     * @param attrIds
     *            the identifiers of the attributes to retrieve. null indicates that all attributes should be retrieved; an empty array indicates that none should be
     *            retrieved
     * @exception NamingException
     *                if a naming exception is encountered
     */
    ...
    /**
     * Modifies the attributes associated with a named object. The order of the modifications is not specified. Where possible, the modifications are performed
     * atomically.
     * 
     * @param name
     *            the name of the object whose attributes will be updated
     * @param mod_op
     *            the modification operation, one of: ADD_ATTRIBUTE, REPLACE_ATTRIBUTE, REMOVE_ATTRIBUTE
     * @param attrs
    ...
    /**
     * Modifies the attributes associated with a named object using an an ordered list of modifications. The modifications are performed in the order specified. Each
     * modification specifies a modification operation code and an attribute on which to operate. Where possible, the modifications are performed atomically.
     * 
     * @param name
     *            the name of the object whose attributes will be updated
     * @param mods
     ...
     *                if some "mandatory" attributes of the binding are not supplied
     * @exception NamingException
     *                if a naming exception is encountered
     */
    @Override
    public void bind(String name, Object obj, Attributes attrs) throws NamingException {
        ...
        // TODO: OLAT-1011: No support of subscription via WebDAV => Remove this code at the end of OLAT-1011
        // VFSSecurityCallback callback = folder.getLocalSecurityCallback();
        // Subscribed subscribed = (Subscribed) callback;
        // if (callback != null && subscribed.getSubscriptionContext() != null) {
        // SubscriptionContext subContext = subscribed.getSubscriptionContext();
        // getNotificationService().markPublisherNews(subContext, null);
        // }
    ...
    /**
     * Binds a name to an object, along with associated attributes, overwriting any existing binding. If attrs is null and obj is a DirContext, the attributes from obj
     * are used. If attrs is null and obj is not a DirContext, any existing attributes associated with the object already bound in the directory remain unchanged. If
     * attrs is non-null, any existing attributes associated with the object already bound in the directory are removed and attrs is associated with the named object. If
     * obj is a DirContext and attrs is non-null, the attributes of obj are ignored.
     * 
     * @param name
     *            the name to bind; may not be empty
     * @param obj
    ...
    /**
     * Searches in a single context for objects that contain a specified set of attributes, and retrieves selected attributes. The search is performed using the default
     ...
     * @return a non-null enumeration of SearchResult objects. Each SearchResult contains the attributes identified by attributesToReturn and the name of the
     *         corresponding object, named relative to the context named by name.
     * @exception NamingException
     *                if a naming exception is encountered
     */
    @Override
    ...
    /**
     * Searches in a single context for objects that contain a specified set of attributes. This method returns all the attributes of such objects. It is equivalent to
     * supplying null as the atributesToReturn parameter to the method search(Name, Attributes, String[]).
     * 
     * @param name
     *            the name of the context to search
     * @param matchingAttributes
     ...
     *                if the search filter specified is not supported or understood by the underlying directory
     ...
     *            the name of the context or object to search
     * @param filterExpr
     *            the filter expression to use for the search. The expression may contain variables of the form "{i}" where i is a nonnegative integer. May not be null.
     * @param filterArgs
     *            the array of arguments to substitute for the variables in filterExpr. The value of filterArgs[i] will replace each occurrence of "{i}". If null,
     ...
     * @exception InvalidSearchFilterException
     *                if filterExpr with filterArgs represents an invalid search filter
     * @exception NamingException
     *                if a naming exception is encountered
     */
    @Override
    ...
    /**
     * Resolve a file relative to this base. Make sure, paths are relative
     */
    ...
    protected class VFSResource extends Resource {
        ...
        public InputStream streamContent() throws IOException {
            if (!(vfsItem instanceof VFSLeaf))
                throw new IOException("Can't get stream for VFSItem.");
    ...
    protected class VFSResourceAttributes extends ResourceAttributes {
        ...
        public String getResourceType() {
            if (!accessed)
                isCollection(); // needed to initialize
