...
public class Formatter {
     ...
     * 
     * @param locale
     *            the locale which the formatter should use in its operations
     ...
     * @param source
     * @return escaped string
     * @deprecated use org.olat.system.commons.StringHelper.escapeJavaScript() instead.
     */
    ...
    public static String truncate(String source, int len) {
        if (source == null)
            return null;
        if (source.length() > len && len > 3)
            return truncate(source, len - 3, "...");
        else
            return source;
    }
     ...
     * @param source
     * @param len
     *            length of the returned string; if negative, return n chars from the end of the string, otherwise from the beginning of the string
     * @param delim
     * @return truncated string
     */
    ...
    public static String formatWikiMarkup(String oldValue) {
        if (oldValue != null) {
            ...
            // added for compatibility with wikimedia syntax used in the new wiki component. WikiMarkupComponent
