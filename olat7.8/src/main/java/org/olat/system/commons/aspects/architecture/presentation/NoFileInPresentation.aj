 ...
 *
 */
public aspect NoFileInPresentation {
	
	pointcut scope():
		within(org.olat.presentation..*);
 
	pointcut useClassesFromAbove(): 
	    call(* java.io.File*.*(..)) || call(java.io.File*.new(..));	   

	declare warning
		: scope() && useClassesFromAbove()
		: "NoFileInPresentation: You are not allowed to use java.io.File in the presentation layer!";
