...
public class WebappHelper implements Initializable, Destroyable, ServletContextAware {
    ...
    private static String srcViewHgRepo;
    /**
    ...
    /**
     * This path is depends how you deploy OLAT. When deploying in eclipse you will get an different path than deploying within tomcat
     * 
     ...
     * @param srcViewHgRepo
     */
    public void setSrcViewHgRepo(String srcViewHgRepo) {
        WebappHelper.srcViewHgRepo = srcViewHgRepo;
    }

    public static String getSrcViewHgRepo() {
        return WebappHelper.srcViewHgRepo;
    }
    ...
    public static String getBuildOutputFolderRoot() {
        ...
        try {
            ...
            throw new StartupException("could not find classpath resource: 'maven.build.properties'", e);
        }
    }
    ...
    public void setInstanceId(String instanceId) {
        if (instanceId == null)
            throw new StartupException("No instance id set for this installation (see olat.properties). Please fix!");
        if (instanceId.length() > 10)
            throw new StartupException("InstanceID is limited to 10 characters (see olat.properties). Please fix!", null);
    ...
    public void setUserDataRoot(String userDataRoot) {
        if (!StringHelper.containsNonWhitespace(userDataRoot)) {
            userDataRoot = SystemPropertiesLoader.USERDATA_DIR_DEFAULT;
            log.info("using " + userDataRoot + " as userdata. this is the default if userdata.dir is not set");
        }
    ...
    public void setMailConfig(Map<String, String> mailConfig) {
        ...
        if (mailHost == null)
            log.warn("Attention! mailhost fqdn is not set. The system has restricted functionality, e.g. Self Registration will not work!");
    }
    ...
    private void testUtf8FileSystem() {
        ...
        try {
            ...
            log.warn("No UTF-8 capable filesystem found!");
            log.debug("Error while writing test file to file system.", e);
        }
