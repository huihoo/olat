 ...
 *
 */
public aspect NoAccessToCoreServiceInPresentation {
	
	pointcut scope():
		within(org.olat.presentation..*);
 
	pointcut useClassesFromAbove(): 
	    call(* org.olat.lms.core..*Service.*(..));	    
	    
	declare error
		: scope() && useClassesFromAbove()
		: "NoAccessToCoreServiceInPresentation: You are not allowed to use org.olat.lms.core.* in the presentation layer!";
