...
public interface Coordinator {
    ...
    /**
     * Use LockingService via Spring (either Autowired or via CoreSpringFactory) instead!
     * 
     ...
     * @deprecated if you really need the node id try to access it via spring. Search the spring files for ${node.id} for an example. Calling this method depends on a
    ...
    /**
     * Try to avoid coupling your code to an either cluster or single vm version. If you have to check out the spring way by declaring an week reference to an lazy spring
     * bean which gets resolved upon startup time and use the ${cluste.mode} property to decide which version you need: See InstantMessagingSessionCount as an example
     * 
     * @deprecated
