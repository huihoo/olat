 ...
 * <p>
 */
package org.olat.system.coordinate.cache;
 ...
 * either be valid for a certain instance of child caches, or for a certain type of child caches (e.g. for all "CourseModule" caches. The latter will mostly be used to
 * specify cache settings such as max elements, time-to-idle etc. depending on the business domain and the usage pattern. See coreextconfig.xml for an example
 * <P>
...
public class CacheConfig {
    ...
    /**
     * Set t he time in seconds to live for an element before it expires, i.e. the maximum time between creation time and when an element expires. It is only used if the
     * element is not eternal. Default is 120 seconds.
     */
    ...
    /**
     * Set the time in seconds to idle for an element before it expires, that is, the maximum amount of time between accesses before an element expires. This is only used
     * if the element is not eternal. Default is 120 seconds.
     */
    ...
    private Cache doCreateCache(String cacheName) {
        return new Cache(cacheName, this.maxElementsInMemory, this.memoryStoreEvictionPolicy, this.overflowToDisk, this.diskStorePath, this.eternal, this.timeToLive,
                this.timeToIdle, this.diskPersistent, this.diskExpiryThreadIntervalSeconds, null);
    }
