 ...
 * <li>via {@link ThreadLocalLogLevelManager#forceThreadLocalLogLevel(Priority)} overwriting the above level if it is lower</li>
 * </ul>
 * <P>
 * Performance note: Logging can be critical to performance and any modification of log4j is therefore prone to cause performance 'leaks'. This method has been written
 * with this in mind but does indeed add a (although only very minimal) overhead to the normal isDebugEnabled()/debug() performance. The impact should be minimal though.
 * Still, enabling this in production should be optional.
 * <p>
...
public class ThreadLocalAwareLogger extends Logger {
    ...
    /**
     * (from log4j Category) The fully qualified name of the Category class. See also the getFQCN method.
     */
    ...
    /**
     * Creates a new ThreadLocalAwareLogger of the given name, using the given ThreadLocal - with which log levels can be overwritten on a ThreadLocal basis - and uses an
     * optional (i.e. can be null) LogMessageModifier
     * 
     * @param name
     *            the name of this Logger
     * @param threadLocalLogLevel
     *            the ThreadLocal to use to fetch overwritten log levels
     * @param logMessageModifier
     *            optional, allows to modify log messages when they are affected by a ThreadLocal log level overwrite - NOTE: This will convert the message passed to the
     */
    ...
    /**
     * Internal helper method to check whether this logger would log the given level anyway - anyway meaning without the feature 'threadLocalLogLevel_'.
     * 
     * @param level
     *            the level which should be checked if that would anyway be logged
    ...
    /**
     * Internal helper method to check whether the 'threadLocalLogLevel_' would force the given level to be logged - i.e. if the 'threadLocalLogLevel_' is smaller or
     * equal to the given level.
     * <p>
     * Note that this is the core of this ThreadLocalAwareLogger feature.
     * 
     * @param level
     *            the level which should be checked if that is now forced to be logged by the 'threadLocalLogLevel_'
     * @return true if the given level is now forced to be logged by the 'threadLocalLogLevel_'
     */
    ...
    /**
     * Internal helper method to convert an original log message using the optional logMessageModifier.
     * <p>
     * The logMessageModifier is a hook for users of the ThreadLocalAwareLogger to e.g. prepend every log message which was forced to log (i.e. isForcedToLog returns
     * true) with a prefix.
     * 
     * @param originalMessage
     *            the original message which should be modified if there is a logMessageModifier
     * @return the originalMessage if no logMessageModifier is set or the result from the logMessageModifier.modifyLogMessage call
     */
