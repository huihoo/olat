...
public abstract class AbstractEventBus implements EventBus {
     ...
     * @param ores
     */
    protected void doFire(MultiUserEvent event, OLATResourceable ores, EventLogger logger) {
        if (logger == null)
            throw new IllegalArgumentException("logger is not set, check your object wiring!");
        ...
        if (oresId != null) {
            ...
            ea.fireEvent(event, logger);// synchronizes short on listeners but only to copy
        }
        ...
        if (log.isDebugEnabled())
            log.debug("Generic Event from: " + typeName + ": of type: " + event.getClass().getName());
        tea.fireEvent(event, logger);// synchronizes short on listeners but only to copy
    }
