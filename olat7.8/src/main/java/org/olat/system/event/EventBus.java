 ...
 * Classes implementing the {@link org.olat.system.event.GenericEventListener GenericEventListener} can register for an event bound to a certain
 * {@link org.olat.system.commons.resource.OLATResourceable}. A class having "news" concerning an OLATResourceable may fire {@link org.olat.system.event.MultiUserEvent
 * Events} which are sent to all listeners.<br>
...
public interface EventBus {
     ...
     * 
     * @param gel
     *            the GenericEventListener / the class implementing it
     * @param identity
     *            the identity to whicinfoh the listening (controller) belongs, or null if that is not known or the olat-system itself.
     * @param ores
     *            the OLATResourceable
     */
    public abstract void registerFor(GenericEventListener gel, OLATPrincipal principal, OLATResourceable ores);
     ...
     * 
     * @param event
     *            the OLATResourceableEvent (must be serializable!, for multiple server olat installations)
     * @param ores
     *            the OLATResourceable
     */
