...
# 19.11.2004
# gnaegi
# updated to use with svn, 11.4.2006, alexander.schneider@id.uzh.ch
...
if [ "$CONFLICTS" != "" ]; then
  ...
  echo $CONFLICTS | mail -s "Conflicts found while updating locale $LOCALE - fix them" -r "olat@olat.uzh.ch" id_olat@id.uzh.ch
  exit;
fi
