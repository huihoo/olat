...
import org.olat.presentation.framework.core.control.generic.portal.PortletFactory;
...
import org.olat.presentation.repository.delete.TabbedPaneController;
...
public class RepositoryMainController extends MainLayoutBasicController implements Activateable {
    ...
    private static final String VELOCITY_ROOT = PackageUtil.getPackageVelocityRoot(RepositoryMainController.class);
    ...
    private TreeModel buildTreeModel(final boolean bIsAuthor) {
        ...
        if (repoPortletOn) {
            ...
            if (bIsAuthor || businessGroupService.findBusinessGroupsAttendedBy(BusinessGroup.TYPE_RIGHTGROUP, getIdentity(), null).size() > 0) {
     ...
     * @param ureq
     * @param uObj
     * @param subViewIdentifyer
     *            optional view identifyer for a sub controller
     */
    ...
    protected void event(final UserRequest urequest, final Controller source, final Event event) {
        if (source == chooseStepsController) {
            ...
            final RepositoryEntry selectedEntry = RepositoryServiceImpl.getInstance().lookupRepositoryEntry(searchController.getSelectedEntry().getKey());
         ...
         */
            ...
            if (event == Event.CHANGED_EVENT) {
                ...
                final RepositoryEntry e = RepositoryServiceImpl.getInstance().lookupRepositoryEntry(entryChangedEvent.getChangedEntryKey());
                if (e != null) {
                    backtocatalog = true;
    ...
    public void activate(final UserRequest ureq, String viewIdentifier) {
        ...
        if (viewIdentifier.startsWith(JUMPFROMEXTERN)) {
            ...
            final RepositoryEntry selectedEntry = RepositoryServiceImpl.getInstance().lookupRepositoryEntry(key);
            if (selectedEntry != null) {
