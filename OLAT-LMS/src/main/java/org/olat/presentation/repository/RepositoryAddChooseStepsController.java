 ...
 * (norbert.englisch@informatik.tu-chemnitz.de) Author Sebastian Fritzsche (seb.fritzsche@googlemail.com) Author Stefan Köber (stefan.koeber@bps-system.de)
 */
...
/**
 * Description:<br>
 * Choose between - start course creation wizard - open course editor - go to detail page
 * <P>
...
public class RepositoryAddChooseStepsController extends BasicController {
    ...
    static Event CREATION_WIZARD = new Event(ChooseStepsForm.KEY_START_WIZARD);
    /** Step to open the details view has been chosen. */
    static Event DETAILS_VIEW = new Event(ChooseStepsForm.KEY_DETAILS_VIEW);
    /** Step to start the raw course editor has been chosen. */
    static Event COURSE_EDIT = new Event(ChooseStepsForm.KEY_COURSE_EDIT);

    // velocity template
    private final VelocityContainer vc;
    // course elements chooser
    private final FormBasicController chooser;
    ...
    public RepositoryAddChooseStepsController(final UserRequest ureq, final WindowControl control, final RepositoryEntry addedEntry) {
        ...
        // velocity template
        vc = createVelocityContainer("addSteps");
        // chooser form
        chooser = new ChooseStepsForm(ureq, control, getTranslator());
        listenTo(chooser);
     ...
     * inner class
     */
    static class ChooseStepsForm extends FormBasicController {
        ...
        public ChooseStepsForm(final UserRequest ureq, final WindowControl wControl, final Translator translator) {
            super(ureq, wControl);
        ...
        protected boolean validateFormLogic(final UserRequest ureq) {
            return chooser.isOneSelected();
        }

        @Override
        protected void formOK(final UserRequest ureq) {
            final String selection = chooser.getSelectedKey();
            if (selection.equals(KEY_COURSE_EDIT)) {
                fireEvent(ureq, COURSE_EDIT);
            } else if (selection.equals(KEY_DETAILS_VIEW)) {
                fireEvent(ureq, DETAILS_VIEW);
            } else if (selection.equals(KEY_START_WIZARD)) {
