...
public class RepositoryDetailsController extends BasicController implements GenericEventListener {
    ...
    private void updateRepositoryEntryView(final UserRequest ureq) {
        ...
        final String displayName = getDisplayName(ureq.getLocale());
    ...
    private void updateDetailsToolC(final UserRequest ureq) {
        ...
        if (identityRoles.isOwnerOrAdmin()) {
    ...
    private String getDisplayName(final Locale locale) {
        ...
        if (isRepositoryEntryClosed(repositoryEntry)) {
            displayName = "[" + translate("title.prefix.closed") + "] ".concat(displayName);
        }
     ...
     * @return
     */
    private boolean isRepositoryEntryClosed(RepositoryEntry repositoryEntry) {
    ...
    void doLaunch(final UserRequest ureq) {
        ...
        if (dt == null) {
            ...
            final WindowControl bwControl = BusinessControlFactory.getInstance().createBusinessWindowControl(ce, dt.getWindowControl());
    ...
    void doEdit(final UserRequest ureq) {
        ...
        if (dt == null) {
            ...
            final Controller editorController = typeToEdit.createEditorController(ores, ureq, dt.getWindowControl());
    ...
    protected void event(final UserRequest ureq, final Controller source, final Event event) {
        ...
        if (source == groupEditController) {
            ...
            if (cmd.equals(ACTION_DOWNLOAD)) { // download
                ...
                if (!identityRoles.isAuthor()) {
                    throw new OLATSecurityException("Trying to copy, but user is not author: user = " + ureq.getIdentity());
                }
                doCopy(ureq);
                return;
