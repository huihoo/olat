...
public class SelectionController extends BasicController {
    ...
    public SelectionController(final UserRequest ureq, final WindowControl wControl) {
        ...
        this.setTranslator(PackageUtil.createPackageTranslator(this.getClass(), ureq.getLocale(), PackageUtil.createPackageTranslator(RepositoryMainController.class,
                ureq.getLocale())));
    ...
    private void initializeContent() {
        ...
        selectionListContent.contextPut("header", translate("selection.delete.header", new String[] { Integer.toString(RepositoryDeletionManager.getInstance()
    ...
    protected void event(final UserRequest ureq, final Controller source, final Event event) {
        if (source == tableCtr) {
            ...
            if (event == Event.DONE_EVENT) {
                final String result = RepositoryDeletionManager.getInstance().sendDeleteEmailTo(selectedRepositoryEntries, deleteRepositoryMailCtr.getMailTemplate(),
    ...
    private void handleEmailButtonEvent(final UserRequest ureq, final TableMultiSelectEvent tmse) {
        if (redtm.getObjects(tmse.getSelection()).size() != 0) {
            ...
            removeAsListenerAndDispose(cmc);
            cmc = new CloseableModalController(getWindowControl(), translate("close"), deleteRepositoryMailCtr.getInitialComponent());
            listenTo(cmc);
    ...
    private void initializeTableController(final UserRequest ureq) {
        ...
        tableCtr
                .addColumnDescriptor(new StaticColumnDescriptor(ACTION_SINGLESELECT_CHOOSE, "table.header.action", myContent.getTranslator().translate("action.activate")));
    ...
    public void updateRepositoryEntryList() {
        final List l = RepositoryDeletionManager.getInstance().getDeletableReprositoryEntries(RepositoryDeletionManager.getInstance().getLastUsageDuration());
