    ...
    protected void event(final UserRequest ureq, final Controller source, final Event event) {
        if (source == ocForm) {
                ...
                if (actionid.equals(CMD_SELECT_NODE)) {
                    ...
                    if (qtiExportEbl.hasResultSets(course.getResourceableId(), currentCourseNode)) {
                        ...
                        if (dummyMode) {
                            ...
                            exportResult = qtiExportEbl.exportResultsForArchiveWithDefaultFormat(ureq.getIdentity(), ureq.getLocale(), results, currentCourseNode,
                            ...
                            ocForm = new OptionsChooseForm(ureq, getWindowControl(), qtiExportEbl.getQtiItemConfig(qtiExportEbl.getFirstResult(results)));
                            listenTo(ocForm);
            ...
            if (event == Event.DONE_EVENT) {
                ...
                exportResult = qtiExportEbl.exportResultsForArchiveWithCustomFormat(ureq.getIdentity(), ureq.getLocale(), results, exportFormatConfig, currentCourseNode,
                        course.getCourseTitle());
