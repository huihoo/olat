...
public class ContactFormController extends BasicController {
    ...
    private void showFeedbackToUserAndAddToLogFile(boolean isWarn, boolean isError, MessageSendStatus messageStatus) {
        if (isError) {
            String message = messageStatus.createErrorMessageWith(getTranslator());
            showError(message);
            log.error(message, null);
        } else if (isWarn) {
            String message = messageStatus.createInfoMessageWith(getTranslator());
            showInfo(message);
