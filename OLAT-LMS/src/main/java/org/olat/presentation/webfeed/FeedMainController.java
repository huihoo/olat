...
public class FeedMainController extends BasicController implements Activateable, GenericEventListener {
    ...
    public FeedMainController(final OLATResourceable ores, final UserRequest ureq, final WindowControl wControl, final Long courseId, final String nodeId,
        ...
        if (subscriptionContext != null) {
            final String businnessPath = wControl.getBusinessControl().getAsString();
            final PublisherData data = new PublisherData(OresHelper.calculateTypeName(uiFactory.getClass()), ores.getResourceableId().toString(), businnessPath);
            contextualSubscriptionController = new ContextualSubscriptionController(ureq, getWindowControl(), subscriptionContext, data);

            if (uiFactory instanceof PodcastUIFactory) {
                contextualSubscriptionController.setSubscribeButtonTitle("podcast.subscribeButton.title");
                contextualSubscriptionController.setUnsubscribeButtonTitle("podcast.unsubscribeButton.title");
            } else if (uiFactory instanceof BlogUIFactory) {
                contextualSubscriptionController.setSubscribeButtonTitle("blog.subscribeButton.title");
                contextualSubscriptionController.setUnsubscribeButtonTitle("blog.unsubscribeButton.title");
            }
