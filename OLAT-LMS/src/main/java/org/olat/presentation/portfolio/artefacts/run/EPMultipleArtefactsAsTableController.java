...
public class EPMultipleArtefactsAsTableController extends BasicController implements EPMultiArtefactsController {
    ...
    private void initOrUpdateTable(final UserRequest ureq, final List<AbstractArtefact> artefacts) {
		...
		artefactListTblCtrl.addMultiSelectAction("delete", "msc");
    ...
    protected void event(final UserRequest ureq, final Controller source, final Event event) {
        if (source == artefactListTblCtrl) {
            if (event instanceof TableEvent) {
				...
				if (tmse.getAction().equals("msc")) {
					List<AbstractArtefact> abstractArtefacts = artefactListModel.getObjects(tmse.getSelection());
					for(AbstractArtefact abstractArtefact : abstractArtefacts) {
						ePFMgr.deleteArtefact(abstractArtefact);
						ArtefactTableDataModel model = (ArtefactTableDataModel)  artefactListTblCtrl.getTableDataModel();
						model.getObjects().remove(abstractArtefact);
