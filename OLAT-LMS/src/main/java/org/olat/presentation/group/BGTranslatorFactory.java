...
/**
 * Description:<BR>
 * Provides a factory method to create a package translator that has a chain of fallback translators.
 * <P>
...
public class BGTranslatorFactory {
    private static final String PACKAGE_BASE = PackageUtil.getPackageName(BGTranslatorFactory.class);
     ...
     * This means: in the group type properties files all strings can me overridden. If a string is not found there, olat tries to translate it using the package
     * properties files. If the string is not in there, it uses the default group ui properties files.
     * 
     * @param packageName
     *            The package for which a translator should be created or null if only the default group and the type specific translator should be used
     * @param groupType
     *            The group type
     * @param locale
     *            The users locale
     ...
     */
    public static PackageTranslator createBGPackageTranslator(final String packageName, final String groupType, final Locale locale) {
