...
/**
 * Description:<br>
 * This event signalizes that a group has been changed. it contains the modified group
 * <p>
...
public class BusinessGroupModifiedEvent extends MultiUserEvent {
     ...
     * @param identity
     */
    public BusinessGroupModifiedEvent(final String command, final BusinessGroup group, final Identity identity) {
        super(command);
        ...
        if (identity != null) {
            if (command.equals(MYSELF_ASOWNER_REMOVED_EVENT)) {
                ...
                isTutor = getBusinessGroupEBL().isOwner(identity, group);
            }
        }
    }
     ...
     * @return
     */
    public boolean wasMyselfAdded(final Identity identity) {
     ...
     * @return
     */
    public boolean wasMyselfRemoved(final Identity identity) {
        if (isTutor) {
     ...
     *            a list of BusinessGroup objects
     * @param identity
     *            the identity for whom the businessGroupsList was built (we only update the list if the identity is affected)
     * @return true if the list was modified
     */
    public boolean updateBusinessGroupList(final List businessGroups, final Identity identity) {
        final boolean added = wasMyselfAdded(identity);
        final boolean removed = wasMyselfRemoved(identity);
        // we are only interested in added and removed-events here
        if (!added && !removed) {
            return false;
        }
        final Long modKey = getModifiedGroupKey();
        ...
        if (added) {
            ...
            final BusinessGroup nGroup = businessGroupService.loadBusinessGroup(modKey, true);
            // if (SyncHelper.)
            businessGroups.add(nGroup);
            return true;
        }
        // else : removed
        for (final Iterator it_groups = businessGroups.iterator(); it_groups.hasNext();) {
            final BusinessGroup group = (BusinessGroup) it_groups.next();
     ...
     * 
     * @param command
     *            The event identifyer, one of CONFIGURATION_MODIFIED_EVENT, IDENTITY_ADDED_EVENT or IDENTITY_REMOVED_EVENT
     * @param group
     *            The group affected by the modification
     * @param identity
     *            The identity affected by the modification
     */
    public static void fireModifiedGroupEvents(final String command, final BusinessGroup group, final Identity identity) {
        final BusinessGroupModifiedEvent modifiedEvent = new BusinessGroupModifiedEvent(command, group, identity);
