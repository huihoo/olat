...
public class BusinessGroupEditController extends BasicController implements ControllerEventListener, GenericEventListener {
    ...
    public BusinessGroupEditController(final UserRequest ureq, final WindowControl wControl, BusinessGroup currBusinessGroup, final BGConfigFlags configurationFlags) {
        ...
        lockEntry = getBusinessGroupEBL().acquireGroupLockForEditing(identity, this.currBusinessGroup);
    ...
    protected void event(final UserRequest ureq, final Controller source, final Event event) {
        if (source == this.dmsForm && event == Event.CHANGED_EVENT) {
            ...
            if (event instanceof IdentitiesMoveEvent) {
                ...
                final BusinessGroupAddResponse response = businessGroupService.moveIdenityFromWaitingListToParticipant(identitiesMoveEvent.getChosenIdentities(),
    ...
    private void updateBusinessGroup() {
        ...
        BusinessGroupTransferObject businessGroupTransferObject = new BusinessGroupTransferObject(bgName, bgDesc, bgMax, bgMin, waitingListEnabled,
                autoCloseRanksEnabled, new Date(System.currentTimeMillis()));
        this.currBusinessGroup = getBusinessGroupEBL().updateBusinessGroup(waitingListEnabled, businessGroupTransferObject, this.currBusinessGroup);
    ...
    private void releaseBusinessGroupEditLock() {
        if (lockEntry.isSuccess()) {
            getBusinessGroupEBL().releaseGroupLock(lockEntry);
