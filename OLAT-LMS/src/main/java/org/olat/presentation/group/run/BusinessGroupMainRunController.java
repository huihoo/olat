...
public class BusinessGroupMainRunController extends MainLayoutBasicController implements GenericEventListener {
        ...
        if (source == collabToolCtr) {
            ...
            if (event.getCommand().equals(Table.COMMANDLINK_ROWACTION_CLICKED)) {
                ...
                if (actionid.equals(RepositoryTableModel.TABLE_ACTION_SELECT_LINK)) {
                    ...
                    dts.activate(ureq, dt, null); // null: do not activate to a
                                                  // certain
                    // view
                }
            }
    ...
    private ContactFormController createContactFormController(final UserRequest ureq) {
        ...
        String subject = translate("businessgroup.contact.subject", businessGroup.getName());
        String bodyText = "";
        if (sendToChooserForm.waitingListChecked().equals(BusinessGroupSendToChooserForm.NLS_RADIO_NOTHING)) {
            ...
            bodyText = translate("businessgroup.contact.bodytext", new String[] { businessGroup.getName(), restUrl });
        }
