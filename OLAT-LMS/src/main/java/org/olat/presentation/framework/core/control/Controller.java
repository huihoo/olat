...
public interface Controller extends Disposable {
     ...
     * 
     * @param el
     *            The controller that should be added as listener
     */
    ...
    /**
     * disposes the controller. to be called only by the same user that created the controller (or better: make sure event(...) and dispose run sequentially). the
     * behavior must be stable even if multiple invocations occur
     */
    @Override
    public void dispose();
