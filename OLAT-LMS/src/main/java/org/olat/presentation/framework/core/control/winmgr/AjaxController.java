...
public class AjaxController extends DefaultController {
    private static final String VELOCITY_ROOT = PackageUtil.getPackageVelocityRoot(AjaxController.class);
    ...
    AjaxController(UserRequest ureq, final WindowBackOfficeImpl wboImpl, boolean ajaxEnabled, String iframeName) {
        ...
        if (ajaxEnabled)
            myContent.contextPut("isAdmin", Boolean.valueOf(ureq.getUserSession().getRoles().isOLATAdmin()));
    ...
    /**
     * @param moreCmds
     *            a List of WindowCommand objects
     * @return
     */
