...
public class OlatTopNavController extends BasicController /*
    ...
    public OlatTopNavController(final UserRequest ureq, final WindowControl wControl) {
		...
		if (InstantMessagingModule.isEnabled()
				&& !ureq.getUserSession().getRoles().isGuestOnly() && !ureq.getUserSession().getRoles().isInvitee()) {
			imController = InstantMessagingModule.getAdapter()
					.createClientController(ureq, this.getWindowControl());
		...
		if (!ureq.getUserSession().getRoles().isGuestOnly() && !ureq.getUserSession().getRoles().isInvitee()) {
			final SearchServiceUIFactory searchUIFactory = (SearchServiceUIFactory) CoreSpringFactory
					.getBean(SearchServiceUIFactory.class);
			searchC = searchUIFactory.createInputController(ureq, wControl,
					DisplayOption.STANDARD, null);
