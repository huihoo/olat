...
package org.olat.presentation.framework.core.control.generic.iframe;
...
import org.olat.presentation.framework.core.control.generic.textmarker.TextMarkerManagerImpl;
...
/**
 * Class that loads a resource (html) in an Iframe and tries to adjust the size of the Iframe to hide the scrollbars. This is done by injecting some javascript into the
 * head part of the loaded html file which then resizes the iframe itself. See package documentation for details. Initial Date: Dec 9, 2004
 * 
...
public class IFrameDisplayController extends BasicController implements GenericEventListener {
     ...
     * @param wControl
     * @param rootDir
     *            VFSItem that points to the root folder of the resource
     */
     ...
     * @param rootDir
     * @param ores
     *            - send an OLATresourcable of the context (e.g. course) where the iframe runs and it will be checked if the user has textmarking (glossar) enabled in
     *            this course
     */
     ...
     * @param rootDir
     * @param frameId
     *            if you need access to the iframe html id, provide it here
     * @param enableTextmarking
     *            to enable textmakring of the content in the iframe enable it here
     */
    ...
    /**
     * Add a custom HTML header element. This string will be added into the HTML HEAD part of the HTML page. This could be CSS, JS or other header elements.
     * 
     * @param customHeaderContent
     *            A custom HEAD element or NULL
     */
     ...
     *            The currentURI to set
     * @param forceLoading
     *            true: force rendering of iframe velocity container, will redraw everything; false: set new uri without redrawing iframe. This implies that the content
     *            in the iframe has already been loaded (e.g. by an in line user click)
     */
    ...
    private boolean loadPageWithGuess(Page page, String content, String encoding) {
        ...
        if (guessedEncoding != null) {
            ...
            if (page.getEncoding().equalsIgnoreCase(encoding) || page.getEncoding().contains(encoding) || page.getEncoding().toLowerCase().contains(encoding)) {
                page.setUseLoadedPageString(true);
                page.setPage(content);
            }
            return true;
    ...
    /**
     * it would be possible to access the iframe.document but there is no event sended when the content changes. Like this is is easier to inject the js code and resize
     * the iframe like this.
     * 
     * @param page
     * @param addCheckForInlineEvents
     *            true: check if page is rendered in iframe, if yes send event to framework; false: don't do this check
     * @return
     */
    /**
     * TODO:gs make more stable by only adding some js stuff to the end of the page. First check if document.height is ready when puttings js to the end or menachism like
     * ext.onReady is needed
     */
    private String injectJavaScript(String page, String mimetype, boolean addCheckForInlineEvents) {
        ...
        if (docType != null && docType.indexOf("XHTML") > 0)
            sb.append("/"); // close tag only when xhtml to validate
        sb.append(">");
        ...
        // the header of the page
            ...
            if ((page.indexOf("class=\"math\"") != -1 || page.indexOf("class='math'") != -1)
    ...
    public void event(Event event) {
        if (event instanceof MultiUserEvent) {
            ...
            if (currentCustomCSS != null)
                customCssURL = currentCustomCSS.getCSSURL();
