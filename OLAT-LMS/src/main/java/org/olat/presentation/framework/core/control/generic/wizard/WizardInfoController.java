...
public class WizardInfoController extends DefaultController {
    private static final String VELOCITY_ROOT = PackageUtil.getPackageVelocityRoot(WizardInfoController.class);
    private static final String PACKAGE = PackageUtil.getPackageName(WizardInfoController.class);
    ...
    private void renderWizardSteps(StringOutput target, int totalSteps, int step) {
        ...
        for (int i = 1; i <= totalSteps; i++) {
            ...
            if (step == i)
                target.append(activeImage);
            else
                target.append(passiveImage);
        }
