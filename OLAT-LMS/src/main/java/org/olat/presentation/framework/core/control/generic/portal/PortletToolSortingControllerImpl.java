...
/**
 * Controller for providing the necessary tools for a Portlet (e.g. ManualSortingConfig, AutoSortingConfig). This has to be provided by every Portlet to the
 * PortletContainer in order to be added to the PortletContainerToolbox.
 * <P>
 * Initial Date: 07.11.2007 <br>
 * 
 * @author Lavinia Dumitrescu
 */
public class PortletToolSortingControllerImpl extends BasicController implements PortletToolController, PortletToolSorting {
     ...
     * @param wControl
     * @param sortingTerms
     *            (a list with e.g. SortingCriteria.TYPE_SORTING, SortingCriteria.ALPHABETICAL_SORTING,SortingCriteria.DATE_SORTING)
     */
    public PortletToolSortingControllerImpl(UserRequest ureq, WindowControl wControl, Translator callerTranslator, SortingCriteria sortingCriteria,
        ...
        manualSorting.setTooltip(translate("manual.sorting.config"), false);
        ...
        autoSorting.setTooltip(translate("auto.sorting.config"), false);
    ...
    public void event(UserRequest ureq, Component source, Event event) {
        if (source == autoSorting) {
            closeableModalController = new CloseableModalController(getWindowControl(), "close", portletAutoSortingConfigurator.getInitialComponent(), true,
                    getTranslator().translate("auto.sorting.title"));
            closeableModalController.addControllerListener(this);
            closeableModalController.activate();
        } else if (source == manualSorting) {
            closeableModalController = new CloseableModalController(getWindowControl(), "close", portletManualSortingConfigurator.getInitialComponent(), true,
                    getTranslator().translate("manual.sorting.title"));
            closeableModalController.addControllerListener(this);
            closeableModalController.activate();
        }
    }
