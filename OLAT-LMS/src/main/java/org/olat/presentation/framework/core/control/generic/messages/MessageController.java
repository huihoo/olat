 ...
 * </ol>
 * The first kind is feedback which should get away on the next click - it reveals an information concerning the whole screen, it can not freely be placed<br>
 * A message with the same styling as the first kind, but it can be placed as controller anywhere and resides there until removed.<br>
 * The last mentioned type is in fact a dialog and may not look like the other two described.<br>
...
public class MessageController extends BasicController {
    ...
    /**
     * Wrapper for simple info, warn, error text to be placed in the gui where it must be explicitly removed - compared to showInfo / showWarn / showError methods on the
     * basic controller.<br>
     * 
     * @param ureq
     * @param control
     ...
     * @param title
     *            if null it is omitted, if not null it is wrapped with h4 tags
     * @param text
     *            can be null and gets then replaced with empty string.
     */
    MessageController(UserRequest ureq, WindowControl control, int msgType, String title, String text) {
        ...
        setTranslator(PackageUtil.createPackageTranslator(I18nManager.class, getLocale()));
        //
