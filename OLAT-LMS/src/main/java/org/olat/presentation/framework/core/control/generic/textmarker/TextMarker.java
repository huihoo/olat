...
public class TextMarker implements Comparable {
     ...
     *            Text to be marked
     * @param cssClass
     *            CSS class, can be null
     * @param hooverText
     *            The hoover text, can be null
     */
    ...
    public void addToElement(Element root) {
        ...
        if (hooverText != null)
            textMarker.addElement(XML_HOOVER_TEXT_ELEMENT).addCDATA(hooverText);
        if (cssClass != null)
            textMarker.addElement(XML_CSS_CLASS_ELEMENT).addCDATA(cssClass);
    }
    ...
    /**
     * Return only the first marked text in case of a ';'-separated keyword list. When the marked text is only a single keyword, this keyword will be return ( equals
     * getMarkedText() ).
     * 
     * @return
     */
