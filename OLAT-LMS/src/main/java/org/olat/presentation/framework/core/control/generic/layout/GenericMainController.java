 ...
 * <p>
 */
package org.olat.presentation.framework.core.control.generic.layout;
...
/**
 * Description:<br>
 * This generic Controller gets menu-items configured for a site If any other than configured (spring: olat_extensions.xml) items need to be in menu, use
 * addChildNodeToAppend/Prepend() before init(). init() needs to be called to put content to panel.
 * <P>
...
public abstract class GenericMainController extends MainLayoutBasicController {
     ...
     * 
     * @param nodeToAppend
     *            node to add, besides the configured ones
     */
    ...
    /**
     * checks if a userObject has been set, is needed to find the required controller
     * 
     * @param node
     */
    private void checkNodeToAdd(GenericTreeNode node) {
        if (node.getUserObject() == null) {
            throw new AssertException("GenericTreeNode to append/prepend needs to have a UserObject set! Please use setUserObject().");
        }
    }
    ...
    private TreeModel buildTreeModel() {
        ...
        ExtManager extm = CoreSpringFactory.getBean(ExtManager.class);
        int cnt = extm.getExtensionCnt();
        int j = 0;
        for (int i = 0; i < cnt; i++) {
            Extension anExt = extm.getExtension(i);
            // check for sites
            ActionExtension ae = (ActionExtension) anExt.getExtensionFor(className);
    ...
    protected void event(UserRequest ureq, Component source, Event event) {
        if (source == olatMenuTree) {
            if (event.getCommand().equals(MenuTree.COMMAND_TREENODE_CLICKED)) {
                ...
                // 1b) Specialised Action Extension which needs some more internals -> handled by the class extending GenericMainController, by overwriting
                // createController
    ...
    /**
     * needs to be implemented to return corresponding controller for menu items which were not generated by a GenericActionExtension
     * 
     * @param uobject
     * @param ureq
     * @return corresponding controller to be opened by click to menu-item / null if none has been defined
     */
    ...
    private Controller getContentCtr(Object uobject, UserRequest ureq) {
        ...
        if (contentCtr1Tmp == null) {
            throw new AssertException("Node must either be an ActionExtension or implementation must handle this MenuTreeEvent: " + uobject.toString());
        }
