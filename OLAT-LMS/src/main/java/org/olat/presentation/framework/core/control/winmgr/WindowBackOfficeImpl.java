...
import org.olat.presentation.framework.core.control.guistack.GuiStackNiceImpl;
import org.olat.presentation.framework.core.control.guistack.GuiStackSimpleImpl;
import org.olat.presentation.framework.core.control.info.WindowControlInfo;
import org.olat.presentation.framework.core.control.pushpoll.WindowCommand;
...
import org.olat.presentation.framework.core.dev.controller.DevelopmentController;
...
import org.olat.presentation.framework.core.render.intercept.debug.GuiDebugDispatcherController;
...
public class WindowBackOfficeImpl implements WindowBackOffice {
    ...
    public void setHighLightingEnabled(boolean enabled) {
        if (ajaxC != null)
            ajaxC.setHighLightingEnabled(enabled);
    }
    ...
    public void setRequiredRefreshInterval(int refreshInterval) {
        if (ajaxC != null)
            ajaxC.setPollPeriod(refreshInterval);
    }
    ...
    public void informControllerDispatched(WindowControl wControl, Controller controller, Component source, Event event) {
        ...
        if (curGPpos < esize - 1) { // we are not rightmost,
            // that is we have traversed back in history -> clear forward history of gui paths, because those are dumped by the browser. (concept of only having one
            // forward path)
    ...
    public GuiStack createGuiStack(Component initialComponent) {
        ...
        /*
         * final GuiStack deleg = currentGuiStack; GuiStack switchable = new GuiStack() { public void pushModalDialog(Component content) { deleg.pushModalDialog(content);
         * } public void pushContent(Component newContent) { deleg.pushContent(newContent); } public void popContent() { deleg.popContent(); } public Panel getPanel() {
         * return deleg.getPanel(); } public Panel getModalPanel() { return deleg.getModalPanel(); }}; GuiStack gsh; if (winmgrImpl.isForScreenReader()) { gsh = new
         * GuiStackSimpleImpl(initialComponent); } else { gsh = new GuiStackNiceImpl(initialComponent); } return gsh;
         */
    }
    ...
    public void releaseBackHandling(BackHandler backHandler) {
        /*
         * FIXME:pb:2008-08-26: OLAT-3411 encountered on 6.0.3 release, must ne enabled again with BACK if (this.backHandler == null || this.backHandler != backHandler)
         * throw new AssertException("wrong acquire/release order. backhandler:"+backHandler); this.backHandler = null;
         */
    }
