...
public abstract class AbstractPortletRunController extends BasicController {
    ...
    protected void saveSortingConfiguration(UserRequest ureq, SortingCriteria sortingCriteria) {
        ...
        if (sortingCriteria.getSortingType() == SortingCriteria.AUTO_SORTING) {
            Map<Long, Integer> storedPrefs = (Map<Long, Integer>) guiPreferences.get(Map.class, getPreferenceKey(AbstractPortletRunController.SORTED_ITEMS_PREF));
            if (storedPrefs != null) {
    ...
    /**
     * Retrieves the persistent sortingCriteria for the current portlet for the current user. Check the persistent sorting type (auto/manual) if none found auto is the
     * default. This has to be implemented by every portlet since the portlets could have different set of sorting terms.
     * 
     * @param ureq
    ...
    /**
     * Retrieves the persistent manually sorted items for the current portlet and sorts the input PortletEntry list.
     * <p>
     * 
     * @param ureq
    ...
    /**
     * Sorts the itemList according with the sortingCriteria. The list contains portlet specific objects. Each portlet has different types of objects (e.g. BookmarkImpl,
     * BusinessGroup, etc.)
     * 
     * @param itemList
     * @param sortingCriteria
    ...
    /**
     * Compares PortletEntrys. Comparator implementation used for sorting PortletEntry entries according with the choosen manually order (available in the sortedItems
     * map).
     * 
     * @param sortedItems
     * @return
