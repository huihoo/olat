 ...
 * 
 * @author gnaegi Comment: Base class that implements common methods used in TextElements, TextAreaElements and PasswordElements.
 */
public abstract class AbstractTextElement extends AbstractFormElement {
    ...
    public void setValues(String[] values) {
        if (isReadOnly())
            throw new AssertException("setting a readonly field:" + getName());
        if (values == null)
            throw new AssertException("no text (not even empty) for field:" + getName());
        if (values.length != 1)
            throw new AssertException("got multiple values for field:" + getName());
        setValue(values[0]);
    }
    ...
    /**
     * if set, a button will be placed to the right of the textelement. the buttons labelKey, the command/action of the button, the id of the textelement, and the width
     * and height of the popup window are determined by the attributes in popupData
     * 
     * @param popupData
     */
