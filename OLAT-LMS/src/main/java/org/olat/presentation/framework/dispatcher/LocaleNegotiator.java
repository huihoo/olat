...
public class LocaleNegotiator {
    ...
    public static Locale getPreferedLocale(final UserRequest ureq) {
        // first try with the prefered language
        final Locale locale = (Locale) ureq.getUserSession().getEntry(NEGOTIATED_LOCALE);
        if (locale != null) {
            // it's my prefered, I set it alone
            return locale;
        }

        final Locale queryingLocale = getNegotiatedLocale(ureq.getHttpReq().getLocale());
        if (queryingLocale != null) {
            return queryingLocale;
        }

        // next try with the other accepted languages
        for (final Enumeration<Locale> en = ureq.getHttpReq().getLocales(); en.hasMoreElements();) {
            final Locale nextQueryingLocale = getNegotiatedLocale(en.nextElement());
    ...
    public static Locale getNegotiatedLocale(final String lang) {
        final Locale locale = getLocaleForLanguage(lang);
     ...
     */
    public static Locale getNegotiatedLocale(final Locale loc) {
        final Map<String, Locale> allLocales = I18nModule.getAllLocales();
        final Set<String> enabledLanguageKeys = I18nModule.getEnabledLanguageKeys();

        final String lang = loc.getLanguage();
        // search a direct match first de_CH_bs...
        for (final String enabledLanguageKey : enabledLanguageKeys) {
            if (lang.startsWith(enabledLanguageKey)) {
                final Locale locale = allLocales.get(enabledLanguageKey);
