...
package org.olat.presentation.framework.core.control.generic.ajax.autocompletion;
...
public class AutoCompleterController extends BasicController {
     ...
     * @param provider
     *            The provider that can be called to return the search-results for a given search query
     * @param noResults
     *            The translated value to display when no results are found, e.g. "no matches found" or "-no users found-". When a NULL value is provided, the controller
     *            will use a generic message.
     * @param showDisplayKey
     *            true: show the key for each record; false: don't show the key, only the value
     * @param inputWidth
     *            The input field width in characters
     * @param minChars
     *            The minimum number of characters the user has to enter to perform a search
     * @param label
     */
            ...
            if (event.getCommand().equals(COMMAND_SELECT)) {
                ...
                if (key == null) {
                    ...
                    if (searchValue == null) {
                        ...
                        log.error("Auto complete JS code must always send 'key' or the autocomplete parameter!", null);
                    ...
                    if (result.length() > 0) {
                        try {
                            ...
                            log.error("Error while getting json object from list receiver", e);
                            key = "";
                        }
                    } else {
                        key = "";
...
class AutoCompleterListReceiver implements ListReceiver {
     ...
     * 
     * @param noResults
     *            Text to use when no results are found
     * @param showDisplayKey
     *            true: add displayKey in result; false: don't add displayKey in results (e.g. to protect privacy)
     */
