...
public class ColWithBulkActionForm extends FormBasicController {
    ...
    private final String[] fKeys, fValues;

    /**
     * @param name
     * @param trans
     */
    public ColWithBulkActionForm(final UserRequest ureq, final WindowControl wControl, final List columns, final List bulkActions) {
        super(ureq, wControl);
        ...
        final int sizeBulkActions = bulkActions.size();
        fKeys = new String[sizeBulkActions];
        fValues = new String[sizeBulkActions];
        int j = 0;
        for (final Iterator iter = bulkActions.iterator(); iter.hasNext();) {
            fKeys[j] = Integer.toString(j);
            final BulkAction ba = (BulkAction) iter.next();
