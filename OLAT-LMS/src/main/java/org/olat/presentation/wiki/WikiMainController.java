...
/**
 * Description:<br>
 * This controller creates the whole GUI for a wiki with a tabbed pane contaning an article view, per page forum view, edit pane and versioning pane. The rendering of the
 * <P>
...
public class WikiMainController extends BasicController implements CloneableController {
    ...
    WikiMainController(final UserRequest ureq, final WindowControl wControl, final OLATResourceable ores, final WikiSecurityCallback securityCallback,
        ...
        if (subsContext != null) {
            ...
            final PublisherData data = new PublisherData(OresHelper.calculateTypeName(WikiPage.class), ores.getResourceableId().toString(), businnessPath);
            cSubscriptionCtrl = new ContextualSubscriptionController(ureq, getWindowControl(), subsContext, data);
            cSubscriptionCtrl.setSubscribeButtonTitle("wiki.subscribeButton.title");
            cSubscriptionCtrl.setUnsubscribeButtonTitle("wiki.unsubscribeButton.title");
