...
public class SendSubscriberMailOption implements SendMailOption {
    ...
    public SendSubscriberMailOption(final OLATResourceable ores, final String resSubPath, final InfoMessageFrontendManager messageManager) {
        this.ores = ores;
    ...
    public String getOptionTranslatedName(final Locale locale) {
        final Translator translator = PackageUtil.createPackageTranslator(SendSubscriberMailOption.class, locale);
        return translator.translate("wizard.step1.send_option.subscriber");
    }

    @Override
    public List<Identity> getSelectedIdentities() {
        final List<Identity> identities = messageManager.getInfoSubscribers(ores, resSubPath);
        return identities;
    }
}
