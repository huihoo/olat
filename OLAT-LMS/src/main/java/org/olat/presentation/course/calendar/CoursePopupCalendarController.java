...
public class CoursePopupCalendarController extends BasicController {
    ...
    private List getListOfCalendarWrappers(final UserRequest ureq) {
        ...
        final boolean isPrivileged = cgm.isIdentityCourseAdministrator(identity, course) || cgm.hasRight(identity, CourseRights.RIGHT_COURSEEDITOR, course)
                || RepositoryServiceImpl.getInstance().isInstitutionalRessourceManagerFor(RepositoryServiceImpl.getInstance().lookupRepositoryEntry(course, false), identity);
        if (isPrivileged) {
