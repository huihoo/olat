...
public class TACourseNodeEditController extends ActivateableTabbableDefaultController implements ControllerEventListener {
    ...
    protected void event(final UserRequest urequest, final Controller source, final Event event) {
        if (source == modulesForm) {
            ...
            if (identitiesToBeNotified.size() > 0) {
                ...
                final RepositoryEntry repositoryEntry = RepositoryServiceImpl.getInstance().lookupRepositoryEntry(course, true);
    ...
    private MailTemplate createTaskDeletedMailTemplate(final UserRequest ureq, final String courseName, final String courseLink, final String fileName) {
        ...
        final MailTemplate mailTempl = new MailTemplate(subjectTemplate, bodyTemplate, MailTemplateHelper.getMailFooter(null, ureq.getIdentity()), null) {
