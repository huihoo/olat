...
public class RunMainController extends MainLayoutBasicController implements GenericEventListener, Activateable {
    ...
    public RunMainController(final UserRequest ureq, final WindowControl wControl, final ICourse course, final String initialViewIdentifier, final boolean offerBookmark,
        ...
        final Panel empty = new Panel("empty");// empty panel set as "menu" and
                                               // "tool"
        final Controller courseCloser = CourseFactory.createDisposedCourseRestartController(ureq, wControl, courseRepositoryEntry.getResourceableId());
        final Controller disposedRestartController = new LayoutMain3ColsController(ureq, wControl, empty, empty, courseCloser.getInitialComponent(), "disposed course"
                + this.course.getResourceableId());
    ...
    public void event(final Event event) {
        ...
        if (event instanceof PublishEvent) {
            ...
            // courseRepositoryEntry changed (e.g. fired at course access rule
            // change)
    ...
    private ToolController initToolController(final Identity identity, final UserRequest ureq) {
        ...
        if (showCourseConfigLink) {
			myTool.addLink(TOOLBOX_LINK_COURSECONFIG,
					translate("command.courseconfig"));
		}
		if (!isGuest) { 
			myTool.addPopUpLink("personalnote", translate("command.personalnote"),
					null, null, "750", "550", false);
        }
