...
public class EnrollmentEditForm extends FormBasicController {
    ...
    private final String SUBSCRIBER_COUNT = "25";
     ...
     * @param courseCOnfig
     */
    protected EnrollmentEditForm(final UserRequest ureq, final WindowControl wControl, final CourseCreationConfiguration courseCOnfig) {
        super(ureq, wControl);
        this.courseConfig = courseCOnfig;
        super.setTranslator(PackageUtil.createPackageTranslator(CourseCreationHelper.class, ureq.getLocale()));
    ...
    private void initConfig() {
        ...
        if (courseConfig.isCreateSinglePage()) {
            elements.add(translate("cce.informationpage"));
        }
        if (courseConfig.isCreateDownloadFolder()) {
            elements.add(translate("cce.downloadfolder"));
        }
        if (courseConfig.isCreateForum()) {
            elements.add(translate("cce.forum"));
        }
        if (courseConfig.isCreateContactForm()) {
            elements.add(translate("cce.contactform"));
        }
    ...
    protected void initForm(final FormItemContainer formLayout, final Controller listener, final UserRequest ureq) {
        final FormLayoutContainer formButtons = FormLayoutContainer.createButtonLayout("formButtons", this.getTranslator());
        ...
        final Set<FormItem> targets = new HashSet<FormItem>() {
            {
                add(courseElements);
            }
        };
    ...
    protected void formOK(final UserRequest ureq) {
        ...
        final String s = subscriberCount.getValue().trim();
        if (s.length() > 0) {
