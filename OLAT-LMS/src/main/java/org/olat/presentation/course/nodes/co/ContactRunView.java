...
/**
 * This creates a contact message sending form in case a contact message is set. In the case where no contact message is defined it shows an
 * info message that no recipients are defined.
 * 
	...
	public Component getInitialComponent(DefaultController listeningController){
		...
		if(courseContactMessage != null){
			...
			ContactFormView contactFormView = 
					new ContactFormView(ureq, wControl, courseContactMessage.getFrom(), contactUImodel.hasAtLeastOneAddress(), false, false, false);
