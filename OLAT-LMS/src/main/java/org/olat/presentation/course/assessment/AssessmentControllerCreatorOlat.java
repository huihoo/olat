...
public class AssessmentControllerCreatorOlat implements AssessmentControllerCreator {
    ...
    @Autowired
    AssessmentNotificationsHandler notificationsHandler;
    ...
    public Controller createContextualSubscriptionController(final UserRequest ureq, final WindowControl wControl, final ICourse course) {
        ...
        final SubscriptionContext subsContext = notificationsHandler.getAssessmentSubscriptionContext(ureq.getIdentity(), course);
        if (subsContext != null) {
            final PublisherData pData = notificationsHandler.getAssessmentPublisherData(course, wControl.getBusinessControl().getAsString());
