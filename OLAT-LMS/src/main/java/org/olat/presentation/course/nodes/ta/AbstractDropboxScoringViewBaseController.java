        ...
        if (source == dropboxFolderRunController) {
            ...
            if (event instanceof FolderEvent) {
                ...
                if (folderEvent.getCommand().equals(FolderEvent.UPLOAD_EVENT) || folderEvent.getCommand().equals(FolderEvent.NEW_FILE_EVENT)) {
                    ...
                    final String toMail = getUserService().getUserProperty(student.getUser(), UserConstants.EMAIL, ureq.getLocale());
                    ...
                    log.error("#####################################################");
                    log.error("#####  TODO: JumpInLink migrate to REST urls !!! ####");
                    log.error("#####################################################");
                    final String link = null; // TODO mirgrate to REST urls
                    ...
                    final MailTemplate mailTempl = new MailTemplate(translate("returnbox.email.subject"), translate("returnbox.email.body", new String[] {
                            userCourseEnv.getCourseEnvironment().getCourseTitle(), node.getShortTitle(), folderEvent.getFilename(), link }),
                            MailTemplateHelper.getMailFooter(student, null), null) {
