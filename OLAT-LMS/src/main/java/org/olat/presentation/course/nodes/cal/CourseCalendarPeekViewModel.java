...
public class CourseCalendarPeekViewModel extends DefaultTableDataModel {
    ...
    public CourseCalendarPeekViewModel(final List<CalendarEntry> events, final Translator translator) {
        super(events);
    ...
    public Object getValueAt(final int row, final int col) {
        ...
        case 0:
            ...
            if (event.isToday()) {
                return translator.translate("calendar.today") + " " + timeFormat.format(event.getBegin()) + " - " + timeFormat.format(event.getEnd());
            }
            if (event.isAllDayEvent()) {
                return dateOnlyFormat.format(event.getBegin());
            }
            if (event.isWithinOneDay()) {
                return dateOnlyFormat.format(event.getBegin()) + " " + timeFormat.format(event.getBegin()) + " - " + timeFormat.format(event.getEnd());
            }
