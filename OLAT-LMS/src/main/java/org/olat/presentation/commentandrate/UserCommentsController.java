...
/**
 * Description:<br>
 * The user comments controller shows all user comments for a certain resource and offers edit functionality
 * <p>
...
public class UserCommentsController extends BasicController {
    ...
    /**
     * Constructor for a user comments controller. Use the CommentAndRatingService instead of calling this constructor directly!
     * 
     * @param ureq
     * @param wControl
     ...
     */
    UserCommentsController(UserRequest ureq, WindowControl wControl, UserCommentsManager commentManager, CommentAndRatingSecurityCallback securityCallback) {
        super(ureq, wControl);
        ...
        if (source == createCommentFormCtr) {
            if (event == Event.CANCELLED_EVENT) {
                ...
                if (allComments.size() != commentsCount.longValue()) {
                    ...
                    UserCommentDisplayController commentController = new UserCommentDisplayController(ureq, getWindowControl(), commentManager, newComment, allComments,
                            securityCallback);
    ...
    /**
     * Internal helper to build the view controller for the direct comments (without replies)
     * 
     * @param ureq
     * @param initDatamodel
     *            true: load datamodel from database; false: just rebuild GUI with existing datamodel
     */
    private void buildTopLevelComments(UserRequest ureq, boolean initDatamodel) {
        ...
        for (UserComment comment : allComments) {
            if (comment.getParent() == null) {
                // Create top level comment controller
                UserCommentDisplayController commentController = new UserCommentDisplayController(ureq, getWindowControl(), commentManager, comment, allComments,
                        securityCallback);
                commentControllers.add(commentController);
                listenTo(commentController);
                userCommentsVC.put(commentController.getViewCompName(), commentController.getInitialComponent());
    ...
    /**
     * Helper method to access the current number of comments in the list displayed by this controller.
     * 
