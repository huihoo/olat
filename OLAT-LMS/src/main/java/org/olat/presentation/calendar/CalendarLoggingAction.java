...
public class CalendarLoggingAction extends BaseLoggingAction {
    ...
    public static final ILoggingAction CALENDAR_ENTRY_MODIFIED = new CalendarLoggingAction(ActionType.tracking, CrudAction.update, ActionVerb.edit, ActionObject.calendar)
            .setTypeList(CALENDAR_READ_RESOURCES);
    public static final ILoggingAction CALENDAR_ENTRY_CREATED = new CalendarLoggingAction(ActionType.tracking, CrudAction.update, ActionVerb.add, ActionObject.calendar)
            .setTypeList(CALENDAR_READ_RESOURCES);
     ...
     */
    static {
        final Field[] fields = CalendarLoggingAction.class.getDeclaredFields();
        if (fields != null) {
            for (int i = 0; i < fields.length; i++) {
                final Field field = fields[i];
                if (field.getType() == CalendarLoggingAction.class) {
                    try {
                        final CalendarLoggingAction aLoggingAction = (CalendarLoggingAction) field.get(null);
    ...
    CalendarLoggingAction(final ActionType resourceActionType, final CrudAction action, final ActionVerb actionVerb, final ActionObject actionObject) {
