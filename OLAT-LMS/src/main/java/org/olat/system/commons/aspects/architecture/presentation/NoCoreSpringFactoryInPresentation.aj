 ...
 *
 */
public aspect NoCoreSpringFactoryInPresentation {
	
	pointcut scope():
		within(org.olat.presentation..*);
 
	pointcut useClassesFromAbove(): 
	    call(* org.olat.system.spring.CoreSpringFactory.*(..));

	declare warning
		: scope() && useClassesFromAbove()
		: "NoCoreSpringFactoryInPresentation: You are not allowed to use org.olat.system.spring.CoreSpringFactory in the presentation layer!";
