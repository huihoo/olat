 ...
 *
 */
public aspect NoAccessToSupportingServiceInPresentation {
	
	pointcut scope():
		within(org.olat.presentation..*);
 
	pointcut useClassesFromAbove(): 
	    call(* org.olat.system.support..*.*(..));	    

	declare error
		: scope() && useClassesFromAbove()
		: "NoAccessToSupportingServiceInPresentation: You are not allowed to use org.olat.system.support.* in the presentation layer!";
