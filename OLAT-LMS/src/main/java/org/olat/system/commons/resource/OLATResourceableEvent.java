...
/**
 * Description:<br>
 * event from an olatresourceable. Important: in order to be serializable, only the flat form of the OLATResoureable is taken (a String), and a method is offered to test
 * on contains and equals.
 * 
 * @author Felix Jost
 */
public abstract class OLATResourceableEvent extends MultiUserEvent {
    ...
    /**
     * @param ores
     *            the OLATResourceable to test against.
     * @param exceptIfFalse
     *            if true = throw an exception if the target does not match ores
    ...
    public boolean targetEquals(OLATResourceable ores, boolean exceptIfFalse) {
        ...
        if (!res && exceptIfFalse)
            throw new AssertException("expected ores to be the same as target, but failed: ores =" + ores.getResourceableTypeName() + " / " + ores.getResourceableId()
                    + ", but event was for " + derived);
        return res;
    }
