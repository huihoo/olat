 ...
 *
 */
public aspect NoAccessToServiceImplInPresentation {
	
	pointcut scope():
		within(org.olat.presentation..*);
 
	pointcut useClassesFromAbove(): 
	    call(* org.olat.lms.learn.*.impl..*.*(..)) || call(org.olat.lms.learn.*.impl..*.new(..));	    

	declare error
		: scope() && useClassesFromAbove()
		: "NoAccessToServiceImplInPresentation: You are not allowed to use org.olat.lms.learn.*.impl.* in the presentation layer!";
