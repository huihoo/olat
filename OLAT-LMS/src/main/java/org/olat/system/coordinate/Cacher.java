 ...
 * only according to their ttl, tti settings. <br>
 ...
 * e.g. com.mystuff.MyClass:myname@RepositoryEntry and further "@" with further subtypes for further subcaches created hereof. <br>
 * <br>
 * the idea of the hierarchy is to represent the dependencies (=composition) between objects cached. e.g. a cached wikipage should be obtained using a cache created from
 * the wiki cache, since the wikipage can only exist as long as the wiki exists.
 * <P>
...
public interface Cacher {
    ...
    /**
     * olat:::: change ownerClass to Manager, introduce Manager interface (marker interface). prevent Controllers from using this cache. <br>
     * to set the max-time-to-live, max-idle-time and max-elements values, edit the file olatcoreconfig.xml.<br>
     * For the exact meaning of these three parameters, consult the ehcache-1.3.0-documentation.
     * 
     * @param coOwnerClass
     *            the class the cache "belongs to" (the (manager) class that is responsible for managing the cache)
     * @param name
     *            an optional name to be able to create more than one cache for the same coOwnerClass
