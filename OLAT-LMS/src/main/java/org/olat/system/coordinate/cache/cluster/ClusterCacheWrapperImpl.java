 ...
 * <p>
 */
package org.olat.system.coordinate.cache.cluster;
...
import org.olat.system.coordinate.cache.singlevm.CacheWrapperImpl;
...
public class ClusterCacheWrapperImpl extends CacheWrapperImpl {
    ...
    public void remove(String key) {
        super.remove(key);
        // always notify the other nodes, even if the key was not there - we cannot know (we would have to introduce flags per key) why it was not there (expired or
        // invalidated)
        afterChanged(new String[] { key });
    }
