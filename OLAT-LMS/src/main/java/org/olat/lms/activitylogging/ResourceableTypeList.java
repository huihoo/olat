 ...
 * <p>
 * The idea of ResouceableTypeList is to be able to make checks between the businessPath and all the LoggingResourceables collected during the Controller lifetime, the
 * event handling and information passed with the log() call.
 * <p>
 * Check with the LoggingAction class to see most common use cases of ResourceableTypeList - here's an excerpt though:<br>
 * 
 * <pre>
 * new ResourceableTypeList().addMandatory(OlatResourceableType.wiki).or().addMandatory(OlatResourceableType.businessGroup).addOptional(OlatResourceableType.wiki);
 * </pre>
 * 
 * So the idea is to chain addMandatory and addOptional calls for as long as types need to be added to the same list - then OR that by calling or() and add a second/third
 * variant which also is allowed etc etc. As soon as one of the lists validates to true it will return true for the whole list.
 * <P>
...
public class ResourceableTypeList {
    ...
    /**
     * ORed lists are stored in the parent_'s orList_ - also, if this list has a parent it will always go to the parent to do a job through the whole list/OR structure
     */
     ...
     * 
     * @param resourceableTypes
     *            the list of ILoggingResourceableTypes to be added as mandatory to this list
     * @return this
     */
     ...
     * 
     * @param resourceableTypes
     *            the list of ILoggingResourceableTypes to be added as optional to this list
     * @return this
     */
    ...
    /**
     * Goes through the resourceInfos list and returns the LoggingResourceable which has the same type as the passed ILoggingResourceableType.
     * 
     * @param resourceInfos
     *            the list which should be searched through
     * @param type
     *            the type which we are looking for
    ...
    /**
     * Executes the businessPath-check on <b>this</b> List - i.e. not on any parent or orList_ child but exactly on this list.
     * <p>
     * This method is called by executeCheckAndGetErrorMessage() which deals with parent/orList tweaks.
     * 
     * @param resourceInfos
     *            the list which should be checked against
