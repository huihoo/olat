...
public class CourseLoggingAction extends BaseLoggingAction {
    ...
    public static final ILoggingAction ST_GOTO_NODE = new CourseLoggingAction(ActionType.statistic, CrudAction.retrieve, ActionVerb.open, ActionObject.gotonode)
            .setTypeList(new ResourceableTypeList().addMandatory(OlatResourceableType.course, OlatResourceableType.node, StringResourceableType.nodeId)
                    ...
                    /*
                     * 2010-02-26 00:03:36,515 [TP-Processor754] WARN UserActivityLoggerImpl - OLAT::WARN ^%^ N2-W40796 ^%^ logging.activity ^%^ vjetel ^%^ 85.2.180.155
                     * ^%^ https://www.olat.uzh.ch/olat/auth/1%3A1%3A0%3A0%3A0/ ^%^ Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; GTB6.4) ^%^ LoggingAction reported
                     * an inconsistency: action=CourseLoggingAction@79888465, fieldId=null, expected: [Mandatory are course,node,nodeId. ], actual: [LoggingResourceables:
                     * 5, LoggingResourceInfo[type=CourseModule,rtype=genRepoEntry,id=80997996474040,name=Krier: Einführung in die Literaturwissenschaft FS 2010],
                     * LoggingResourceInfo[type=st,rtype=node,id=81053185492848,name=Sitzung II],
