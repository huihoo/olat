...
public class EvalAttributeFunction extends AbstractFunction {
     ...
     * @param userCourseEnv
     * @param functionType
     *            the type of function (use defined constants)
     */
    public EvalAttributeFunction(final UserCourseEnvironment userCourseEnv, final int functionType) {
        super(userCourseEnv);
    ...
    public Object call(final Object[] inStack) {
        ...
        if (!(inStack[1] instanceof String)) {
            return handleException(new ArgumentParseException(ArgumentParseException.WRONG_ARGUMENT_FORMAT, name, "", "error.argtype.attribvalue",
                    "solution.example.name.infunction"));
        }
        final String attributeId = (String) inStack[0];
        /*
         ...
         */
        final String attName = (String) inStack[0];
        final String attValue = (String) inStack[1];

        final IdentityEnvironment ienv = getUserCourseEnv().getIdentityEnvironment();
        final Identity ident = ienv.getIdentity();
        final Map attributes = ienv.getAttributes();
        if (attributes == null) {
            return ConditionInterpreter.INT_FALSE;
        }
        final String value = (String) attributes.get(attName);
        ...
        if (log.isDebugEnabled()) {
            log.debug("identity '" + ident.getName() + "' tested on attribute '" + attName + "' to have value '" + attValue + "' user's value was '" + value
                    + "', match=" + match);
        }
