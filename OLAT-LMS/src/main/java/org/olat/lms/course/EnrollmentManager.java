...
import org.olat.presentation.group.mail.BGMailHelper;
...
public class EnrollmentManager extends BasicManager {
    ...
    private final int ENROLMENT_OR_WAINTING_DONE = 10;
                    ...
                    if (reloadedGroup.getMaxParticipants() != null && !reloadedGroup.getMaxParticipants().equals("")) {
                        ...
                        if (participantsCounter >= reloadedGroup.getMaxParticipants().intValue()) {
                            ...
                            if (!reloadedGroup.getWaitingListEnabled().booleanValue()) {
                                ...
                                final MailerResult mailerResult = addUserToWaitingList(identity, reloadedGroup, enNode, coursePropertyManager);
                                final boolean done = mailerResult.getReturnCode() == ENROLMENT_OR_WAINTING_DONE;
                                enrollStatus.setIsInWaitingList(done);
                                enrollStatus.setMailerResult(mailerResult);
                            }
                        } else {
                            final MailerResult mailerResult = addUserToParticipantList(identity, reloadedGroup, enNode, coursePropertyManager);
                            final boolean done = mailerResult.getReturnCode() == ENROLMENT_OR_WAINTING_DONE;
    ...
    public EnrollStatus doCancelEnrollment(final Identity identity, final BusinessGroup enrolledGroup, final ENCourseNode enNode,
        ...
        final MailTemplate mailTemplate = BGMailHelper.createRemoveMyselfMailTemplate(enrolledGroup, identity);
        final MailerWithTemplate mailer = MailerWithTemplate.getInstance();
        final MailerResult mailerResult = mailer.sendMail(identity, null, null, mailTemplate, null);
    ...
    public EnrollStatus doCancelEnrollmentInWaitingList(final Identity identity, final BusinessGroup enrolledWaitingListGroup, final ENCourseNode enNode,
        ...
        // 3. Send notification mail
        final MailTemplate mailTemplate = BGMailHelper.createRemoveWaitinglistMailTemplate(enrolledWaitingListGroup, identity);
        final MailerWithTemplate mailer = MailerWithTemplate.getInstance();
        final MailerResult mailerResult = mailer.sendMail(identity, null, null, mailTemplate, null);
     ...
     */
    public List loadGroupsFromNames(final OLATResourceable ores, final List groupNames, final List areaNames, final CourseGroupManager cgm) {
    ...
    // Private Methods
    // /////////////////
    private MailerResult addUserToParticipantList(final Identity identity, final BusinessGroup group, final ENCourseNode enNode,
            final CoursePropertyManager coursePropertyManager) {
        ...
        final MailTemplate mailTemplate = BGMailHelper.createAddMyselfMailTemplate(group, identity);
        final MailerWithTemplate mailer = MailerWithTemplate.getInstance();
        final MailerResult mailerResult = mailer.sendMail(identity, null, null, mailTemplate, null);
        mailerResult.setReturnCode(ENROLMENT_OR_WAINTING_DONE);
        // MailHelper.printErrorsAndWarnings(mailerResult, wControl, trans.getLocale());

        return mailerResult;
    }

    private MailerResult addUserToWaitingList(final Identity identity, final BusinessGroup group, final ENCourseNode enNode,
            final CoursePropertyManager coursePropertyManager) {
        CoordinatorManager.getInstance().getCoordinator().getSyncer().assertAlreadyDoInSyncFor(group);
        // 1. Add user to group, fire events, do loggin etc.
        ...
        // 4. Send notification mail
        final MailTemplate mailTemplate = BGMailHelper.createAddWaitinglistMailTemplate(group, identity);
        final MailerWithTemplate mailer = MailerWithTemplate.getInstance();
        final MailerResult mailerResult = mailer.sendMail(identity, null, null, mailTemplate, null);
        mailerResult.setReturnCode(ENROLMENT_OR_WAINTING_DONE);
        // MailHelper.printErrorsAndWarnings(mailerResult, wControl, trans.getLocale());
