...
public class NewCachePersistingAssessmentManager extends BasicManager implements AssessmentManager {
    ...
    private static CacheWrapper assessmentMainCache = CoordinatorManager.getInstance().getCoordinator().getCacher().getOrCreateCache(
            NewCachePersistingAssessmentManager.class, null);
        ...
        CoordinatorManager.getInstance().getCoordinator().getSyncer().doInSync(createOLATResourceableForLocking(assessedIdentity), new SyncerExecutor() {
            ...
            public void execute() {
                ...
                if (attemptsProperty == null) {
                    attemptsProperty = cpm.createCourseNodePropertyInstance(courseNode, assessedIdentity, null, AssessmentPropertyDao.ATTEMPTS, null, new Long(attempts
                            .intValue()), null, null);
    ...
    private void incrementNodeAttempts(final CourseNode courseNode, final Identity identity, final UserCourseEnvironment userCourseEnv, final boolean logActivity) {
        final ICourse course = CourseFactory.loadCourse(ores);
        final CoursePropertyManager cpm = course.getCourseEnvironment().getCoursePropertyManager();
        final long attempts = CoordinatorManager.getInstance().getCoordinator().getSyncer().doInSync(createOLATResourceableForLocking(identity),
                new SyncerCallback<Long>() {
                    @Override
                    public Long execute() {
        ...
        if (logActivity) {
            // user activity logging
            ThreadLocalUserActivityLogger.log(AssessmentLoggingAction.ASSESSMENT_ATTEMPTS_UPDATED, getClass(), LoggingResourceable.wrap(identity), LoggingResourceable
                    .wrapNonOlatResource(StringResourceableType.qtiAttempts, "", String.valueOf(attempts)));
        }
    }
    ...
    public void saveScoreEvaluation(final CourseNode courseNode, final Identity identity, final Identity assessedIdentity, final ScoreEvaluation scoreEvaluation,
        ...
        final Long attempts = CoordinatorManager.getInstance().getCoordinator().getSyncer().doInSync(createOLATResourceableForLocking(assessedIdentity),
                new SyncerCallback<Long>() {
                    @Override
                    public Long execute() {
                        Long attempts = null;
