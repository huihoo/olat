...
import org.olat.lms.ims.qti.navigator.MenuSectionNavigator;
import org.olat.lms.ims.qti.navigator.Navigator;
import org.olat.lms.ims.qti.navigator.SequentialItemNavigator;
import org.olat.lms.ims.qti.navigator.SequentialSectionNavigator;
...
public class AssessmentInstance implements Serializable {
    ...
    private long assessID; // the key given to this instance by the constructor; identifying the assessment within the olat context; needed after deserialisation to find
                           // the correct qti tree
     ...
     * @param resolver
     * @param persistor
     *            the Persistor, may be null
     * @param modConfig
     */
    public AssessmentInstance(final long repositoryEntryKey, final long assessID, final Resolver resolver, final Persister persistor, final ModuleConfiguration modConfig) {
        ...
        final Boolean confDisplayTitles = (Boolean) modConfig.get(IQEditController.CONFIG_KEY_QUESTIONTITLE);
        ...
        final Boolean confAutoEnum = (Boolean) modConfig.get(IQEditController.CONFIG_KEY_AUTOENUM_CHOICES);
        ...
        // fetch auto enum choice options switch
        memo = MEMO_DEFAULT;
        final Boolean confMemo = (Boolean) modConfig.get(IQEditController.CONFIG_KEY_MEMO);
        if (confMemo != null) {
        ...
        // fetch type of menu
        menu = MENU_DEFAULT;
        final Boolean confEnableMenu = (Boolean) modConfig.get(IQEditController.CONFIG_KEY_ENABLEMENU);
        ...
        final String confMode = (String) modConfig.get(IQEditController.CONFIG_KEY_TYPE);
        if (confMode != null) {
            if (confMode.equals(QMD_ENTRY_TYPE_SELF)) {
                type = TYPE_SELF;
            } else if (confMode.equals(QMD_ENTRY_TYPE_ASSESS)) {
                type = TYPE_ASSESS;
            } else if (confMode.equals(QMD_ENTRY_TYPE_SURVEY)) {
                type = TYPE_SURVEY;
            }
        }
        ...
        final String confSequence = (String) modConfig.get(IQEditController.CONFIG_KEY_SEQUENCE);
        if (confSequence != null) {
            if (confSequence.equals(QMD_ENTRY_SEQUENCE_ITEM)) {
                sequence = SEQUENCE_ITEM;
            } else if (confSequence.equals(QMD_ENTRY_SEQUENCE_SECTION)) {
        ...
        final String confSummary = (String) modConfig.get(IQEditController.CONFIG_KEY_SUMMARY);
        if (confSummary != null) {
     ...
     * @return String
     */
    public static String getFormattedType(final int iType) {
        switch (iType) {
        case TYPE_ASSESS:
            return QMD_ENTRY_TYPE_ASSESS;
        case TYPE_SELF:
            return QMD_ENTRY_TYPE_SELF;
        case TYPE_SURVEY:
            return QMD_ENTRY_TYPE_SURVEY;
        }
        return null; // error?
    }

    public boolean isAssess() {
        return type == TYPE_ASSESS;
    }
     ...
     */
    public static int getSummaryType(final String confSummary) {
    ...
    public void close() {
        ...
        final SectionContext sc = assessmentContext.getCurrentSectionContext();
        if (sc != null) {
    ...
    public void persist() {
        if (!closed) { // ignore if assessment has already been closed
            final Persister pers = getPersister();
     ...
     * @param sourceBankRef
     */
    public void addObjectBankRef(final String sourceBankRef) {
     ...
     *            The resuming to set
     */
    public void setResuming(final boolean resuming) {
     ...
     */
    private final Map<String, Boolean> marked = new HashMap<String, Boolean>();

    public void mark(final String id, final boolean b) {
        marked.put(id, Boolean.valueOf(b));
    }

    public boolean isMarked(final String id) {
        final Boolean rv = marked.get(id);
     ...
     */
    private final Map<String, String> memos = new HashMap<String, String>();

    public void setMemo(final String id, final String m) {
        memos.put(id, m);
    }

    public String getMemo(final String id) {
        return memos.get(id);
    }
}
