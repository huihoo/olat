...
package org.olat.lms.ims.resources;
...
public class IMSEntityResolver implements EntityResolver {
    ...
    public InputSource resolveEntity(final String publicId, String systemId) {
        ...
        if (systemId_.endsWith(".dtd")) {
            final InputStream in = getClass().getResourceAsStream("/org/olat/lms/ims/resources/dtd/" + systemId);
            return new InputSource(in);
        } else if (systemId_.endsWith(".xsd")) {
            final InputStream in = getClass().getResourceAsStream("/org/olat/lms/ims/resources/xsd/" + systemId);
            return new InputSource(in);
        } else if (systemId_.endsWith(".xsl")) {
            final InputStream in = getClass().getResourceAsStream("/org/olat/lms/ims/resources/xsl/" + systemId);
            return new InputSource(in);
        }
