...
public class CPTreeDataModel extends AjaxTreeModel {
    ...
    private final Hashtable<String, String> map, reverseMap;
    private int nodeCounter;

    /**
     * Constructor
     * 
     * @param orgaIdentifyer
     *            The identifier of the organization element of the imsmanifest file.
     * @param rootNodeId
     *            The root node id of the ext-js tree. (We use an md5 hash to make sure it doesn't contain any weird characters.)
     */
    public CPTreeDataModel(final String orgaIdentifyer, final String rootNodeId) {
     ...
     * @param cp
     */
    public void setContentPackage(final ContentPackage cp) {
        this.cp = cp;
    }

    /**
     * Node-ids in the datamodel must be integers This method returns the next nodeID (incremental)
     * 
     * @return
     */
     ...
     * @return
     */
    public String getIdentifierForNodeID(final String nodeID) {
        final String identifier = map.get(nodeID);
        return identifier;
    }
     ...
     * @return
     */
    public String getNodeIDForIdentifier(final String identifier) {
    ...
    /**
     * gets a new nodeID for the identifier and puts the key/value pair into the map Returns the new nodeID
     * 
     * @param identifier
     * @return
     */
    private String putIdentifierForNodeID(final String identifier) {
        String nodeId = reverseMap.get(identifier);
        if (nodeId == null) {
     ...
     */
    private void addItem(final Vector<AjaxTreeNode> nodeList, final CPItem item) throws JSONException {
        String nId;
        nId = putIdentifierForNodeID(item.getIdentifier());
        final AjaxTreeNode child = new AjaxTreeNode(nId, item.getTitle());
     ...
     */
    public String getPath(final String identifier) {
        final StringBuffer path = new StringBuffer();
        final DefaultElement elem = cp.getElementByIdentifier(identifier);
     ...
     * @param elem
     */
    private void addElementToPath(final DefaultElement elem, final StringBuffer path) {
        final String slash = "/";
        if (elem instanceof CPItem) {
            final CPItem item = (CPItem) elem;
            path.insert(0, slash).insert(1, getNodeIDForIdentifier(item.getIdentifier()));
            final DefaultElement parent = item.getParentElement();
            if (parent != null) {
                addElementToPath(parent, path);
            }
        } else if (elem instanceof CPOrganization) {
            final CPOrganization item = (CPOrganization) elem;
            path.insert(0, slash).insert(1, getNodeIDForIdentifier(item.getIdentifier()));
        }
    }
}
