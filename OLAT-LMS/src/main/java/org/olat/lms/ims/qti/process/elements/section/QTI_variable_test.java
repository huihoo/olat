...
public class QTI_variable_test implements ScoreBooleanEvaluable {
    ...
    public boolean eval(final Element boolElement, final float score) {
        /*
         * Attributes: varname (optional. Default = 'SCORE'). The name of the variable that is to be tested. The default name is 'SCORE'. Data-type = String (max of 256
         * chars). testoperator (mandatory with selection from the enumerated list of: EQ, NEQ, LT, LTE, GT, GTE). Identifies the nature of the variable comparison that
         * is to be applied. Data-type = string (1-16 chars).
         */
        final String testOperator = boolElement.attributeValue("testoperator");
        final String val = boolElement.getText();
        final float nval = Integer.parseInt(val);
