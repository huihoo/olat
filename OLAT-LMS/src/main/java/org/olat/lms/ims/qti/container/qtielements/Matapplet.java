...
public class Matapplet extends GenericQTIElement {
     ...
     * @param el_matimage
     */
    public Matapplet(final Element el_matimage) {
        super(el_matimage);
    ...
    public void render(final StringBuilder buffer, final RenderInstructions ri) {
        ...
        final String staticsPath = (String) ri.get(RenderInstructions.KEY_STATICS_PATH);
        final int paramStopChar = uri.indexOf('?');
        if (paramStopChar != -1) {
            strippedURI = uri.substring(0, paramStopChar);
        }
        ...
        if (strippedURI.toLowerCase().endsWith(".swf")) {
            ...
            buffer.append("oResponseURL=").append(ri.get(RenderInstructions.KEY_APPLET_SUBMIT_URI)).append("&oResponseParam=qti").append(PARA)
                    .append(ri.get(RenderInstructions.KEY_ITEM_IDENT)).append(PARA).append(ri.get(RenderInstructions.KEY_RESPONSE_IDENT)).append(PARA).append("flash")
