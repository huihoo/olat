...
public class DeletionModule extends AbstractOLATModule {
	 ...
	 * 
	 */
    private synchronized void initializeEmailResponeTo() {
        if (emailResponseTo == null) {
            // 16.06.2011/cg: lazy-init because 'identity.getUser().getProperty(UserConstants.EMAIL, null);' needs UserService and CoreSpringFactory
            ...
            if (!emailResponseTo.contains("@")) {
                final Identity identity = baseSecurity.findIdentityByName(emailResponseTo);
                if (identity != null) {
                    emailResponseTo = userService.getUserProperty(identity.getUser(), UserConstants.EMAIL);
                } else {
