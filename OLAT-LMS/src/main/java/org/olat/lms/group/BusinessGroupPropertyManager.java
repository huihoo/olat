...
import org.olat.lms.properties.PropertyConstants;
import org.olat.system.commons.manager.BasicManager;

/**
 * Description:<BR>
 * Handles the displayMembers property assigned to BusinessGroups. It uses the float column in the property table, and stores there a bitfield.<br>
 * The DisplayMembers Property is used to switch on - off exposing the owner members, partipiciant members respectively, to the partipiciants of a BusinessGroup.
 * <p>
...
public class BusinessGroupPropertyManager extends BasicManager {
    ...
    private final int showOwnersVal = 1;// 0x..0001
    private final int showPartipsVal = 2;// 0x..0010
    private final int showWaitingListVal = 4;// 0x..0100

    // 0x..1000
    // next used int values should be 8, 16, ....
    //
    private final BusinessGroup businessGroup;
    private final NarrowedPropertyManager npm;
    private PropertyImpl myProperty; // local copy
     ...
     */
    public BusinessGroupPropertyManager(final BusinessGroup bg) {
     ...
     */
    public PropertyImpl createAndPersistDisplayMembers(final boolean showOwners, final boolean showPartips, final boolean showWaitingList) {
        long showXXX = 0;
        if (showOwners) {
            showXXX += showOwnersVal;
        }
        if (showPartips) {
            showXXX += showPartipsVal;
        }
        if (showWaitingList) {
            showXXX += showWaitingListVal;
        }
        final PropertyImpl prop = npm.createPropertyInstance(null, this.businessGroup, PropertyConstants.OLATRESOURCE_CONFIGURATION, PROP_NAME, null, new Long(showXXX),
                null, null);
     ...
     * @param showPartips
     */
    public void updateDisplayMembers(final boolean showOwners, final boolean showPartips, final boolean showWaitingList) {
        long showXXX = 0;
        if (showOwners) {
            showXXX += showOwnersVal;
        }
        ...
        if (this.myProperty == null) {
            ...
            this.myProperty = (PropertyImpl) DBFactory.getInstance().loadObject(this.myProperty);
        }
    ...
    /**
     * true if Members can see the Owners, false otherwise. If the property is not existing, i.e. called before the property was created and persisted, a null pointer
     * exception occurs.
     * 
    ...
    /**
     * true if Members can see the Partipiciants, false otherwise. If the property is not existing, i.e. called before the property was created and persisted, a null
    ...
    private int getDisplayMembersValue() {
        ...
        //
        final int showXXX = this.myProperty.getLongValue().intValue();
        return showXXX;
    }
     ...
     */
    private PropertyImpl findProperty() {
        PropertyImpl prop = npm.findProperty(null, businessGroup, PropertyConstants.OLATRESOURCE_CONFIGURATION, PROP_NAME);
    ...
    /**
     * @param sourceGroup
     *            The group from which the configuration should be copied
     */
    public void copyConfigurationFromGroup(final BusinessGroup sourceGroup) {
        final BusinessGroupPropertyManager sourceGPM = new BusinessGroupPropertyManager(sourceGroup);
        updateDisplayMembers(sourceGPM.showOwners(), sourceGPM.showPartips(), sourceGPM.showWaitingList());
    }

    /**
     * true if Members can see the Waiting, false otherwise. If the property is not existing, i.e. called before the property was created and persisted, a null pointer
     * exception occurs.
     * 
     * @return true if group participants are configured to be displayed
     */
