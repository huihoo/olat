...
public class BGRightManagerImpl extends BasicManager implements BGRightManager {
	 ...
	 */
    @Override
    public List findBGRights(final BusinessGroup rightGroup) {
        final BaseSecurity secm = getBaseSecurity();
        final List results = secm.getPoliciesOfSecurityGroup(rightGroup.getPartipiciantGroup());
        // filter all business group rights permissions. group right permissions
        // start with bgr.
        final List rights = new ArrayList();
        for (int i = 0; i < results.size(); i++) {
            final Policy rightPolicy = (Policy) results.get(i);
