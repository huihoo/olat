 ...
 * @author gnaegi
 */
class ContextHelpVisitor implements FileVisitor {
     ...
     *            hash map that offers a way to lookup the package of a page when only the page name is known (used for legacy help pages)
     * @param allContextHelpPages
     *            set that contains all help pages pathes
     * @param blacklist
     *            Set of page pathes that should not be indexed
     */
    ContextHelpVisitor(String basePathConfig, Map<String, String> contextHelpPagesLegacyLookupIndex, Set<String> allContextHelpPages, Set<String> blacklist) {
    ...
    public void visit(File file) {
        ...
        if (file.isFile()) { // regular file
            ...
            if (parentFile.getName().equals(CHELP_DIR)) {
                ...
                // add one for the '/'
                String classPath = pPath.substring(basePath.length()); // cg/29.03.2011: Remove '+1' to fix ERROR 'ResourceManager : unable to find resource' (this
                                                                       // comment can be removed later)
                ...
                try {
                    ...
                    log.error("Error while searching for context help file in a jar", e);
                }
            }
        }
        // else ignore
    ...
    private void addHelpPageToList(String toBeChechedkFilName, String classPath) {
        ...
        if (contextHelpPagesLegacyLookupIndex.containsKey(toBeChechedkFilName)) {
            log.debug(
                    "Detected two help pages with the same name::" + toBeChechedkFilName + ", in package::" + contextHelpPagesLegacyLookupIndex.get(toBeChechedkFilName)
                            + " and in package::" + classPath
