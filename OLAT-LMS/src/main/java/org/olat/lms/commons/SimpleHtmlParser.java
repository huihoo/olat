...
/**
 * Simple html parser that grabs css, javascript etc. stuff out of the header. Not very tolerant but at least very fast Initial Date: 11.07.2004
 * 
 * @author Mike Stock
 * @author guido
 */
public class SimpleHtmlParser {
    ...
    private static final Pattern COMMENT_OR_LINK = Pattern.compile("<(!--.*--|link[^>]*)>", Pattern.DOTALL | Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);

    private static final Pattern LINK_REL = Pattern.compile("rel\\s*=\\s*\"?(home|next|prev(ious)?|start|up)\"?", Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
     ...
     * @param file
     */
    public static String extractHTMLCharset(InputStream inputStream) { // VFSLeaf sourceFile.getInputStream()

        ByteArrayOutputStream target = new ByteArrayOutputStream(8192);
        FileUtils.copy(inputStream, target);
        FileUtils.closeSafely(inputStream);

        byte[] bytes = target.toByteArray();
        if (bytes.length == 0)
            return ISO_8859_1; // empty file
        ...
        String charset = null;
        if (bspos > 0)
            charset = checkForCharset(parse.substring(0, bspos));
        // if nothing found return default
        return StringHelper.containsNonWhitespace(charset) ? charset : ISO_8859_1;
    }

    /**
     * Parses the given string and looks for head, onload functions and the page body the parsed data is stored in the class variables
     * <ul>
     * <li />htmlHead
     * <li />jsOnLoad
     * <li />htmlContent
    ...
    private void parseContent(String cont) {
        ...
        if (spos == -1)
            spos = cont.indexOf("<HTML");
        if (spos == -1)
            spos = cont.toLowerCase().indexOf("<html");
        if (spos == -1) {
        ...
        if (bodypos == -1)
            bodypos = cont.indexOf("<BODY");
        if (bodypos == -1)
            bodypos = cont.toLowerCase().indexOf("<body");
        if (bodypos == -1) {
            // This is not valid HTML - assume whole file is the content
            htmlContent = cont;
            if (log.isDebugEnabled())
        ...
        if (hspos == -1)
            hspos = cont.indexOf("<HEAD"); // be tolerant, try also capitals
        if (hspos == -1)
            hspos = cont.toLowerCase().indexOf("<HEAD"); // be tolerant, try also mixed
        if (hspos != -1) { // look for end tag if start tag found
            int hepos = lookForTag(cont, hspos, "</head>");
            if (hepos > hspos)
                htmlHead = cont.substring(hspos + 6, hepos);
        ...
        if (htmlHead != null) {
            ...
            if (bsPos == -1)
                bsPos = htmlHead.indexOf("<BASE ");
            if (bsPos == -1)
                bsPos = htmlHead.toLowerCase().indexOf("<BASE ");
            if (bsPos != -1) {
            ...
            if (htmlHead.indexOf("text/css") > 0)
                ownCss = true;
        ...
        if (jspos == -1)
            jspos = cont.indexOf("onLoad=", bodypos);
        if (jspos == -1)
            jspos = cont.toLowerCase().indexOf("onload=", bodypos);
    ...
    private static String checkForCharset(String hdr) {
        ...
        /*
         * from http://www.w3.org/TR/html401/charset.html#doc-char-set <META http-equiv="Content-Type" content="text/html; charset=EUC-JP"> The META declaration must only
         * be used when the character encoding is organized such that ASCII-valued bytes stand for ASCII characters (at least until the META element is parsed). META
         * declarations should appear as early as possible in the HEAD element.
         */
    ...
    /**
     * Looks for a tag (both lower and upper case). Returns -1 if not present, tag position within content, otherwise.
     * 
     * @param tag
    ...
    private static int lookForTag(String content, int offset, String tag) {
        ...
        while (true) {
            ...
            if (tagpos == -1)
                break;
            if (!isWithinComment(content, offset, tagpos))
                break;
            else {
    ...
    /**
     * Check if source beginning at startPos and ending at endPos has a comment start tag which is not (yet) closed
     * 
     * @param startPos
     * @param endPos
