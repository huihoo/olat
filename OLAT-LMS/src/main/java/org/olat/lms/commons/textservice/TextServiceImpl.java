...
import org.olat.lms.commons.textservice.nutch.LanguageIdentifier;
import org.springframework.stereotype.Service;

/**
 * Description:<br>
 * The language guess is based on a NGRam algorithm. The implementation is based on a plugin of nutch (an apache project). To create a profile for a new language, do
 * collect a sample text of the language. It need a lot of text (the standard are done with more than a millions of words). Then create the profile:<br>
 * java .nutch.NGramPofile -create profile-name filename encoding<br>
...
public class TextServiceImpl implements TextService {
    ...
    /**
     * Return the locale found by a NGram analyse. The different profile are saved in _resources. The best result are with longer text, other more then 100 characters.
     */
    @Override
    ...
    /**
     * Use the java.text.BreakIterator to count the number of words. There is an excpetion for chinese language because only a human count reliably count the number of
     * words in a chinese text (Word are not separated by space in Chinese, Japanese and Thai). The different entreprise involved in traduction count the words with the
     * number of characters and a factor 2.2.
     * 
     * @param text
     * @param locale
     * @return
