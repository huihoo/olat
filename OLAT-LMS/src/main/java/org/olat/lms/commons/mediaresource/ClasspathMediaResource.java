 ...
 * from jar). Normally you just use the JsAndCSSComponent or the ClassPathStaticDispatcher
 * 
 * @author Felix Jost
 */
public class ClasspathMediaResource implements MediaResource {
     ...
     * @param location
     *            the relative file path (e.g. _static/my/file.css)
     */
    ...
    public InputStream getInputStream() {
        ...
        try {
            ...
            log.warn("cannot get inputstream for url:" + url.toExternalForm()
                    + "Unknown host which starts with windows path like \"C\" points to a wrong configured brasato.src in the olat.properties file", null);
        } catch (IOException e) {
            log.warn("cannot get inputstream for url:" + url.toExternalForm(), null);
        }
        return is;
    }
    ...
    /**
     * @return true: resource exists and can be delivered; false: resource could not be found, delivery will fail
     */
