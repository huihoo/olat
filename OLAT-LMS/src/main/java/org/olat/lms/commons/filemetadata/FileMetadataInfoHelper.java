 ...
 * @author srosse
 */
public class FileMetadataInfoHelper {
     ...
     * @return
     */
    private static boolean isLocked(VFSItem item, IdentityEnvironment identityEnv) {
    ...
    public static boolean isLocked(VFSItem item, Identity identity, boolean isAdmin) {
        ...
        if (item instanceof OlatRelPathImpl) {
            ...
            info = metaInfoService.createMetaInfoFor((OlatRelPathImpl) item);
    ...
    public static List<String> hasLockedFiles(VFSContainer container, FileSelection selection, UserRequest ureq) {
        ...
        for (String file : selection.getFiles()) {
            ...
            if (isLocked(item, ureq.getIdentity(), ureq.getUserSession().getRoles().isOLATAdmin())) {
