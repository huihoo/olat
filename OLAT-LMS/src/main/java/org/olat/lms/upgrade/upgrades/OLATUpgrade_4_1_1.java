...
public class OLATUpgrade_4_1_1 extends OLATUpgrade {
    ...
    private void replaceOldInternalCourseLinksWithNewForm(final UpgradeManager upgradeManager, final UpgradeHistoryData uhd) {

        if (!uhd.getBooleanDataValue(TASK_REPLACE_OLDINTERNALLINKS)) {
            log.info("Audit:+---------- replace internal links -----------+");
            ...
            final File coursesRoot = new File(bcRootDir, "course");
            final String[] courses = coursesRoot.list();
            for (int i = 0; i < courses.length; i++) {
                final String course = courses[i];
                final File courseRoot = new File(coursesRoot, course);
                final File cFolder = new File(courseRoot, "coursefolder");
                log.info("Audit:visiting " + cFolder.getAbsolutePath());
                final FileVisitor fv = new FileVisitor() {
                    ...
                    public void visit(final File file) {
                        ...
                        if ((file.isFile()) && (fname.endsWith(".htm") || fname.endsWith(".html"))) {
                            ...
                            final String page = FileUtils.load(file, DEFAULT_ENCODING);
                            ...
                            // <meta.*charset=([^"]*)"
                            final Matcher m = PATTERN_ENCTYPE.matcher(page);
                            final boolean found = m.find();
                            if (found) {
                                // found an encoding definition
                                final String htmlcharset = m.group(1);
                            ...
                            if (useLoaded) {
                                ...
                                try {
                                    ...
                                    log.info("Audit:ERROR: could not load file " + file.getAbsolutePath() + " ENCTYPE: " + enc);
                                    return;
                                }
                            }
                            ...
                            final Matcher ma = OLDLINK.matcher(rpage);
                            final StringBuffer sb = new StringBuffer();
                            int changed = 0;
                            final StringBuilder links = new StringBuilder();
                            while (ma.find()) {
                                // int gp = ma.groupCount();
                                final String mm = ma.group(0);
                                final String id = ma.group(2);
                            ...
                            if (changed > 0) {
                                ma.appendTail(sb);
                                final String repl = sb.toString();
                                FileUtils.save(file, repl, enc);
                                log.info("Audit:file " + file.getAbsolutePath() + " :: " + changed + " links changed :" + links);
                            }
                        }
                    }
                };
