...
public class CourseNodeIndexerFactory {
    ...
    static {
        INSTANCE = new CourseNodeIndexerFactory();
    }
    ...
    public void registerIndexer(final CourseNodeIndexer indexer) {
     ...
     */
    public CourseNodeIndexer getCourseNodeIndexer(final CourseNode node) {
        final String courseNodeName = node.getClass().getName();
        final CourseNodeIndexer courseNodeIndexer = indexerMap.get(courseNodeName);
        if (courseNodeIndexer != null) {
            return courseNodeIndexer;
        } else {
            if (log.isDebugEnabled()) {
                log.debug("No indexer found for node=" + node);
            }
            return null;
        }
    }
