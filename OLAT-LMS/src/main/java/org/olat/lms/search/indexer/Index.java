...
public class Index {
    ...
    private final SearchSpellChecker spellChecker;

    /**
     * @param indexPath
     *            Absolute directory path of search index.
     * @param tempIndexPath
     *            Absolute directory path of temporary index.
     * @param restartInterval
     *            Restart interval of full-index in milliseconds.
     * @param indexInterval
     *            Sleeping time in milliseconds between adding documents to index.
     */
    public Index(final SearchModule searchModuleConfig, final SearchSpellChecker spellChecker, final MainIndexer mainIndexer) {
    ...
    private void moveTempIndexToIndex(final String tempIndexPath, final String indexPath) {
        final File indexDir = new File(indexPath);
        if (!indexDir.exists()) {
            indexDir.mkdirs();
        }
        if (log.isDebugEnabled()) {
            log.debug("Copy new generated Index from '" + tempIndexPath + "/main" + "' to '" + indexPath + "'");
        }
     ...
     * 
     * @param indexInterval
     *            New index-interval in milliseconds.
     */
