...
public class EmailBuilder {
    ...
    @Value("${olat.web.url}")
    private String olatWebUrl;

    @Autowired
    private UriBuilder uriBuilder;

    String getSubjectForImmediatelyNotification(Locale locale) {
        return getLocaleTranslatorForUserEmail(locale).translate("mail.subject");
    }
    ...
    private MailBody getMailBody(List<NotificationEventTO> eventToList) {
        Locale locale = getUserLocaleFromListOfEvents(eventToList);
        return new MailBody(eventToList, getLocaleTranslatorForUserEmail(locale), olatWebUrl, uriBuilder);
    }
    ...
    private Translator getLocaleTranslatorForUserEmail(Locale locale) {
        return PackageUtil.createPackageTranslator(EmailBuilder.class, locale);
    }
