...
/**
 * <h3>Description:</h3> This login name and password syntax checker provides methods to check if a user name and a password is syntactically correct. Inherit from this
 * class and configure your own checker in the spring user manager configuration to allow different user names that the default (e.g. if you want to use email addresses
 * as login names)
 * <p>
...
public class UserNameAndPasswordSyntaxCheckerWithEmail extends UserNameAndPasswordSyntaxChecker {
    ...
    /**
     * Validates an OLAT password on a syntactical level. A password is valid when it matches the following requrements:<br>
     * <ol>
     * <li />Only numbers (0-9), letters (a-z, A-Z) and the following special characters (!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~) are allowed
    ...
    public boolean syntaxCheckOlatPassword(final String password) {
        final Pattern p = Pattern.compile("^\\p{Graph}{4,128}$");
    ...
    /**
     * Checks for a valid login name. The method does only check if the name is syntactically correct. It does not check if the name is valid
     * 
     * @param login
