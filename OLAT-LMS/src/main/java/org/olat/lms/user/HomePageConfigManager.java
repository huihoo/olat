...
public interface HomePageConfigManager extends UserDataDeletable {
    ...
    /**
     * Load the configuration of homepage. If the configuration is not existing, a new default homepage configuration is created, initialized with default values and
     * persisted. Otherwise the configuration is loaded from the file system. If the <code>HomePageConfig</code>'s version does not match the loaded configuration version
     * a procedure for converting/migrating to the new version is initiated.
     * 
     * @param userName
