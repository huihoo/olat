...
import org.olat.lms.search.searcher.SearchClientProxy;
...
public class EPFrontendManager extends BasicManager {
	...
	public boolean moveStructureToNewParentStructure(
			final PortfolioStructure structToBeMvd,
			final PortfolioStructure oldParStruct,
            final PortfolioStructure newParStruct, int destinationPos) {
		if (structToBeMvd == null || oldParStruct == null
				|| newParStruct == null) {
    ...
    public PortfolioStructureMap assignStructuredMapToUser(final Identity identity, final PortfolioStructureMap mapTemplate, final OLATResourceable targetOres,
        ...
        final PortfolioStructureMap map = coordinator.getSyncer().doInSync(template.getOlatResource(), new SyncerCallback<PortfolioStructureMap>() {
            ...
            public PortfolioStructureMap execute() {
				...
				final PortfolioStructureMap copy = structMgr.createPortfolioStructuredMap(template, author, title, description, ores, subPath,
						targetBusinessPath);
    ...
    /**
	 * Get all Structure-Elements linked which the identity can see over a policy, WITHOUT those that are public to all OLAT users ( GROUP_OLATUSERS ) !! this should
	 * be used, to save performance when there are a lot of public shared maps!!
     * 
     * @param ident
     *            The identity which what see maps
     * @param chosenOwner
     ...
     * @return
     */
	public boolean isTemplateInUse(final PortfolioStructureMap template, final OLATResourceable targetOres, final String targetSubPath,
			final String targetBusinessPath) {
    ...
    /**
	 * The structure will be without any check on the DB copied. All the children structures MUST be loaded. This method is to use with the output of XStream at
	 * examples.
     * 
     * @param root
     * @param identity
     ...
     * For the latter case we check here if a corresponding "Learning journal" exists as an e-portfolio artefact and the given identity has the right to access it.  
