 ...
 * @author Stefan
 */
public class HomeOrgStatisticDaoImpl extends BasicManager implements IStatisticManager {
    ...
    public StatisticResult generateStatisticResult(final Locale locale, final ICourse course, final long courseRepositoryEntryKey) {
        final DBQuery dbQuery = DBFactory.getInstance().createQuery(
                "select businessPath,homeOrg,value from org.olat.data.course.statistic.homeorg.HomeOrgStat sv " + "where sv.resId=:resId");
    ...
    public STATISTIC_TYPE getStatisticType() {
        return STATISTIC_TYPE.HOME_ORG;
    }
