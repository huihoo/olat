 ...
 * @author Stefan
 */
public class OrgTypeStatisticDaoImpl implements IStatisticManager {
    ...
    public StatisticResult generateStatisticResult(final Locale locale, final ICourse course, final long courseRepositoryEntryKey) {
        final DBQuery dbQuery = DBFactory.getInstance().createQuery(
                "select businessPath,orgType,value from org.olat.data.course.statistic.orgtype.OrgTypeStat sv " + "where sv.resId=:resId");
    ...
    public STATISTIC_TYPE getStatisticType() {
        return STATISTIC_TYPE.ORG_TYPE;
    }
