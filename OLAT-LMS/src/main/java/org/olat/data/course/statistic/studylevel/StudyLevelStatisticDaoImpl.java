...
package org.olat.data.course.statistic.studylevel;
 ...
 * @author Stefan
 */
public class StudyLevelStatisticDaoImpl extends BasicManager implements IStatisticManager {
    ...
    public StatisticResult generateStatisticResult(final Locale locale, final ICourse course, final long courseRepositoryEntryKey) {
        final DBQuery dbQuery = DBFactory.getInstance().createQuery(
                "select businessPath,studyLevel,value from org.olat.data.course.statistic.studylevel.StudyLevelStat sv " + "where sv.resId=:resId");
    ...
    public STATISTIC_TYPE getStatisticType() {
        return STATISTIC_TYPE.STUDY_LEVEL;
    }
