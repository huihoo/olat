 ...
 * <p>
 * Note that this will <b>not work with big tables</b> as you have with big installations. The reason being that when the resulting log set is big, it both requires a
 * huge amount of memory plus the loop which happens in this class takes forever (minutes).
 * <P>
...
public class SimpleLogExporter implements ICourseLogExporter {
    ...
    /** injected by spring **/
    public void setLogLineConverter(final LogLineConverter logLineConverter) {
    ...
    public void exportCourseLog(final File outFile, final String charset, final Long resourceableId, final Date begin, Date end, final boolean resourceAdminAction,
        ...
        String query = "select v from LoggingObject v " + "where v.resourceAdminAction = :resAdminAction " + "AND ( " + "(v.targetResId = :resId) OR "
                + "(v.parentResId = :resId) OR " + "(v.grandParentResId = :resId) OR " + "(v.greatGrandParentResId = :resId) " + ")";
        ...
        final List queryResult = dbQuery.list();
        result.append(logLineConverter_.getCSVHeader());
        result.append(LINE_SEPARATOR);

        for (final Object loggingObject : queryResult) {
