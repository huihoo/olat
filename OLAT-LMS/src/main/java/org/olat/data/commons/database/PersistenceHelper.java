...
public class PersistenceHelper {
    ...
    /**
     * Truncate the given original string to the defined max length. The method does also check if some legacy UTF-8 conversion is done by the database driver and
     * shortens the allowed length to two third to be on the save side
     * 
     * @param original
     *            The original String
     * @param maxLength
     ...
     * @param showThreeDots
     *            Replace the last three characters with ... to indicate that the string has been truncated
    ...
    /**
     * FIXME:fj:c cleanup up these methods here and class SyncHelper Checks if the given persistable is in the given iterator (database identity). If so, the persistable
     * from the iterator is returned. If not, null is returned.
     * 
     * @param iter
     *            Iterator of persistable objects
     * @param persistable
     *            The persistable object that is looked for in the iterator
     * @return null if not found or the persistable object that has the same key as the given persistable object. The object might also have object identity, but this is
     *         not guaranteed.
     */
    ...
    /**
     * Iterates over a list to see if the given persistable object is already in this list. This differs from list.contains() in the way that it does not check object
     * identity but hibernate identity. The object list contains the object if it contains any object with object.getKey() equals persistable.getKey()
     * 
     * @param objects
     ...
     * 
     * @param objects
     *            List ob original objects
     * @param toBeReplacedObject
     *            The object that should be searched for in the list and the replace value
     ...
     * @return int number of objects removed from the originalList After calling this operation the originalList will contain less or the same amount of objects
     */
