...
/**
 * Description:<br>
 * Helper class that checks on startup if transactions are supported on mysql. Which means you run your tables with the innoDB engine.
 * <P>
...
public class InnoDBAwareDriverManagerDataSource extends DriverManagerDataSource implements Initializable {
    ...
    public void init() {
        ...
        try {
            log.info("Audit:Checking whether mysql tables support transactions based on innoDB tab...");
            ...
            if (createTableCommand.contains("InnoDB")) {
                log.info("Audit:Your mysql tables look like they support transactions, fine!");
            } else {
                throw new StartupException(
                        "Your tables do not support transactions based on innoDB tables. Check your database server and enable innoDB engine! Your table currently runs: "
                                + createTableCommand);
            }

        } catch (SQLException e) {
            if (e.getMessage().contains("doesn't exist")) {
                log.info("Audit:o_plock table does not yet exist, will check transaction support on next startup");
            } else {
