...
/**
 * Description:<BR>
 * A business group context is a container for groups that are used together as a set. Eg. all groups of a course, or a class with some subgroups, or a Lehrgang /
 * Studiengang. Group contexts can be attached many courses. The group context can contain only business groups of one specific business group type. Default group
 * contexts can only be used for one resource (one association allowed). This is configured by the setDefaultContext() method.
 * <P>
...
public interface BGContext extends Persistable, CreateInfo, OLATResourceable {
    ...
    /**
     * @return boolen true: this is a default context, only one relation to a resource allowed
     */
