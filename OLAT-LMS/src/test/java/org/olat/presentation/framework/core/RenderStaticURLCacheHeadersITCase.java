...
/**
 * Description:<br>
 * downloads the entry page from olat*.uzh.ch and checks if the response headers are sent with proper cache control entries like: Cache-Control: max-age=31536000, public
 * This allows browsers to cache static resources and minimize traffic which returns 304 (not modified). If the tests fails make sure that http.conf from apache has the
 * following entry <FilesMatch "\.(ico|pdf|flv|jpg|jpeg|png|gif|js|css|swf)$"> Header set Cache-Control "max-age=31536000, public" </FilesMatch>
 * <P>
 * Initial Date: 17.01.2011 <br>
 * 
 * @author guido
 */
@Ignore("instable: fails if server not available / connection timeout: its a Smoketest (after setting up a new Environment) or AfterGoLiveTest or SystemTCase - decide with Alex / Guido where to place the test.")
public class RenderStaticURLCacheHeadersITCase {

    private String[] uris = { "https://olat.uzh.ch/" };
    ...
    public void loadFromUrl(String uri) throws IOException, MalformedURLException, ParseException {
        ...
        String jsUrl = inputLine.substring(inputLine.indexOf("src=\"") + 6, inputLine.lastIndexOf("\""));
