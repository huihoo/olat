...
public class Sniffer {
    ...
    private static void usage(final String errorMsg) {
        ...
        System.out.println("Usage: java org.olat.system.coordinate.jms.Sniffer --brokerUrl brokerUrl --topicName topicName [--topicName ...]");
        System.out.println("");
        System.out.println(" Hint: brokerUrl is something like tcp://localhost:61616");
        System.exit(1);
    }

    private static void go(final String brokerUrl, final String topicName) throws JMSException {
        final ActiveMQConnectionFactory connectionFactory = new ActiveMQConnectionFactory();
        connectionFactory.setBrokerURL(brokerUrl);
        final Connection connection = connectionFactory.createConnection();
        final ActiveMQTopic topic = new ActiveMQTopic(topicName);
        final MessageConsumer consumer = connection.createSession(false, Session.AUTO_ACKNOWLEDGE).createConsumer(topic);
        connection.start();
        while (true) {
            ...
            if (!(m instanceof ObjectMessage)) {
                ...
                final ObjectMessage objMsg = (ObjectMessage) m;
                final Object o = objMsg.getObject();
                if (!(o instanceof JMSWrapper)) {
                    System.out.println("Received a non-JMSWrapper: " + o + ", msg: " + m);
                } else {
                    final JMSWrapper jmsWrapper = (JMSWrapper) o;
                    System.out.println("Received a JMSWrapper: time=" + new Date() + ", msgid=" + jmsWrapper.getMsgId() + ", nodeId=" + jmsWrapper.getNodeId()
                            + ", event=" + jmsWrapper.getMultiUserEvent() + ", msg: " + m);
                }
            }
        }
    }
