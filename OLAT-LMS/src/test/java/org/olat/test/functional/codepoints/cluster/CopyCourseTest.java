 ...
 * Administrator and author create 2 copy of the same course simultaneously, so if the administrator starts to copy and the thread_A pauses at codepoint, the author finds
 * the course locked when tries to copy. Author succeds to copy after the thread_A is continued. <br/>
 * 
 * 13.09.2010: fixed to conform with the new code behaviour (a course copy is already created after click copyLR, before insert new title/description and OK).
 * 
...
public class CopyCourseTest extends BaseSeleneseTestCase {
    ...
    public void testCopyCourse() throws Exception {
        ...
        CodepointRef longCopyStartedCp_A = codepointClient_A.getCodepoint("org.olat.lms.course.CourseFactory.copyCourseAfterSaveTreeModel");
        ...
        for (int second = 0;; second++) {
            ...
            try {
                ...
                if (selenium_B.isTextPresent("This course is currently locked by") || selenium_B.isTextPresent("This course is currently edited by"))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
