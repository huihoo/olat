...
/**
 * 
 * User creates course and tests case tests if he can add owner and participants. <br/>
 * Test setup: <br/>
 * 1. Clean-up: delete all groups from author 2. Create test user
 * <p>
 * Test case: <br/>
 * 1. author creates group GROUP_NAME and adds users and owners 2. student logs in, checks if he's in GROUP_NAME and leaves GROUP_NAME 3. author deletes group <br/>
 * 
 * 
 * @author sandra
 * 
    ...
    public void testCreateProjectGroupAddUsersTest() throws Exception {
        ...
        for (int second = 0;; second++) {
            if (second >= 60)
                fail("timeout");
            try {
                if (groupsTab.getSelenium().isElementPresent("ui=groups::content_leaveGroup(nameOfGroup=project group selenium 3)"))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
