...
/**
 * Tests that no course export, copy and edit could occur concurrently - clusterwide - but protected by a GUI lock. <br/>
 * Test setup: <br/>
 * - admin clones DEMO_COURSE_NAME and adds author as owner to this. <br/>
 * - admin and author login on separate nodes (Theoretically, since this depends of the multiVmOlatUrl1 and multiVmOlatUrl2 in customcontext.xml) <br/>
 * Test case: <br/>
...
public class ClusteredCourseExportTest extends BaseSeleneseTestCase {
    ...
    /**
     * Tests that no course export, copy and edit could occur concurently - clusterwide - but protected by a GUI lock. Steps: - author opens "Demo Course" and start
     * editing course - administrator wants to export course - but it is locked - author closes course editor - administrator exports course - threadA stops at codepoint
     * - - author want to open course editor but course locked for export - threadA stopped at codepoint - - continue threadA - exports finishes - lock released - author
     * opens successfully course editor - logout
     * 
     * @throws Exception
     */
    public void testExportCourse() throws Exception {
        ...
        CodepointRef longExportStartedCp_A = codepointClient_A.getCodepoint("org.olat.lms.course.CourseFactory.longExportCourseToZIP");
        ...
        for (int second = 0;; second++) {
            if (second >= 120)
                fail("timeout"); // WARNING: HERE POTENTIAL FAILURE EACH TIME THE TRANSLATION CHANGES!
            try {
                // Make sure that the string really exists in the _en property file
                if (selenium_B.isTextPresent("This course is being locked by") || selenium_B.isTextPresent("This course is being edited by"))
                    break;
            } catch (Exception e) {
            }
            Thread.sleep(1000);
        ...
        for (int second = 0;; second++) {
            if (selenium_B.isTextPresent("This course was last published on"))
                break;
            if (second >= 120) {
