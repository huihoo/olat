...
/**
 * OLAT abstraction for the Learning Resources tab. Provides methods for the most common workflows.
 * 
...
public class LearningResources extends OLATSeleniumWrapper {
    ...
    public enum LR_Types {
        COURSE, TEST, WIKI, GLOSSARY, QUESTIONNAIRE, RESOURCE_FOLDER, BLOG, PODCAST, CP
    }
    ...
    /**
     * Select "Search form" and find the resource with the given title if provided, and with the given author if any provided. The title and the author could be null if
     * not used for filtering. TODO : LD: filter after the resource type <br/>
    ...
    public LRDetailedView searchResource(String title, String author) {
        ...
        /*
         * if(selenium.isChecked("ui=learningResources::content_searchForm_courseType()")) { selenium.uncheck("ui=learningResources::content_searchForm_courseType()"); }
         */
    ...
    /**
     * Select "My Entries", if too many [Show all] link present, click [Show all], and select the entry with the input name.
     * 
     * @param selenium
     * @param entryName
     * @return
    ...
    /**
     * Selects the "Courses" link, and next clicks on "Show content" of the course with the given title. It assumes that the course is visible on the first page of the
     * courses list.
     * 
     * @return
     */
     ...
     * 
     * Click the Catalog entry on the left hand menu click it twice so the catalog is refreshed
     * 
    ...
    /**
     * Imports a course with the given title only if there is no course with this title. Opens course editor and publish course.
     * 
     * @param zippedCourse
    ...
    /**
     * Import CP and open CP editor. Attention: It could return null, if a resource with the same name already exists! Use unique names for resources!!!
     * 
    ...
    /**
     * Selects the courseIndex course in the next page, if desired, and returns a CourseRun if any found or null otherwise.
     * 
    ...
    /**
     * Select courses menu item ONLY if not already selected. Motivation: the implementation changed on 14.12.2010: Was: if the courses was on second page, it remained on
     * the second page upon new selection. Now: each selection of courses leads to the first page.
     * 
     */
