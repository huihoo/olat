...
public abstract class Context {
    ...
    static {
        ...
        new ClassPathXmlApplicationContext(new String[] { "classpath*:customcontext.xml" });
        if (contexts_.size() == 0) {
            new ClassPathXmlApplicationContext(new String[] { "classpath*:defaultcontext.xml" });
        }
        System.out.println("DONE.");
    }

    /**
     * The SeleniumManager is used for creating and closing Selenium instances. The abstract Context base class sets up this instance before the Context is used for
     * creating selenium instances the first time.
     */
    ...
    /**
     * Try to find out if it is a KnownIssueException somewhere in the olat.log - and if so, don't report it as the original failure but as a known issue so we
     * immediately know from the test result that it is a known issue. <br/>
     * Returns by default false, that is "Do not mask the test failure if ERROR encountered in olat.log". <br/>
    ...
    public static boolean maskTestFailureOrError(Throwable th) throws Exception {
        ...
        if (c.seleniumManager_.anySeleniumBrowserHasKnownIssue3857()) {
            throw new AssertionFailedError(
                    "Known Issue 3857 encountered. (\"Please do not use the `Reload` or `Back` button of your browser.\" encountered in one of the browsers)");
        }
        return false;
    }
     ...
     * @return
     */
    public OlatLoginInfos createuserIfNotExists(int nodeId, String username, String password, boolean isSystemUser, boolean userManagementRole,
            boolean groupManagementRole, boolean authorRole, boolean systemAdminRole) {
        ...
        try {
            return WorkflowHelper.createUserIfNotExists(getStandardAdminOlatLoginInfos(nodeId), username, password, isSystemUser, userManagementRole,
                    groupManagementRole, authorRole, systemAdminRole);
     ...
     * @return
     */
    public OlatLoginInfos createuserIfNotExists(int nodeId, String username, boolean isSystemUser, boolean userManagementRole, boolean groupManagementRole,
    ...
    protected OlatLoginInfos createLoginInfos(int nodeId, String username, String password) {
        ...
        try {
            return new OlatLoginInfos(getConfigProperty("seleniumHost"), getConfigProperty("browserId"), olatUrl, username, password);
