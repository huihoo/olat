...
public class TestRun extends OLATSeleniumWrapper {
    ...
    public TestElement finishTest(boolean assertAchievedScore, int referenceScore) throws Exception {
        ...
        for (int second = 0;; second++) {
            if (second >= 60)
                fail("timeout");
            if (selenium.isTextPresent("Your results were saved"))
                break;
            Thread.sleep(1000);
        }
        // check the score
    ...
    /**
     * Waits for confirmation: your results were saved, and closes test. The self test finishes silently, no explicit finish test command needed.
     * 
     * @throws Exception
     */
    ...
    /*
     * public TestElement closeTest() { selenium.click("ui=qti::closeTest()"); selenium.waitForPageToLoad("30000"); return new TestElement(selenium); }
     */
    ...
    /**
     * Fill in the answer for the given text fragment. TODO: LD: check if it works with more than 1 text fragments.
     * 
     * @param textFragment
     * @param answer
     */
    ...
    /**
     * Select in the KPrim answers. The input map contains as keys the answer text and as value a Boolean for correct or not.
     * 
