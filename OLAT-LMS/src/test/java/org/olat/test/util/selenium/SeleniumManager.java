...
public class SeleniumManager {
    ...
    public Selenium createSelenium(OlatLoginInfos loginInfos) {
        ...
        for (int i = 1; i < 6; i++) {
            try {
                ...
                selenium = new LoggingSeleniumWrapper(selenium, log);
        ...
        if (selenium.isTextPresent("OLAT-Benutzername")) {
            ...
            for (int second = 0;; second++) {
                if (second >= 120)
                    throw new AssertionError("timeout while checking for language change to English to occur");
                try {
    ...
    public void closeSeleniums() {
        ...
        for (Iterator<Selenium> it = seleniums_.iterator(); it.hasNext();) {
            ...
            /*
             * try{ String filename = System.currentTimeMillis()+".png"; System.out.println("Capturing screenshot into: "+filename);
             * s.captureEntirePageScreenshot(filename); } catch(Exception e) { System.out.println("Couldn't capture screenshot :(  - "+e); }
             */
        }
