...
public class UserManagement extends OLATSeleniumWrapper {
    ...
    private final int numberOfClickAttempts = 5;
    ...
    /**
     * Creates user without check whether already exists. Language: EN. Should this check if user exists?
     * 
     * @param username
     * @param firstname
     * @param lastname
    ...
    /**
     * Imports users, the parameter is a excel like string, see the GUI example. Imports only the new users. If at least one imported (new) returns true, else false.
     * 
    ...
    public boolean importAllUsers(String rowsFromExcelString) {
        ...
        /* TODO made loop workaround to click Finish button while is not correctly done */
        while (selenium.isElementPresent("ui=userManagement::content_importUsers_finish()")
                && !selenium.isElementPresent("ui=userManagement::content_importUsers_importWarnIcon()") && importCount < numberOfClickAttempts) {
        ...
        log.info("import users attempt count was " + importCount);
        return importCount == 0 ? false : true;
    }
    ...
    /**
     * Imports only the new users from the rowsFromExcelString. It checks that there are already existing users in the input data set.
     * 
     * @param rowsFromExcelString
    ...
    /**
     * Deletes user if the user deletion workflow is not locked and if a user is found. It assumes that a user identified by username exists, so call searchUser
     * beforehand.
     * 
     * @param username
