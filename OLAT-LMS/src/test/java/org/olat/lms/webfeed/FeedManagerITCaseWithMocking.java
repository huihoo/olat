...
import org.olat.data.commons.database.DBImpl;
...
public class FeedManagerITCaseWithMocking extends OlatTestCase {
    ...
    private void setupMocking() {
        final DBQuery query = Mockito.mock(DBQueryImpl.class);
        final DBImpl mockDB = Mockito.mock(DBImpl.class);
        Mockito.when(mockDB.createQuery(Matchers.anyString())).thenReturn(query);

        resourceManager.setDbInstance(mockDB);

        final PessimisticLockDao mockLock = Mockito.mock(PessimisticLockDao.class);
