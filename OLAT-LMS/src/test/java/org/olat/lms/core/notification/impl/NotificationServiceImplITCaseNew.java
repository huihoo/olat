...
public class NotificationServiceImplITCaseNew extends AbstractTransactionalJUnit4SpringContextTests {
    ...
    @Test
    @Ignore
    /** TODO: set to ignore and should be deleted because domain logic is changed - no more delete subscription (and notification events) when unsubscribe **/
    public void unsubscribe_ifExistingEvents() {
        ...
        List<NotificationEvent> events = notificationEventDao.findAllNotificationEvents();
        System.out.println("existing events: " + events.size());
        assertEquals("Wrong number of events found: ", 2, events.size());

        daoObjectMother.publishEventAsClara(notificationServiceImpl);
        events = notificationEventDao.findAllNotificationEvents();
        System.out.println("existing events: " + events.size());
        assertEquals("Wrong number of events found: ", 4, events.size());

        List<Subscription> subscriptions = notificationServiceImpl.getSubscriptions(daoObjectMother.getSubscriberIdentityAlf());
        Subscription subscription = subscriptions.get(0);
        assertEquals("Wrong number of events for alf: ", 2, subscription.getNotificationEvents().size());

        for (NotificationEvent event : subscription.getNotificationEvents()) {
            System.out.println("event status: " + event.getStatus().name());
        }

        notificationServiceImpl.unsubscribe(daoObjectMother.getAlfNotificationSubscriptionContext());
        events = notificationEventDao.findAllNotificationEvents();
        System.out.println("existing events after ALF unsubscribed: " + events.size());
        assertEquals("Wrong number of events found after ALF unsubscribed: ", 2, events.size());

        subscriptions = notificationServiceImpl.getSubscriptions(daoObjectMother.getSubscriberIdentityAlf());
        assertTrue("Alf should have no subscription anymore", subscriptions.size() == 0);
